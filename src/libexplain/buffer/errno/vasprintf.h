...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_VASPRINTF_H
#define LIBEXPLAIN_BUFFER_ERRNO_VASPRINTF_H
...
/**
  * The explain_buffer_errno_vasprintf function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_vasprintf(explain_string_buffer_t *sb, int errnum,
    char **data, const char *format, va_list ap);

/**
  * The explain_buffer_errno_vasprintf_explanation function is used to
  * obtain the explanation (the part after "because") of an error returned
  * by the <i>vasprintf</i>(3) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
  ...
  *     system call.
  */
void explain_buffer_errno_vasprintf_explanation(explain_string_buffer_t *sb, int
    errnum, const char *syscall_name, char **data, const char *format, va_list
    ap);

/* vim: set ts=8 sw=4 et : */
#endif /* LIBEXPLAIN_BUFFER_ERRNO_VASPRINTF_H */
