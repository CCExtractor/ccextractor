...
#include <libexplain/buffer/errno/strtoul.h>
...
static void
explain_buffer_errno_strtoul_system_call(explain_string_buffer_t *sb,
    int errnum, const char *nptr, char **endptr, int base)
{
    (void)errnum;
    explain_string_buffer_puts(sb, "strtoul(nptr = ");
...
static void
explain_buffer_errno_strtoul_explanation(explain_string_buffer_t *sb,
    int errnum, const char *nptr, char **endptr, int base)
{
    /*
     * http://www.opengroup.org/onlinepubs/009695399/functions/strtoul.html
     */
    switch (errnum)
    {
    case EINVAL:
    ...
    default:
        generic:
        explain_buffer_errno_generic(sb, errnum, "strtoul");
        break;
    }
}
...
void
explain_buffer_errno_strtoul(explain_string_buffer_t *sb, int errnum,
    const char *nptr, char **endptr, int base)
{
    explain_explanation_t exp;

    explain_explanation_init(&exp, errnum);
    explain_buffer_errno_strtoul_system_call(&exp.system_call_sb, errnum, nptr,
        endptr, base);
    explain_buffer_errno_strtoul_explanation(&exp.explanation_sb, errnum, nptr,
        endptr, base);
