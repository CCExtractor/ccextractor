...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_LINKAT_H
#define LIBEXPLAIN_BUFFER_ERRNO_LINKAT_H
...
/**
  * The explain_buffer_errno_linkat function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_linkat(explain_string_buffer_t *sb, int errnum, int
    old_fildes, const char *old_path, int new_fildes, const char *new_path, int
    flags);

/**
  * The explain_buffer_errno_linkat_explanation function is used to obtain
  * the explanation (the part after "because") of an error returned by the
  * <i>linkat</i>(2) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
  ...
  *     system call.
  */
void explain_buffer_errno_linkat_explanation(explain_string_buffer_t *sb, int
    errnum, const char *syscall_name, int old_fildes, const char *old_path, int
    new_fildes, const char *new_path, int flags);

/* vim: set ts=8 sw=4 et : */
#endif /* LIBEXPLAIN_BUFFER_ERRNO_LINKAT_H */
