...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_TCGETATTR_H
#define LIBEXPLAIN_BUFFER_ERRNO_TCGETATTR_H

#include <libexplain/string_buffer.h>
struct termios; /* forward */

/**
  * The explain_buffer_errno_tcgetattr function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_tcgetattr(explain_string_buffer_t *sb, int errnum, int
    fildes, struct termios *data);

/**
  * The explain_buffer_errno_tcgetattr_explanation function is used to
  * obtain the explanation (the part after "because") of an error returned
  * by the <i>tcgetattr</i>(3) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
  ...
  *     system call.
  */
void explain_buffer_errno_tcgetattr_explanation(explain_string_buffer_t *sb, int
    errnum, const char *syscall_name, int fildes, struct termios *data);

/* vim: set ts=8 sw=4 et : */
#endif /* LIBEXPLAIN_BUFFER_ERRNO_TCGETATTR_H */
