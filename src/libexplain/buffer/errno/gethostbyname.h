...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_GETHOSTBYNAME_H
#define LIBEXPLAIN_BUFFER_ERRNO_GETHOSTBYNAME_H
...
/**
  * The explain_buffer_errno_gethostbyname function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_gethostbyname(explain_string_buffer_t *sb, int errnum,
    const char *name);

/**
  * The explain_buffer_errno_gethostbyname_explanation function is used to
  * obtain the explanation (the part after "because") of an error returned
  * by the <i>gethostbyname</i>(3) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
...
#endif /* LIBEXPLAIN_BUFFER_ERRNO_GETHOSTBYNAME_H */
