...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_ICONV_CLOSE_H
#define LIBEXPLAIN_BUFFER_ERRNO_ICONV_CLOSE_H
...
/**
  * The explain_buffer_errno_iconv_close function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_iconv_close(explain_string_buffer_t *sb, int errnum,
    iconv_t cd);

/**
  * The explain_buffer_errno_iconv_close_explanation function is used to
  * obtain the explanation (the part after "because") of an error returned
  * by the <i>iconv_close</i>(3) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
...
#endif /* LIBEXPLAIN_BUFFER_ERRNO_ICONV_CLOSE_H */
