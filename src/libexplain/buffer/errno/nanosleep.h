...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_NANOSLEEP_H
#define LIBEXPLAIN_BUFFER_ERRNO_NANOSLEEP_H
...
/**
  * The explain_buffer_errno_nanosleep function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_nanosleep(explain_string_buffer_t *sb, int errnum,
    const struct timespec *req, struct timespec *rem);

/**
  * The explain_buffer_errno_nanosleep_explanation function is used to
  * obtain the explanation (the part after "because") of an error returned
  * by the <i>nanosleep</i>(2) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
  ...
  *     system call.
  */
void explain_buffer_errno_nanosleep_explanation(explain_string_buffer_t *sb, int
    errnum, const char *syscall_name, const struct timespec *req, struct
    timespec *rem);

/* vim: set ts=8 sw=4 et : */
#endif /* LIBEXPLAIN_BUFFER_ERRNO_NANOSLEEP_H */
