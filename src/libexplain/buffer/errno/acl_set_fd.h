...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_ACL_SET_FD_H
#define LIBEXPLAIN_BUFFER_ERRNO_ACL_SET_FD_H
...
/**
  * The explain_buffer_errno_acl_set_fd function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_acl_set_fd(explain_string_buffer_t *sb, int errnum,
    int fildes, acl_t acl);

/**
  * The explain_buffer_errno_acl_set_fd_explanation function is used to
  * obtain the explanation (the part after "because") of an error returned
  * by the <i>acl_set_fd</i>(3) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
  ...
  *     system call.
  */
void explain_buffer_errno_acl_set_fd_explanation(explain_string_buffer_t *sb,
    int errnum, const char *syscall_name, int fildes, acl_t acl);

/* vim: set ts=8 sw=4 et : */
#endif /* LIBEXPLAIN_BUFFER_ERRNO_ACL_SET_FD_H */
