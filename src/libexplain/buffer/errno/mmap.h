...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_MMAP_H
#define LIBEXPLAIN_BUFFER_ERRNO_MMAP_H
...
/**
  * The explain_buffer_errno_mmap function is used to obtain an explanation
  * of an error returned by the <i>mmap</i>(2) system call. The least the
  ...
  *     call.
  */
void explain_buffer_errno_mmap(explain_string_buffer_t *sb, int errnum, void
    *data, size_t data_size, int prot, int flags, int fildes, off_t offset);

/**
  * The explain_buffer_errno_mmap_explanation function is used to obtain
  * the explanation (the part after "because") of an error returned by the
  * <i>mmap</i>(2) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
  ...
  *     call.
  */
void explain_buffer_errno_mmap_explanation(explain_string_buffer_t *sb, int
    errnum, const char *syscall_name, void *data, size_t data_size, int prot,
    int flags, int fildes, off_t offset);

/* vim: set ts=8 sw=4 et : */
#endif /* LIBEXPLAIN_BUFFER_ERRNO_MMAP_H */
