...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_WRITEV_H
#define LIBEXPLAIN_BUFFER_ERRNO_WRITEV_H

#include <libexplain/string_buffer.h>
struct iovec; /* forward */

/**
  * The explain_buffer_errno_writev function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_writev(explain_string_buffer_t *sb, int errnum, int
    fildes, const struct iovec *data, int data_size);

/**
  * The explain_buffer_errno_writev_explanation function is used to obtain
  * the explanation (the part after "because") of an error returned by the
  * <i>writev</i>(2) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
  ...
  *     system call.
  */
void explain_buffer_errno_writev_explanation(explain_string_buffer_t *sb, int
    errnum, const char *syscall_name, int fildes, const struct iovec *data, int
    data_size);

/* vim: set ts=8 sw=4 et : */
#endif /* LIBEXPLAIN_BUFFER_ERRNO_WRITEV_H */
