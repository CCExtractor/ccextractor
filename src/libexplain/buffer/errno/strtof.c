...
#include <libexplain/buffer/errno/strtof.h>
...
static void
explain_buffer_errno_strtof_system_call(explain_string_buffer_t *sb, int errnum,
    const char *nptr, char **endptr)
{
    (void)errnum;
    explain_string_buffer_puts(sb, "strtof(nptr = ");
...
static void
explain_buffer_errno_strtof_explanation(explain_string_buffer_t *sb, int errnum,
    const char *nptr, char **endptr)
{
    /*
     * http://www.opengroup.org/onlinepubs/009695399/functions/strtof.html
     */
    switch (errnum)
    {
    case EINVAL:
...
void
explain_buffer_errno_strtof(explain_string_buffer_t *sb, int errnum, const char
    *nptr, char **endptr)
{
    explain_explanation_t exp;

    explain_explanation_init(&exp, errnum);
    explain_buffer_errno_strtof_system_call(&exp.system_call_sb, errnum, nptr,
        endptr);
    explain_buffer_errno_strtof_explanation(&exp.explanation_sb, errnum, nptr,
        endptr);
