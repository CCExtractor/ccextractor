...
static void
explain_buffer_errno_snprintf_system_call(explain_string_buffer_t *sb,
    int errnum, char *data, size_t data_size, const char *format, va_list ap)
{
    (void)errnum;
    explain_string_buffer_puts(sb, "snprintf(data = ");
...
void
explain_buffer_errno_snprintf_explanation(explain_string_buffer_t *sb,
    ...
    const char *format, va_list ap)
{
    ...
    if (errpos > 0)
    {
        explain_buffer_einval_format_string(sb, "format", format, errpos);
...
void
explain_buffer_errno_snprintf(explain_string_buffer_t *sb, int errnum,
    char *data, size_t data_size, const char *format, va_list ap)
{
    explain_explanation_t exp;

    explain_explanation_init(&exp, errnum);
    explain_buffer_errno_snprintf_system_call(&exp.system_call_sb, errnum, data,
        data_size, format, ap);
    explain_buffer_errno_snprintf_explanation(&exp.explanation_sb, errnum,
        "snprintf", data, data_size, format, ap);
