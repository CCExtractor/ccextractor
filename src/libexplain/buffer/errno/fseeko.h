...
/**
  * The explain_buffer_errno_fseeko function is used to obtain an
  ...
  *     system call.
  */
void explain_buffer_errno_fseeko(explain_string_buffer_t *sb, int errnum,
    FILE *fp, off_t offset, int whence);

/**
  * The explain_buffer_errno_fseeko_explanation function is used to obtain
  * the explanation (the part after "because") of an error returned by the
  * <i>fseeko</i>(3) system call (and similar).
  *
  * @param sb
  *     The string buffer to print the message into. If a suitable buffer
  *     is specified, this function is thread safe.
  ...
  *     system call.
  */
void explain_buffer_errno_fseeko_explanation(explain_string_buffer_t *sb,
    int errnum, const char *syscall_name, FILE *fp, off_t offset, int whence);

/* vim: set ts=8 sw=4 et : */
#endif /* LIBEXPLAIN_BUFFER_ERRNO_FSEEKO_H */
