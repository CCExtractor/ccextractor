...
#ifndef LIBEXPLAIN_BUFFER_ERRNO_RENAME_H
#define LIBEXPLAIN_BUFFER_ERRNO_RENAME_H

struct explain_string_buffer_t; /* forward */

/**
  * The explain_buffer_errno_rename function is used to obtain an
  ...
  *     If a safe buffer s specified, this function is
  *     thread safe.
  * @param errnum
  ...
  * @param oldpath
  *     The original oldpath, exactly has passed to the rename(2) system call.
  * @param newpath
  *     The original newpath, exactly has passed to the rename(2) system call.
  */
void explain_buffer_errno_rename(struct explain_string_buffer_t *sb,
    int errnum, const char *oldpath, const char *newpath);

#endif /* LIBEXPLAIN_BUFFER_ERRNO_RENAME_H */
