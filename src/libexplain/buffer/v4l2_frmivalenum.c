...
#include <libexplain/buffer/v4l2_frmival_stepwise.h>
...
void
explain_buffer_v4l2_frmivalenum(explain_string_buffer_t *sb,
    const struct v4l2_frmivalenum *data, int extra)
{
    ...
    if (extra)
    {
        ...
        explain_buffer_v4l2_frmivaltypes(sb, data->type);
        switch (data->type)
        {
        case V4L2_FRMIVAL_TYPE_DISCRETE:
            explain_string_buffer_puts(sb, ", discrete = ");
            explain_buffer_v4l2_fract(sb, &data->discrete);
            break;

        case V4L2_FRMIVAL_TYPE_STEPWISE:
            explain_string_buffer_puts(sb, ", stepwise = ");
            explain_buffer_v4l2_frmival_stepwise(sb, &data->stepwise);
            break;
...
int
explain_v4l2_frmivalenum_get_n(int fildes, int pixfmt)
{
    int lo = 0;
    int hi = 200;
    for (;;)
    {
        ...
        if (ioctl(fildes, VIDIOC_ENUM_FRAMEINTERVALS, &qry) >= 0)
        {
            if (hi <= 0 && lo <= 0)
                return -1;
            /* mid < ninputs */
