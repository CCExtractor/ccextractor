...
void
explain_buffer_v4l2_outputparm(explain_string_buffer_t *sb,
    const struct v4l2_outputparm *data)
{
    ...
    explain_string_buffer_puts(sb, ", outputmode = ");
    explain_buffer_v4l2_captureparm_flags(sb, data->outputmode);
    if (data->outputmode & V4L2_CAP_TIMEPERFRAME)
    {
    ...
    if (data->writebuffers)
    {
        explain_string_buffer_puts(sb, ", writebuffers = ");
        explain_buffer_uint32_t(sb, data->writebuffers);
    }
