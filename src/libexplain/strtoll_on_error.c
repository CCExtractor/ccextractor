...
long long
explain_strtoll_on_error(const char *nptr, char **endptr, int base)
{
    int             hold_errno;
    char            *dummy;
    long long       result;
    ...
    dummy = 0;
    errno = 0;
    result = strtoll(nptr, endptr ? endptr : &dummy, base);
    ...
    if (errno != 0)
    {
        ...
        (
            explain_errno_strtoll(hold_errno, nptr, endptr, base)
        );
    }
    errno = hold_errno;
    return result;
