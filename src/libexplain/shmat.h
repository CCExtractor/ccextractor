...
#ifndef LIBEXPLAIN_SHMAT_H
#define LIBEXPLAIN_SHMAT_H

/**
  * @file
  * @brief explain shmat(2) errors
  *
  * These functions may be used to obtain explanations for errors returned
  * by the <i>shmat</i>(2) system call.
  */
...
/**
  * The explain_shmat_or_die function is used to call the <i>shmat</i>(2)
  * system call. On failure an explanation will be printed to stderr,
  * obtained from the <i>#explain_shmat</i>(3) function, and then the
  * process terminates by calling <tt>exit(EXIT_FAILURE)</tt>.
  *
  * @param shmid
  *     The shmid, exactly as to be passed to the <i>shmat</i>(2) system
  *     call.
  * @param shmaddr
  *     The shmaddr, exactly as to be passed to the <i>shmat</i>(2) system
  *     call.
  * @param shmflg
  *     The shmflg, exactly as to be passed to the <i>shmat</i>(2) system
  *     call.
  * @returns
  *     This function only returns on success, see <i>shmat</i>(2) for more
...
/**
  * The explain_shmat_on_error function is used to call the <i>shmat</i>(2)
  * system call. On failure an explanation will be printed to stderr,
  * obtained from the <i>#explain_shmat</i>(3) function.
  *
  * @param shmid
  *     The shmid, exactly as to be passed to the <i>shmat</i>(2) system
  *     call.
  ...
  *     call.
  * @returns
  *     The value returned by the wrapped <i>shmat</i>(2) system call.
  *
  * @par Example:
  * This function is intended to be used in a fashion similar to the
  * following example:
  * @code
  * void *result = explain_shmat_on_error(shmid, shmaddr, shmflg);
  * if (!result)
  * {
  *     ...cope with error
...
/**
  ...
  * returned by the <i>shmat</i>(2) system call. The least the message will
...
/**
  ...
  * error returned by the <i>shmat</i>(2) system call. The least the
  ...
  * #explain_shmat_or_die function.
  */
const char *explain_errno_shmat(int errnum, int shmid, const void *shmaddr, int
    shmflg)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The explain_message_shmat function is used to obtain an explanation of
  * an error returned by the <i>shmat</i>(2) system call. The least the
  * message will contain is the value of <tt>strerror(errnum)</tt>, but
  * usually it will do much better, and indicate the underlying cause in
  * more detail.
  *
...
/**
  * The explain_message_errno_shmat function is used to obtain an
  ...
  *     int err = errno;
  *     char message[3000];
  *     explain_message_errno_shmat(message, sizeof(message), err, shmid,
  *         shmaddr, shmflg);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
