    ...
    int errnum, int fildes, int request, const void *data)
{
    ...
    (void)fildes;
    (void)request;
    explain_buffer_v4l2_dv_timings(sb, data);
}
    ...
    case EINVAL:
...

        {
            ...
            if (!explain_is_efault_pointer(arg, sizeof(*arg)))
            {
                ...
                if (ioctl(fildes, VIDIOC_G_DV_TIMINGS, &qry) >= 0)
                {
...
const explain_iocontrol_t explain_iocontrol_vidioc_s_dv_timings =
{
    "VIDIOC_S_DV_TIMINGS", /* name */
    VIDIOC_S_DV_TIMINGS, /* value */
    0, /* disambiguate */
    0, /* print_name */
    print_data,
    print_explanation,
