...
int api_start(struct ccx_s_options api_options)
{
	...
	if (!ctx)
	{
		if (errno == ENOMEM)
			fatal(EXIT_NOT_ENOUGH_MEMORY,
				  "Not enough memory, could not initialize "
				  "libraries\n");
			...
			fatal(CCX_COMMON_EXIT_FILE_CREATION_FAILED,
				  "Unable to create output file: Operation not "
				  "permitted.\n");
		else if (errno == EACCES)
			fatal(CCX_COMMON_EXIT_FILE_CREATION_FAILED,
				  "Unable to create output file: Permission "
				  "denied\n");
		else
	...
	if (api_options.transcript_settings.xds)
	{
		if (api_options.write_format != CCX_OF_TRANSCRIPT)
		{
			...
			mprint("Warning: -xds ignored, XDS can only be "
				   "exported to transcripts "
				   "at this time.\n");
		}
	}
	...
	if (api_options.binary_concat)
	{
		...
		if (ctx->total_inputsize < 0)
		{
			...
			{
			case -1 * ENOENT:
				fatal(EXIT_NO_INPUT_FILES, "Failed to open one of the input "
										   "file(s): File does not exist.");
			case -1 * EACCES:
				fatal(EXIT_READ_ERROR, "Failed to open input file: Unable to "
									   "access");
			case -1 * EINVAL:
				fatal(EXIT_READ_ERROR, "Failed to open input file: Invalid "
									   "opening flag.");
			case -1 * EMFILE:
				fatal(EXIT_TOO_MANY_INPUT_FILES,
					  "Failed to open input file: Too many "
					  "files are open.");
			default:
				fatal(EXIT_READ_ERROR, "Failed to open input "
									   "file: Reason unknown");
			}
		}
	}
	...
	{
		mprint("[share] WARNING: simultaneous translation of several "
			   "input files "
	...
	if (api_options.translate_enabled)
	{
		...
		ccx_share_launch_translator(api_options.translate_langs,
									api_options.translate_key);
	}
	...
	while (switch_to_next_file(ctx, 0))
	{
		...
		// Disable sync check for raw formats - they have the right
		// timeline. Also true for bin formats, but -nosync might have
		// created a broken timeline for debug purposes. Disable too in
		// MP4, specs doesn't say that there can't be a jump
		switch (stream_mode)
		{
		...
		*/
		...
		case CCX_SM_ELEMENTARY_OR_NOT_FOUND:
			if (!api_options.use_gop_as_pts)	// If !0 then the user
												// selected something
		...
		case CCX_SM_MCPOODLESRAW:
			...
			break;
		case CCX_SM_RCWT:
			mprint("\rAnalyzing data in CCExtractor's binary "
				   "format\n");
			tmp = rcwt_loop(ctx);
			if (!ret)
				ret = tmp;
		...
		case CCX_SM_MP4:
			...
			close_input_file(ctx); // No need to have it open. GPAC
								   // will do it for us
			if (ctx->current_file ==
				-1) // We don't have a file to open, must be
					// stdin, and GPAC is incompatible with stdin
			{
				fatal(EXIT_INCOMPATIBLE_PARAMETERS,
					  "MP4 requires an actual file, it's not "
					  "possible to read from a "
					  "stream, including stdin.\n");
			}
			if (api_options.extract_chapters)
			{
				tmp = dumpchapters(ctx, &ctx->mp4_cfg,
								   ctx->inputfile[ctx->current_file]);
			}
			else
			{
				tmp = processmp4(ctx, &ctx->mp4_cfg,
								 ctx->inputfile[ctx->current_file]);
			}
							...
							struct lib_cc_decode)
		{
			...
			// Add one frame as fts_max marks the beginning of the
			// last frame, but we need the end.
			dec_ctx->timing->fts_global +=
				dec_ctx->timing->fts_max + (LLONG)(1000.0 / current_fps);
			// CFS: At least in Hauppage mode, cb_field can be
			// responsible for ALL the timing (cb_fields having a
			// huge number and fts_now and fts_global being 0 all
			// the time), so we need to take that into account in
			// fts_global before resetting counters.
			if (cb_field1 != 0)
			...
			// Reset counters - This is needed if some captions are
			// still buffered and need to be written after the last
			// file is processed.
			cb_field1 = 0;
			cb_field2 = 0;
			cb_708 = 0;
			...
			if (dec_ctx->total_pulldownframes)
				mprint("incl. pulldown frames:  %s  (%u frames "
					   "at %.2ffps)\n",
					   print_mstime_static((LLONG)(
						   dec_ctx->total_pulldownframes * 1000 / current_fps)),
					   dec_ctx->total_pulldownframes, current_fps);
			if (dec_ctx->timing->pts_set >= 1 &&
				dec_ctx->timing->min_pts != 0x01FFFFFFFFLL)
			{
				LLONG postsyncms = (LLONG)(dec_ctx->frames_since_last_gop *
										   1000 / current_fps);
				mprint("\nMin PTS:				%s\n",
					   print_mstime_static(dec_ctx->timing->min_pts /
											   (MPEG_CLOCK_FREQ / 1000) -
										   dec_ctx->timing->fts_offset));
				if (pts_big_change)
					mprint("(Reference clock was reset at "
						   "some point, Min PTS is "
						   "approximated)\n");
				mprint("Max PTS:				%s\n",
				...
				mprint(
					"Length:				 %s\n",
					print_mstime_static(
						dec_ctx->timing->sync_pts / (MPEG_CLOCK_FREQ / 1000) +
						postsyncms -
			...
			if (dec_ctx->false_pict_header)
				mprint("Number of likely false picture headers "
					   "(discarded): %d\n",
			...
			if (dec_ctx->stat_divicom)
			{
				...
				mprint("\n\nNOTE! The CEA 608 / Divicom "
					   "standard encoding for closed\n");
				mprint("caption is not well understood!\n\n");
				mprint("Please submit samples to the "
					   "developers.\n\n\n");
			}
		}
	...
	if (is_decoder_processed_enough(ctx) == CCX_TRUE)
	{
		mprint("\rNote: Processing was canceled before all data was "
			   "processed "
			   "because\n");
	...
	if (show_myth_banner)
	{
		mprint("NOTICE: Due to the major rework in 0.49, we needed to "
			   "change part "
			   "of the timing\n");
		mprint("code in the MythTV's branch. Please report results to "
			   "the address "
			   "above. If\n");
...
{
	// adding the parameter ./ccextractor to the list of python_params for
	// further parsing
	api_options->python_params = realloc(
		api_options->python_params, (api_options->python_param_count + 1) *
										sizeof *api_options->python_params);
	api_options->python_params[api_options->python_param_count] =
		malloc(strlen("./ccextractor") + 1);
	strcpy(api_options->python_params[api_options->python_param_count],
		   "./ccextractor");
	...
	int ret = parse_parameters(api_options, api_options->python_param_count,
							   api_options->python_params);
...
int main(int argc, char *argv[])
{
	...
	if (compile_ret == EXIT_NO_INPUT_FILES)
	{
		...
		fatal(EXIT_NO_INPUT_FILES,
			  "(This help screen was shown because there were no input "
			  "files)\n");
	}
