...
struct encoder_cfg {
  ...
  ccx_encoders_transcript_format transcript_settings;  // Keeps the settings for
...
struct ccx_s_options  // Options from user parameters
  ...
  /* MP4 related stuff */
  unsigned mp4vidtrack;  // Process the video track even if a CC dedicated track
                         // exists.
  /* General settings */
  ...
  char *
      ocrlang;  // The name of the .traineddata file to be loaded with tesseract
  ...
  LLONG debug_mask;           // dbg_print will use this mask to print or ignore
                              // different types
  LLONG debug_mask_on_debug;  // If we're using temp_debug to enable/disable
                              // debug "live", this is the mask when
                              // temp_debug=1
  /* Networking */
  char *udpaddr;
  unsigned
