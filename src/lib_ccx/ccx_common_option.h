...
struct demuxer_cfg
{
	...
	unsigned ts_autoprogram; // Try to find a stream with captions automatically
							 // (no -pn needed)
	...
	int ts_forced_program;	   // Specific program to process in TS files, if
							   // ts_forced_program_selected==1
	unsigned ts_forced_program_selected;
	int ts_datastreamtype; // User WANTED stream type (i.e. use the stream that
						   // has this type)
...
struct encoder_cfg
{
	...
	int with_semaphore; // Write a .sem file on file open and delete it on
						// close?
	/* Credit stuff */
	...
	int no_bom; // Set to 1 when no BOM (Byte Order Mark) should be used for
				// files. Note, this might make files unreadable in windows!
	char *first_input_file;
	int multiple_files;
	int no_font_color;
	int no_type_setting;
	int cc_to_stdout; // If this is set to 1, the stdout will be flushed when
					  // data was written to the screen during a process_608
					  // call.
	int line_terminator_lf; // 0 = CRLF, 1=LF
	LLONG subs_delay;		// ms to delay (or advance) subs
	int program_number;
	unsigned char in_format;
	int nospupngocr; // 1 if we don't want to OCR bitmaps to add the text as
					 // comments in the XML file in spupng

	// MCC File
	int force_dropframe; // 1 if dropframe frame count should be used. defaults
						 // to no drop frame.

	// text -> png (text render)
	char *render_font; // The font used to render text if needed (e.g.
					   // teletext->spupng)
...
{
	int extract;   // Extract 1st, 2nd or both fields
	int no_rollup; // Disable roll-up emulation (no duplicate output in
				   // generated file)
	int noscte20;
	...
	int use_gop_as_pts; // Use GOP instead of PTS timing (0=do as needed,
						// 1=always, -1=never)
	...
	int live_stream;	 /* -1 -> Not a complete file but a live stream, without
							timeout	 0 -> A regular file	 >0 -> Live stream with a
							timeout of this value in seconds */
	...
	int levdistmincnt, levdistmaxpct; // Means 2 fails or less is "the same",
									  // 10% or less is also "the same"
	int investigate_packets; // Look for captions in all packets when everything
							 // else fails
	...
	/* MP4 related stuff */
	unsigned mp4vidtrack; // Process the video track even if a CC dedicated
						  // track exists.
	int extract_chapters; // If 1, extracts chapters (if present), from MP4
						  // files.
	/* General settings */
	int usepicorder; // Force the use of pic_order_cnt_lsb in AVC/H.264 data
					 // streams
	int xmltv;		 // 1 = full output. 2 = live output. 3 = both
	int xmltvliveinterval;	 // interval in seconds between writing xmltv output
							 // files in live mode
	int xmltvoutputinterval; // interval in seconds between writing xmltv full
							 // file output
	...
	char *ocrlang;	   // The name of the .traineddata file to be loaded with
					   // tesseract
	int ocr_oem;	   // The Tesseract OEM mode, could be 0 (default), 1 or 2
	int ocr_quantmode; // How to quantize the bitmap before passing to to
					   // tesseract (0=no quantization at all, 1=CCExtractor's
					   // internal)
	char *mkvlang;	   // The name of the language stream for MKV
	int analyze_video_stream; // If 1, the video stream will be processed even
							  // if we're using a different one for subtitles.
	...
	LLONG debug_mask; // dbg_print will use this mask to print or ignore
					  // different types
	LLONG
		debug_mask_on_debug; // If we're using temp_debug to enable/disable
							 // debug "live", this is the mask when temp_debug=1
	/* Networking */
	char *udpsrc;
	char *udpaddr;
	unsigned
		udpport; // Non-zero => Listen for UDP packets on this port, no files.
	char *tcpport;
	char *tcp_password;
	char *tcp_desc;
	char *srv_addr;
	...
	int cc_to_stdout; // If this is set to 1, the stdout will be flushed when
					  // data was written to the screen during a process_608
					  // call.
	int pes_header_to_stdout; // If this is set to 1, the PES Header will be
							  // printed to console (debugging purposes)
	int ignore_pts_jumps; // If 1, the program will ignore PTS jumps. Sometimes
						  // this parameter is required for DVB subs with > 30s
						  // pause time
	int multiprogram;
	int out_interval;
...
#ifdef PYTHON_API
	char **python_params; // An array of strings to store the params supplied in
						  // the python processing
	int python_param_count; // Count of the total number of params passed via
							// python bindings
#endif
};
