...
static const char *webvtt_pac_row_percent[] = {
	"10",	 "15.33", "20.66", "26",	"31.33", "36.66", "42",	  "47.33",
	"52.66", "58",	  "63.33", "68.66", "74",	 "79.33", "84.66"};

/* The timing here is not PTS based, but output based, i.e. user delay must be
accounted for if there is any */
							...
							LLONG ms_start, LLONG ms_end)
{
	...
	millis_to_time(ms_end - 1, &h2, &m2, &s2,
				   &ms2); // -1 To prevent overlapping with next line.

	sprintf(timeline, "%02u:%02u:%02u.%03u --> %02u:%02u:%02u.%03u%s", h1, m1,
			s1, ms1, h2, m2, s2, ms2, context->encoded_crlf);
	...
	if (el == NULL || unescaped == NULL)
		fatal(EXIT_NOT_ENOUGH_MEMORY,
			  "In write_stringz_as_webvtt() - not enough memory.\n");
	int pos_r = 0;
	int pos_w = 0;
	...
	while (begin < unescaped + len)
	{
		...
		written = write(context->out->fh, context->encoded_crlf,
...
int write_webvtt_header(struct encoder_ctx *context)
{
	...
	if (context->timing->sync_pts2fts_set)
	{
		...
		sprintf(header_string,
				"X-TIMESTAMP-MAP=MPEGTS:%ld,LOCAL:%02u:%02u:%02u.%03u%s",
				context->timing->sync_pts2fts_pts, h1, m1, s1, ms1,
				ccx_options.enc_cfg.line_terminator_lf ? "\n\n" : "\r\n\r\n");
		used = encode_line(context, context->buffer,
						   (unsigned char *)header_string);
	...
	if (ccx_options.webvtt_create_css)
	{
		...
		char *css_file_name = (char *)malloc(
			(strlen(basefilename) + 4) * sizeof(char)); // strlen(".css") == 4
		...
		char *outline_css_file = (char *)malloc(
			(strlen(css_file_name) + strlen(webvtt_outline_css)) *
			sizeof(char));
	...
	else if (ccx_options.use_webvtt_styling)
	{
		...
		if (ccx_options.enc_cfg.line_terminator_lf ==
			1) // If -lf parameter is set.
		{
	...
	context->wrote_webvtt_header =
		1; // Do it even if couldn't write the header, because it won't be
		   // possible anyway
}
							  ...
							  struct encoder_ctx *context)
{
	...
	if (str)
	{
		if (context->prev_start != -1 || !(sub->flags & SUB_EOD_MARKER))
		{
			...
			millis_to_time(ms_end - 1, &h2, &m2, &s2,
						   &ms2);	// -1 To prevent overlapping with next line.
			context->srt_counter++; // Not needed for WebVTT but let's keep it
									// around for now
								...
								struct encoder_ctx *context)
{
	...
	while (sub)
	{
		if (sub->type == CC_TEXT)
		{
			ret = write_stringz_as_webvtt(sub->data, context, sub->start_time,
										  sub->end_time);
			freep(&sub->data);
			sub->nb_data = 0;
		}
...
void get_color_events(int *color_events, int line_num,
					  struct eia608_screen *data)
{
	...
	for (int i = first; i <= last; i++)
	{
		...
		{
			// It does not make sense to keep the default white color in the
			// events WebVTT supports colors only is [COL_WHITE..COL_MAGENTA]
			if (data->colors[line_num][i] <= COL_MAGENTA)
				color_events[i] |=
					data->colors[line_num][i]; // Add this new color

			if (last_color != COL_WHITE && last_color <= COL_MAGENTA)
				color_events[i - 1] |= last_color
									   << 16; // Remove old color (event in the
											  // second part of the integer)
...
void get_font_events(int *font_events, int line_num, struct eia608_screen *data)
{
	...
	for (int i = first; i <= last; i++)
	{
		if (data->fonts[line_num][i] != last_font)
		{
			...
			if (data->fonts[line_num][i] !=
				FONT_REGULAR) // Really can do it without condition because
							  // FONT_REGULAR == 0
				font_events[i] |= data->fonts[line_num][i]; // Add this new font

			if (last_font != FONT_REGULAR)
				font_events[i] |= last_font
								  << 16; // Remove old font (event in the second
										 // part of the integer)
							  ...
							  struct encoder_ctx *context)
{
	...
	for (int i = 0; i < 15; i++)
	{
		if (data->row_used[i])
		{
			...
			sprintf(timeline,
					"%02u:%02u:%02u.%03u --> %02u:%02u:%02u.%03u line:%s%%%s",
					h1, m1, s1, ms1, h2, m2, s2, ms2, webvtt_pac_row_percent[i],
			...
			for (int j = 0; j < length; j++)
			{
				if (ccx_options.use_webvtt_styling)
				{
					...
					if (open_color != COL_WHITE)
					{
						...
						write(context->out->fh, color_text[open_color][0],
							  strlen(color_text[open_color][0]));
