...
LLONG ts_start_of_xds =
	-1; // Time at which we switched to XDS mode, =-1 hasn't happened yet

static const char *XDSclasses[] = {
	"Current",		  "Future",	  "Channel",	  "Miscellaneous",
	"Public service", "Reserved", "Private data", "End"};
...
#define XDS_CLASS_OUT_OF_BAND                                                  \
	0x40 // Not a real class, a marker for packets for out-of-band data.
...
#define XDS_TYPE_ASPECT_RATIO_INFO                                             \
	9 // Appears in CEA-608-B but in E it's been removed as is "reserved"
...
int write_xds_string(struct cc_subtitle *sub,
					 struct ccx_decoders_xds_context *ctx, char *p, size_t len)
{
	struct eia608_screen *data = NULL;
	data = (struct eia608_screen *)realloc(sub->data,
										   (sub->nb_data + 1) * sizeof(*data));
	if (!data)
	{
		freep(&sub->data);
		sub->nb_data = 0;
...
void xdsprint(struct cc_subtitle *sub, struct ccx_decoders_xds_context *ctx,
			  const char *fmt, ...)
{
...
void xds_debug_test(struct ccx_decoders_xds_context *ctx,
...
void xds_cea608_test(struct ccx_decoders_xds_context *ctx,
					 struct cc_subtitle *sub)
{
	/* This test is the sample data that comes in CEA-608. It sets the program
	   name to be "Star Trek". The checksum is 0x1d and the validation must
	   succeed. */
					   ...
					   const unsigned char hi, int lo)
{
	...
	if (hi >= 0x01 && hi <= 0x0f)
	{
		int xds_class =
			(hi - 1) /
			2; // Start codes 1 and 2 are "class type" 0, 3-4 are 2, and so on.
		is_new = hi % 2; // Start codes are even
		ccx_common_logging.debug_ftn(CCX_DMT_DECODER_XDS,
									 "XDS Start: %u.%u  Is new: %d  | Class: "
									 "%d (%s), Used buffers: %d\n",
									 hi, lo, is_new, xds_class,
									 XDSclasses[xds_class], how_many_used(ctx));
		...
		/* Here, 3 possibilities:
		   1) We already had a buffer for this class/type and matching_buf
		   points to it 2) We didn't have a buffer for this class/type and
		   first_free_buf points to an unused one 3) All buffers are full and we
		   will have to skip this packet.
		 */
		if (matching_buf == -1 && first_free_buf == -1)
		{
			ccx_common_logging.log_ftn(
				"Note: All XDS buffers full (bug or suicidal stream). Ignoring "
				"this one (%d,%d).\n",
				xds_class, lo);
			ctx->cur_xds_buffer_idx = -1;
			return;
		}
		ctx->cur_xds_buffer_idx =
			(matching_buf != -1) ? matching_buf : first_free_buf;

		if (is_new || !ctx->xds_buffers[ctx->cur_xds_buffer_idx].in_use)
		{
			// Whatever we had before we discard; must belong to an interrupted
			// packet
			...
			memset(ctx->xds_buffers[ctx->cur_xds_buffer_idx].bytes, 0,
	...
	else
	{
		...
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_XDS, "XDS: %02X.%02X (%c, %c)\n", hi, lo, hi, lo);
	...
	if (ctx->xds_buffers[ctx->cur_xds_buffer_idx].used_bytes <= 32)
	{
		...
		ctx->xds_buffers[ctx->cur_xds_buffer_idx]
			.bytes[ctx->xds_buffers[ctx->cur_xds_buffer_idx].used_bytes++] = hi;
		ctx->xds_buffers[ctx->cur_xds_buffer_idx]
			.bytes[ctx->xds_buffers[ctx->cur_xds_buffer_idx].used_bytes++] = lo;
		ctx->xds_buffers[ctx->cur_xds_buffer_idx]
			.bytes[ctx->xds_buffers[ctx->cur_xds_buffer_idx].used_bytes] = 0;
	}
}
/**
 * ctx XDS context can be NULL, if user don't want to write xds in transcript
 */
void xds_do_copy_generation_management_system(
	struct cc_subtitle *sub, struct ccx_decoders_xds_context *ctx, unsigned c1,
	unsigned c2)
{
	...
	if (last_c1 != c1 || last_c2 != c2)
	{
		...
		const char *apstext[4] = {"No APS", "PSP On; Split Burst Off",
...
void xds_do_content_advisory(struct cc_subtitle *sub,
							 struct ccx_decoders_xds_context *ctx, unsigned c1,
							 unsigned c2)
{
	static unsigned last_c1 = -1, last_c2 = -1;
	static char age[256];
	...
	if (last_c1 != c1 || last_c2 != c2)
	{
		...
		last_c2 = c2;
		// Changed since last time, decode
		// Bits a1 and a0 determine the encoding. I'll add parsing as more
		// samples become available
		if (!a1 && a0) // US TV parental guidelines
		{
			const char *agetext[8] = {"None",
									  "TV-Y (All Children)",
			...
			sprintf(
				age,
				"ContentAdvisory: US TV Parental Guidelines. Age Rating: %s",
				agetext[g2 * 4 + g1 * 2 + g0]);
			content[0] = 0;
			if (!g2 && g1 && !g0) // For TV-Y7 (Older children), the Violence
								  // bit is "fantasy violence"
			{
				if (FV)
		...
		if (!a0) // MPA
		{
			const char *ratingtext[8] = {"N/A", "G",	 "PG", "PG-13",
										 "R",	"NC-17", "X",  "Not Rated"};
			sprintf(rating, "ContentAdvisory: MPA Rating: %s",
					ratingtext[r2 * 4 + r1 * 2 + r0]);
			supported = 1;
		}
		if (a0 && a1 && !Da2 && !La3) // Canadian English Language Rating
		{
			const char *ratingtext[8] = {
				"Exempt",
				"Children",
				...
				"Adult Programming",
				"[undefined]"};
			sprintf(rating, "ContentAdvisory: Canadian English Rating: %s",
					ratingtext[g2 * 4 + g1 * 2 + g0]);
			supported = 1;
		}
		if (a0 && a1 && Da2 && !La3) // Canadian French Language Rating
		{
			const char *ratingtext[8] = {
				...
				"Cette ?mission peut ne pas convenir aux enfants de moins de "
				"13 ans",
				"Cette ?mission ne convient pas aux moins de 16 ans",
				"Cette ?mission est r?serv?e aux adultes",
				"[invalid]",
				"[invalid]"};
			sprintf(rating, "ContentAdvisory: Canadian French Rating: %s",
					ratingtext[g2 * 4 + g1 * 2 + g0]);
			supported = 1;
		}
	}
	// Bits a1 and a0 determine the encoding. I'll add parsing as more samples
	// become available
	if (!a1 && a0) // US TV parental guidelines
	{
	...
	if (changed && !supported)
		ccx_common_logging.log_ftn("XDS: Unsupported ContentAdvisory encoding, "
							  ...
							  struct ccx_decoders_xds_context *ctx)
{
	...
	case XDS_TYPE_PIN_START_TIME:
		...
		/* int changed=0; */
		if (ctx->current_xds_min != min || ctx->current_xds_hour != hour ||
			ctx->current_xds_date != date || ctx->current_xds_month != month)
		{
			/* changed=1; */
			ctx->xds_start_time_shown = 0;
		...
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_XDS, "PIN (Start Time): %s  %02d-%02d %02d:%02d\n",
			(ctx->cur_xds_packet_class == XDS_CLASS_CURRENT ? "Current"
															: "Future"),
			date, month, hour, min);
		...
		if (!ctx->xds_start_time_shown &&
			ctx->cur_xds_packet_class == XDS_CLASS_CURRENT)
		{
			ccx_common_logging.log_ftn("\rXDS: Program changed.\n");
			ccx_common_logging.log_ftn(
				"XDS program start time (DD/MM HH:MM) %02d-%02d %02d:%02d\n",
				date, month, hour, min);
			ccx_common_logging.gui_ftn(
				CCX_COMMON_LOGGING_GUI_XDS_PROGRAM_ID_NR, ctx->current_xds_min,
				ctx->current_xds_hour, ctx->current_xds_date,
				ctx->current_xds_month);
	...
	case XDS_TYPE_LENGH_AND_CURRENT_TIME:
	{
		...
		if (!ctx->xds_program_length_shown)
			ccx_common_logging.log_ftn(
				"\rXDS: Program length (HH:MM): %02d:%02d  ", hour, min);
		else
			ccx_common_logging.debug_ftn(
				CCX_DMT_DECODER_XDS,
				"\rXDS: Program length (HH:MM): %02d:%02d  ", hour, min);
		...
		if (ctx->cur_xds_payload_length >
			6) // Next two bytes (optional) available
		{
			int el_min = ctx->cur_xds_payload[4] & 0x3f;  // 6 bits
			int el_hour = ctx->cur_xds_payload[5] & 0x1f; // 5 bits
			if (!ctx->xds_program_length_shown)
				ccx_common_logging.log_ftn("Elapsed (HH:MM): %02d:%02d",
										   el_hour, el_min);
			else
				ccx_common_logging.debug_ftn(CCX_DMT_DECODER_XDS,
		...
		if (ctx->cur_xds_payload_length >
			8) // Next two bytes (optional) available
		{
			int el_sec = ctx->cur_xds_payload[6] & 0x3f; // 6 bits
			if (!ctx->xds_program_length_shown)
				ccx_common_logging.debug_ftn(CCX_DMT_DECODER_XDS, ":%02d",
											 el_sec);
	...
	case XDS_TYPE_PROGRAM_NAME:
	{
		...
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_XDS, "\rXDS Program name: %s\n", xds_program_name);
		xdsprint(sub, ctx, "Program name: %s", xds_program_name);
		if (ctx->cur_xds_packet_class == XDS_CLASS_CURRENT &&
			strcmp(xds_program_name,
				   ctx->current_xds_program_name)) // Change of program
		{
	...
	case XDS_TYPE_PROGRAM_TYPE:
		...
		// if (!(ccx_common_logging.debug_mask & CCX_DMT_DECODER_XDS) &&
		// ctx->current_program_type_reported) 	break;
		memcpy(ctx->current_xds_program_type, ctx->cur_xds_payload,
		...
		*str = '\0';
		...
		for (int i = 2; i < ctx->cur_xds_payload_length - 1; i++)
		{
			...
			if (ctx->cur_xds_payload[i] >= 0x20 &&
				ctx->cur_xds_payload[i] < 0x7F)
			{
				snprintf(tstr, str_len - (tstr - str), "[%s] ",
						 XDSProgramTypes[ctx->cur_xds_payload[i] - 0x20]);
				tstr += strlen(tstr);
			}
			if (!ctx->current_program_type_reported)
			{
				if (ctx->cur_xds_payload[i] >= 0x20 &&
					ctx->cur_xds_payload[i] < 0x7F)
					ccx_common_logging.log_ftn(
						"%s", XDSProgramTypes[ctx->cur_xds_payload[i] - 0x20]);
				else
	...
	case XDS_TYPE_CONTENT_ADVISORY:
		...
		if (ctx->cur_xds_payload_length < 5) // We need 2 data bytes
			break;
		xds_do_content_advisory(sub, ctx, ctx->cur_xds_payload[2],
	...
	case XDS_TYPE_CGMS:
		...
		xds_do_copy_generation_management_system(
			sub, ctx, ctx->cur_xds_payload[2], ctx->cur_xds_payload[3]);
		break;
	case XDS_TYPE_ASPECT_RATIO_INFO:
	{
		...
		if (ctx->cur_xds_payload_length < 5) // We need 2 data bytes
			break;
		if (!(ctx->cur_xds_payload[2] & 0x20) ||
			!(ctx->cur_xds_payload[3] & 0x20)) // Bit 6 must be 1
			break; // if bit 6 is not 1 - skip invalid data.

		/* CEA-608-B: The starting line is computed by adding 22 to the decimal
		   number represented by bits S0 to S5. The ending line is computing by
		   subtracting
		...
		if (ar_start != ctx->current_ar_start)
		{
			...
			ccx_common_logging.log_ftn(
				"\rXDS Notice: Aspect ratio info, start line=%u, end line=%u\n",
				ar_start, ar_end);
			ccx_common_logging.log_ftn("\rXDS Notice: Aspect ratio info, "
									   "active picture height=%u, ratio=%f\n",
	...
	case XDS_TYPE_PROGRAM_DESC_8:
	{
		...
		if (xds_desc[0])
		{
			...
			{
				ccx_common_logging.debug_ftn(CCX_DMT_DECODER_XDS,
											 "\rXDS description line %d: %s\n",
											 line_num, xds_desc);
			}
			xdsprint(sub, ctx, "XDS description line %d: %s", line_num,
					 xds_desc);
			ccx_common_logging.gui_ftn(
				CCX_COMMON_LOGGING_GUI_XDS_PROGRAM_DESCRIPTION, line_num,
				xds_desc);
		}
		break;
	}
				   ...
				   struct ccx_decoders_xds_context *ctx)
{
	...
	case XDS_TYPE_NETWORK_NAME:
		...
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_XDS, "XDS Network name: %s\n", xds_network_name);
		xdsprint(sub, ctx, "Network: %s", xds_network_name);
		if (strcmp(xds_network_name,
				   ctx->current_xds_network_name)) // Change of station
		{
	...
	case XDS_TYPE_CALL_LETTERS_AND_CHANNEL:
	{
		...
		if (ctx->cur_xds_payload_length != 7 &&
			ctx->cur_xds_payload_length != 9) // We need 4-6 data bytes
			break;
		for (i = 2; i < ctx->cur_xds_payload_length - 1; i++)
		{
		...
		ccx_common_logging.debug_ftn(CCX_DMT_DECODER_XDS,
									 "XDS Network call letters: %s\n",
									 xds_call_letters);
		xdsprint(sub, ctx, "Call Letters: %s", xds_call_letters);
		if (strncmp(xds_call_letters, ctx->current_xds_call_letters,
					7)) // Change of station
		{
			ccx_common_logging.log_ftn(
				"XDS Notice: Network call letters now %s\n", xds_call_letters);
			strncpy(ctx->current_xds_call_letters, xds_call_letters, 7);
			ccx_common_logging.gui_ftn(CCX_COMMON_LOGGING_GUI_XDS_CALL_LETTERS,
									   ctx->current_xds_call_letters);
		}
	}
	break;
	case XDS_TYPE_TSID:
		...
		if (ctx->cur_xds_payload_length < 7) // We need 4 data bytes
			break;
		unsigned b1 =
			(ctx->cur_xds_payload[2]) & 0x10; // Only low 4 bits from each byte
...
int xds_do_misc(struct ccx_decoders_xds_context *ctx)
{
	...
	case XDS_TYPE_TIME_OF_DAY:
	{
		...
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_XDS,
			"Time of day: (YYYY/MM/DD) %04d/%02d/%02d (HH:SS) %02d:%02d DoW: "
			"%d  Reset seconds: %d\n",
	...
	case XDS_TYPE_LOCAL_TIME_ZONE:
	{
		...
		if (ctx->cur_xds_payload_length < 5) // We need 2 data bytes
			break;
		// int b6 = (ctx->cur_xds_payload[2] & 0x40) >>6; // Bit 6 should always
		// be 1
		int dst = (ctx->cur_xds_payload[2] & 0x20) >> 5; // Daylight Saving Time
		int hour = ctx->cur_xds_payload[2] & 0x1f;		 // 5 bits
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_XDS, "Local Time Zone: %02d DST: %d\n", hour, dst);
		break;
	}
	default:
		was_proc = 0;
				   ...
				   unsigned char expected_checksum)
{
	...
	if (ctx->cur_xds_buffer_idx ==
			-1 || /* Unknown buffer, or not in use (bug) */
		!ctx->xds_buffers[ctx->cur_xds_buffer_idx].in_use)
		return;
	ctx->cur_xds_packet_class =
		ctx->xds_buffers[ctx->cur_xds_buffer_idx].xds_class;
	ctx->cur_xds_payload = ctx->xds_buffers[ctx->cur_xds_buffer_idx].bytes;
	ctx->cur_xds_payload_length =
		ctx->xds_buffers[ctx->cur_xds_buffer_idx].used_bytes;
	ctx->cur_xds_packet_type = ctx->cur_xds_payload[1];
	ctx->cur_xds_payload[ctx->cur_xds_payload_length++] =
		0x0F; // The end byte itself, added to the packet
	...
	cs =
		(128 - cs) & 0x7F; // Convert to 2's complement & discard high-order bit

	ccx_common_logging.debug_ftn(CCX_DMT_DECODER_XDS,
								 "End of XDS. Class=%d (%s), size=%d  Checksum "
								 "OK: %d   Used buffers: %d\n",
								 ctx->cur_xds_packet_class,
								 XDSclasses[ctx->cur_xds_packet_class],
								 ctx->cur_xds_payload_length,
	...
	{
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_XDS, "Expected checksum: %02X  Calculated: %02X\n",
	...
	int was_proc =
		0; /* Indicated if the packet was processed. Not processed means "code
			  to do it doesn't exist yet", not an error. */
	...
	case XDS_CLASS_FUTURE: // Info on future program
		if (!(ccx_common_logging.debug_mask &
			  CCX_DMT_DECODER_XDS)) // Don't bother processing something we
									// don't need
		{
			was_proc = 1;
			break;
	...
	case XDS_CLASS_PRIVATE: // CEA-608:
		// The Private Data Class is for use in any closed system for whatever
		// that system wishes. It shall not be defined by this standard now or
		// in the future.
	...
	{
		ccx_common_logging.log_ftn(
			"Note: We found a currently unsupported XDS packet.\n");
		dump(CCX_DMT_DECODER_XDS, ctx->cur_xds_payload,
			 ctx->cur_xds_payload_length, 0, 0);
	}
	clear_xds_buffer(ctx, ctx->cur_xds_buffer_idx);
}
