...
typedef struct ccx_encoders_transcript_format
{
  ...
  int isFinal; // Used to determine if these parameters should be changed
               // afterwards.
...
{
  int fh;
  int temporarily_closed; // 1 means the file was created OK before but we
                          // released the handle
  char* filename;
  void* spupng_data;
  int with_semaphore; // 1 means create a .sem file when the file is open and
                      // delete it when it's closed
  char* semaphore_filename;
  int with_playlist; // For m3u8 /webvtt: If 1, we'll generate a playlist and
                     // segments
