 ...
 */
#
ifndef _LINUX_LIST_H# define _LINUX_LIST_H# include "ccx_common_platform.h"
  /**
...
#
ifndef ccx_offsetof# define ccx_offsetof(TYPE, MEMBER)((size_t) & ((TYPE * ) 0) - > MEMBER)# endif
...
# define container_of(ptr, type, member)\
  ((type * )((char * ) ptr - ccx_offsetof(type, member)))
  /*@}*/
...
for (pos = list_entry((head) - > next, type, member), \
  n = list_entry(pos - > member.next, type, member);\ & pos - > member != (head);\ pos = n, n = list_entry(n - > member.next, type, member))
...
for (pos = list_entry((head) - > prev, typeof ( * pos), member), \
  n = list_entry(pos - > member.prev, typeof ( * pos), member);\ & pos - > member != (head);\ pos = n, n = list_entry(n - > member.prev, typeof ( * n), member))
...
define INIT_HLIST_HEAD(ptr)((ptr) - > first = NULL)# define INIT_HLIST_NODE(ptr)((ptr) - > next = NULL, (ptr) - > pprev = NULL)
...
static inline void __hlist_del(struct hlist_node * n) {
  ...
  struct hlist_node * * pprev = n - > pprev; * pprev = next;
