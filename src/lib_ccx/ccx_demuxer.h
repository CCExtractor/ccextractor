...
struct ccx_demuxer
{
	...
	/*51 possible stream ids in total, 0xbd is private stream, 0xbe is padding
	stream, 0xbf private stream 2, 0xc0 - 0xdf audio, 0xe0 - 0xef video
	...
	/* Hauppauge support */
	unsigned hauppauge_warning_shown; // Did we detect a possible Hauppauge
...
struct demuxer_data
{
	...
	size_t len;
	unsigned int rollover_bits; // The PTS rolls over every 26 hours and that
								// can happen in the middle of a stream.
	LLONG pts;
