...
struct ccx_demuxer
{
  ...
  LLONG past;                   /* Position in file, if in sync same as
                                   ftell() */
   ...
   struct ccx_demux_report freport;
    ...
    /* Hauppauge support */ 
  unsigned hauppauge_warning_shown;     // Did we detect a possible
                                        // Hauppauge
  ...
  unsigned int filebuffer_pos;  // Position of pointer relative to buffer
                                // start
  unsigned int bytesinbuffer;   // Number of bytes we actually have on
                                // buffer
   int warning_program_not_found_shown;
   
    // Remember if the last header was valid. Used to suppress too much
    // output
  ...
  void *private_data;
   ...
   int (*write_es) (struct ccx_demuxer * ctx, unsigned char *buf,
                     size_t len);
 };
struct demuxer_data
{
