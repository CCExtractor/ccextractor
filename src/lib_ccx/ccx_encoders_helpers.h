...
// Helper functions
void correct_case_with_dictionary	 (int line_num,
					  struct eia608_screen *data);

int is_all_caps    			 (struct encoder_ctx *context,
		      			  int line_num,
		   			  struct eia608_screen *data);

int clever_capitalize			 (struct encoder_ctx *context,
		      			  int line_num,
		   		 	  struct eia608_screen *data);
...
unsigned encode_line 			 (struct encoder_ctx *ctx,
		   			  unsigned char *buffer,
					  unsigned char *text);

void shell_sort(void *base,
		int nb,
		size_t size,
...
void ccx_encoders_helpers_setup	         (enum ccx_encoding_type encoding,
					  int no_font_color,
					  int no_type_setting,
					  int trim_subs);
#endif
