...
void _dtvcc_get_write_interval(dtvcc_tv_screen *tv, int row_index, int *first,
							   int *last)
{
...
void _dtvcc_color_to_hex(int color, unsigned *hR, unsigned *hG, unsigned *hB)
{
	...
	*hB = (unsigned)(color & 0x3);
	ccx_common_logging.debug_ftn(CCX_DMT_708,
								 "[CEA-708] Color: %d [%06x] %u %u %u\n", color,
								 color, *hR, *hG, *hB);
}

void _dtvcc_change_pen_colors(dtvcc_tv_screen *tv,
							  ccx_dtvcc_pen_color pen_color, int row_index,
							  int column_index, struct encoder_ctx *encoder,
							  size_t *buf_len, int open)
{
	...
	if (pen_color.fg_color != new_pen_color.fg_color)
	{
		if (pen_color.fg_color != 0x3f && !open)
			(*buf_len) +=
				sprintf(buf + (*buf_len),
						"</font>"); // should close older non-white color
		...
		if (new_pen_color.fg_color != 0x3f && open)
		{
			...
			blue = (255 / 3) * blue;
			(*buf_len) +=
				sprintf(buf + (*buf_len), "<font color=\"%02x%02x%02x\">", red,
						green, blue);
		}
	}
}

void _dtvcc_change_pen_attribs(dtvcc_tv_screen *tv,
							   ccx_dtvcc_pen_attribs pen_attribs, int row_index,
							   int column_index, struct encoder_ctx *encoder,
							   size_t *buf_len, int open)
{
	if (encoder->no_font_color)
...
void _dtvcc_write_row(ccx_dtvcc_writer_ctx *writer,
					  ccx_dtvcc_service_decoder *decoder, int row_index,
					  struct encoder_ctx *encoder, int use_colors)
{
	...
	if (decoder->current_window == -1)
		ccx_common_logging.log_ftn(
			"[CEA-708] _dtvcc_write_row: Window has to be defined first\n");

	int length;
	if (decoder->current_window ==
		-1) // Bug - in this case we have broken timing. See issue in GitHub
		length = last + 1;
	else
	...
	for (int i = 0; i < length; i++)
	{
		...
		if (use_colors)
			_dtvcc_change_pen_colors(tv, pen_color, row_index, i, encoder,
									 &buf_len, 0);
		_dtvcc_change_pen_attribs(tv, pen_attribs, row_index, i, encoder,
								  &buf_len, 0);
		_dtvcc_change_pen_attribs(tv, pen_attribs, row_index, i, encoder,
								  &buf_len, 1);
		if (use_colors)
		...
		{
			size_t size =
				write_utf16_char(tv->chars[row_index][i].sym, buf + buf_len);
			buf_len += size;
		}
	}
	...
	if (use_colors)
		_dtvcc_change_pen_colors(tv, pen_color, row_index,
								 CCX_DTVCC_SCREENGRID_COLUMNS, encoder,
								 &buf_len, 0);
	_dtvcc_change_pen_attribs(tv, pen_attribs, row_index,
							  CCX_DTVCC_SCREENGRID_COLUMNS, encoder, &buf_len,
							  0);
	// Tags can still be crossed e.g <f><i>text</f></i>, but testing HTML code
	// has shown that they still are handled correctly. In case of errors fix it
	// once and for all.
	...
	if (writer->cd != (iconv_t)-1)
	{
		...
		size_t result = iconv(writer->cd, &buf, &in_bytes_left, &encoded_buf,
							  &out_bytes_left);
...
void ccx_dtvcc_write_srt(ccx_dtvcc_writer_ctx *writer,
...
void ccx_dtvcc_write_transcript(ccx_dtvcc_writer_ctx *writer,
								ccx_dtvcc_service_decoder *decoder,
								struct encoder_ctx *encoder)
{
	dtvcc_tv_screen *tv = decoder->tv;
	...
	if (tv->time_ms_show + encoder->subs_delay <
	...
	for (int i = 0; i < CCX_DTVCC_SCREENGRID_ROWS; i++)
	{
		if (!_dtvcc_is_row_empty(tv, i))
		{
			...
			if (encoder->transcript_settings->showCC)
				sprintf(
					buf + strlen(buf),
					"CC1|"); // always CC1 because CEA-708 is field-independent

			if (encoder->transcript_settings->showMode)
				sprintf(buf + strlen(buf),
						"POP|"); // TODO caption mode(pop, rollup, etc.)

			if (strlen(buf))
				write(encoder->dtvcc_writers[tv->service_number - 1].fd, buf,
					  strlen(buf));
...
void _dtvcc_write_sami_header(dtvcc_tv_screen *tv, struct encoder_ctx *encoder)
{
	...
	buf_len += sprintf(buf + buf_len, "<style type=\"text/css\">%s",
					   encoder->encoded_crlf);
	buf_len += sprintf(buf + buf_len, "<!--%s", encoder->encoded_crlf);
	buf_len += sprintf(buf + buf_len,
					   "p {margin-left: 16pt; margin-right: 16pt; "
					   "margin-bottom: 16pt; margin-top: 16pt;%s",
					   encoder->encoded_crlf);
	buf_len += sprintf(buf + buf_len,
					   "text-align: center; font-size: 18pt; font-family: "
					   "arial; font-weight: bold; color: #f0f0f0;}%s",
					   encoder->encoded_crlf);
	buf_len += sprintf(buf + buf_len,
					   ".unknowncc {Name:Unknown; lang:en-US; SAMIType:CC;}%s",
					   encoder->encoded_crlf);
	buf_len += sprintf(buf + buf_len, "-->%s", encoder->encoded_crlf);
	buf_len += sprintf(buf + buf_len, "</style>%s", encoder->encoded_crlf);
	buf_len += sprintf(buf + buf_len, "</head>%s%s", encoder->encoded_crlf,
					   encoder->encoded_crlf);
...
void ccx_dtvcc_write_sami(ccx_dtvcc_writer_ctx *writer,
						  ccx_dtvcc_service_decoder *decoder,
						  struct encoder_ctx *encoder)
{
	dtvcc_tv_screen *tv = decoder->tv;
...
void ccx_dtvcc_writer_init(ccx_dtvcc_writer_ctx *writer, char *base_filename,
						   int program_number, int service_number,
						   enum ccx_output_format write_format,
						   struct encoder_cfg *cfg)
{
	ccx_common_logging.debug_ftn(CCX_DMT_708,
								 "[CEA-708] ccx_dtvcc_writer_init\n");
	writer->fd = -1;
	writer->cd = (iconv_t)-1;
	...
	char suffix[32];
	sprintf(suffix, CCX_DTVCC_FILENAME_TEMPLATE, program_number,
			service_number);
	...
	if (!writer->filename)
		ccx_common_logging.fatal_ftn(
			...
			"[CEA-708] _dtvcc_decoder_init_write: not enough memory");

	ccx_common_logging.debug_ftn(
		CCX_DMT_708, "[CEA-708] ccx_dtvcc_writer_init: inited [%s]\n",
		writer->filename);
...
void ccx_dtvcc_writer_output(ccx_dtvcc_writer_ctx *writer,
							 ccx_dtvcc_service_decoder *decoder,
							 struct encoder_ctx *encoder)
{
	ccx_common_logging.debug_ftn(CCX_DMT_708,
								 "[CEA-708] ccx_dtvcc_writer_output: "
								 "writing... [%s][%d]\n",
	...
	if (writer->filename && writer->fd < 0) // first request to write
	{
		ccx_common_logging.debug_ftn(CCX_DMT_708,
									 ...
									 writer->filename);
		writer->fd =
			open(writer->filename, O_RDWR | O_CREAT | O_TRUNC | O_BINARY,
				 S_IREAD | S_IWRITE);
		if (writer->fd == -1)
		{
			ccx_common_logging.fatal_ftn(CCX_COMMON_EXIT_FILE_CREATION_FAILED,
										 "[CEA-708] Failed to open a file\n");
		}
		if (!encoder->no_bom)
