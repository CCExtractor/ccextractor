...
int temp_debug =
	0; // This is a convenience variable used to enable/disable debug on
	   // variable conditions. Find references to understand.
...
void timestamp_to_srttime(uint64_t timestamp, char *buffer)
{
	...
	sprintf(buffer, "%02" PRIu8 ":%02" PRIu8 ":%02" PRIu8 ",%03" PRIu16, h, m,
			s, u);
}
					 ...
					 unsigned s2len)
{
	...
	unsigned int *column =
		(unsigned *)malloc((s1len + 1) * sizeof(unsigned int));
	for (y = 1; y <= s1len; y++)
		column[y] = y;
	for (x = 1; x <= s2len; x++)
	{
...
int levenshtein_dist_char(const char *s1, const char *s2, unsigned s1len,
						  unsigned s2len)
{
	unsigned int x, y, v, lastdiag, olddiag;
	unsigned int *column =
					...
					char millis_separator)
{
	...
	case ODF_SECONDS:
		...
		sprintf(buffer, "%lu%c%03u", (unsigned long)secs, millis_separator,
				(unsigned)millis);
		break;
	case ODF_DATE:
		secs = (time_t)(timestamp / 1000);
	...
	default:
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "Invalid value for date_format in millis_to_date()\n");
	}
}
...
/* General output, replacement for printf so we can control globally where
   messages go. mprint => Message print */
...
void dbg_print(LLONG mask, const char *fmt, ...)
{
	...
	if (!ccx_options.messages_target)
		return;
	t = temp_debug ? (ccx_options.debug_mask_on_debug | ccx_options.debug_mask)
				   : ccx_options.debug_mask; // Mask override?
...
{
	LLONG t = temp_debug
				  ? (ccx_options.debug_mask_on_debug | ccx_options.debug_mask)
				  : ccx_options.debug_mask; // Mask override?
	if (!(mask & t))
		return;
			...
			if (x + j < l && start[x + j] >= ' ')
				mprint("%c",
					   start[x + j] &
						   (clear_high_bit
								? 0x7F
								: 0xFF)); // 0x7F < remove high bit, convenient
										  // for visual CC inspection
			else
				mprint(" ");
		}
		mprint("\n");
...
struct encoder_ctx *change_filename(struct encoder_ctx *enc_ctx)
{
	...
	*temp_encoder = *enc_ctx;
	if (enc_ctx->out->fh != -1)
	{
		...
		char str_number[15];
		char *current_name =
			malloc(sizeof(char) * (strlen(enc_ctx->out->filename) + 10));
...
char *get_file_extension(enum ccx_output_format write_format)
{
	...
	default:
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "write_format doesn't have any legal value. Please report this "
			  "bug on Github.com\n");
		errno = EINVAL;
		return NULL;
	}
...
char *create_outfilename(const char *basename, const char *suffix,
...
LLONG change_timebase(LLONG val, struct ccx_rational cur_tb,
					  struct ccx_rational dest_tb)
{
