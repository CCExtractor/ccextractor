...
{
	int t = 0;
	// Try a few times before giving up. This is because this close/open stuff
	// exists for processes that demand exclusive access to the file, so often
	// we'll find out that we cannot reopen the file immediately.
	while (t < 5 && wb->fh == -1)
	{
		wb->fh = open(wb->filename, O_RDWR | O_CREAT | O_TRUNC | O_BINARY,
					  S_IREAD | S_IWRITE);
		sleep(1);
	}
	if (wb->fh == -1)
...
int init_write(struct ccx_s_write *wb, char *filename, int with_semaphore)
{
	...
	if (!(wb->append_mode))
		wb->fh = open(filename, O_RDWR | O_CREAT | O_TRUNC | O_BINARY,
					  S_IREAD | S_IWRITE);
	else
		wb->fh = open(filename, O_RDWR | O_CREAT | O_APPEND | O_BINARY,
					  S_IREAD | S_IWRITE);
	...
	if (with_semaphore)
	{
		...
		int t = open(wb->semaphore_filename,
				 ...
				 struct cc_subtitle *sub)
{
	...
	if (((loopcount == 1) && (datacount < 5)) ||
		((loopcount == 2) && (datacount < 8)) ||
		((loopcount == 3) && (datacount < 11)) ||
...
void printdata(struct lib_cc_decode *ctx, const unsigned char *data1,
			   int length1, const unsigned char *data2, int length2,
			   struct cc_subtitle *sub)
{
				   ...
				   struct cc_subtitle *sub)
{
	...
	if ((prevfts != currfts && prevfts != -1) || data == NULL ||
		cbcount == 0xFFFF)
	{
		...
		if (cbcount != 0xFFFF)
		{
			...
			dbg_print(CCX_DMT_CBRAW,
					  "%s Write %d RCWT blocks - skipped %d padding / %d "
					  "unused blocks.\n",
					  print_mstime_static(prevfts), cbcount,
					  storecbcount - cbcount, cbempty);
		}
		...
		// 8-9       blocks  Number of 3 byte data blocks with the same FTS that
		// are
		...
		if (cbcount > 0)
		{
			...
			ctx->writedata(cbbuffer, 3 * cbcount, ctx->context_cc608_field_1,
						   sub);
		}
		cbcount = 0;
		cbempty = 0;
