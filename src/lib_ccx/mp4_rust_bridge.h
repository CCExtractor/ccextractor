...
extern "C"
{
	...
	/*
	 * Process an AVC (H.264) sample through the existing NAL parser.
	 * Wraps process_avc_sample() with flat FFI-safe arguments.
	 *
	 ...
	 * @param cts_offset    Composition time offset (signed, cast from u32)
	...
	/*
	 * Process an HEVC (H.265) sample through the existing NAL parser.
	 * Wraps process_hevc_sample() with flat FFI-safe arguments.
	 *
	 * @param ctx           CCExtractor library context
	 * @param timescale     Media timescale for timestamp conversion
	 * @param nal_unit_size NAL unit length field size (1, 2, or 4 bytes)
