		...
		if (ctx->frame->side_data[i]->type == AV_FRAME_DATA_A53_CC)
		{
			...
			if (ctx->frame->side_data[i]->size > maxlen)
				av_log(NULL, AV_LOG_ERROR,
					   "Please consider increasing length of data\n");
			else
			{
				memcpy(data, ctx->frame->side_data[i]->data,
					   ctx->frame->side_data[i]->size);
...
int ffmpeg_get_more_data(struct lib_ccx_ctx *ctx, struct demuxer_data **ppdata)
{
	...
	{
		int len =
			ff_get_ccframe(ctx->demux_ctx->ffmpeg_ctx, data->buffer, BUFSIZE);
