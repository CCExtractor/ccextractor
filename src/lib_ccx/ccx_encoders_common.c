...
// These are the default settings for plain transcripts. No times, no CC or
// caption mode, and no XDS.
...
static const char *ssa_header = "[Script Info]\n\
Title: Default file\n\
ScriptType: v4.00+\n\
\n\
[V4+ Styles]\n\
...
TODO -- Set correct values for "ttp:dropMode", "ttp:frameRate" .Using common for
now.
...
Colombia, etc.), ATSC, PAL-M (Brazil)) 30 frame/sec (ATSC)
...
static const char *smptett_header =
	...
	"ttp:dropMode=\"dropNTSC\" ttp:frameRate=\"30\" "
	"ttp:frameRateMultiplier=\"1000 1001\" ttp:timeBase=\"smpte\" "
	"xmlns:m608=\"http://www.smpte-ra.org/schemas/2052-1/2010/"
	"smpte-tt#cea608\" "
	...
	"    </layout>\n"
	"    <metadata/>\n"
	"    <smpte:information m608:captionService=\"F1C1CC\" "
	"m608:channel=\"cc1\"/>\n"
	"  </head>\n"
	"  <body>\n"
	"    <div>\n";
...
static const char *simple_xml_header =
...
int change_utf8_encoding(unsigned char *dest, unsigned char *src, int len,
	...
	for (int i = 0; src < orig_src + len;)
	{
		...
		case CCX_ENC_LATIN_1:
			...
			else if (c_len == 2)
			{
				if ((src[1] & 0x40) == 0)
				{
					c = utf8_to_latin1_map(
						(((unsigned int)(src[0] & 0x1F)) << 6) |
			...
			{
				if ((src[1] & 0x40) == 0 && (src[2] & 0x40) == 0 &&
					(src[3] & 0x40) == 0 && (src[4] & 0x40) == 0)
				{
					c = utf8_to_latin1_map(
						(((unsigned int)(src[0] & 0x03)) << 24U) |
...
int change_latin1_encoding(unsigned char *dest, unsigned char *src, int len,
						   enum ccx_encoding_type out_enc)
{
	return CCX_ENOSUPP;
}

int change_unicode_encoding(unsigned char *dest, unsigned char *src, int len,
							enum ccx_encoding_type out_enc)
{
	return CCX_ENOSUPP;
}

int change_ascii_encoding(unsigned char *dest, unsigned char *src, int len,
						  enum ccx_encoding_type out_enc)
{
	unsigned char *orig = dest; // Keep for calculating length
	int bytes = 0;
				  ...
				  enum ccx_encoding_type out_enc, int max_len)
{
	...
	int len = 0;
	find_limit_characters(in_buffer, &first_non_blank, &last_non_blank,
						  max_len);
	if (!trim_subs)
		first_non_blank = 0;
	...
	case CCX_ENC_UTF_8:
		len =
			change_utf8_encoding(out_buffer, in_buffer + first_non_blank,
								 last_non_blank - first_non_blank + 1, out_enc);
		break;
	case CCX_ENC_LATIN_1:
		len = change_latin1_encoding(out_buffer, in_buffer + first_non_blank,
									 last_non_blank - first_non_blank + 1,
									 out_enc);
		break;
	case CCX_ENC_UNICODE:
		len = change_unicode_encoding(out_buffer, in_buffer + first_non_blank,
									  last_non_blank - first_non_blank + 1,
									  out_enc);
		break;
	case CCX_ENC_ASCII:
		len = change_ascii_encoding(out_buffer, in_buffer + first_non_blank,
									last_non_blank - first_non_blank + 1,
									out_enc);
		break;
	}
...
static int write_subtitle_file_header(struct encoder_ctx *ctx,
									  struct ccx_s_write *out)
{
	...
	case CCX_OF_WEBVTT:
		...
		for (int i = 0; webvtt_header[i] != NULL; i++)
		{
			...
			{
				used = encode_line(ctx, ctx->buffer,
								   (unsigned char *)webvtt_header[i]);
			}
			ret = write(out->fh, ctx->buffer, used);
			if (ret < used)
			{
	...
	case CCX_OF_SIMPLE_XML: // No header. Fall thru
		...
		used =
			encode_line(ctx, ctx->buffer, (unsigned char *)simple_xml_header);
		ret = write(out->fh, ctx->buffer, used);
		if (ret < used)
		{
			mprint("WARNING: Unable to write complete Buffer \n");
...
int write_cc_subtitle_as_simplexml(struct cc_subtitle *sub,
...
void write_cc_line_as_simplexml(struct eia608_screen *data,
								struct encoder_ctx *context, int line_number)
{
	...
	length = get_str_basic(context->subline, data->characters[line_number],
						   context->trim_subs, CCX_ENC_ASCII, context->encoding,
...
int write_cc_buffer_as_simplexml(struct eia608_screen *data,
								 struct encoder_ctx *context)
{
...
int write_cc_bitmap_as_simplexml(struct cc_subtitle *sub,
								 struct encoder_ctx *context)
{
	int ret = 0;
 ...
 */
static void try_to_add_end_credits(struct encoder_ctx *context,
								   struct ccx_s_write *out, LLONG current_fts)
{
	LLONG window, length, st, end;
	if (out->fh == -1)
		return;
	window = current_fts - context->last_displayed_subs_ms - 1;
	if (window < context->endcreditsforatleast
					 .time_in_ms) // Won't happen, window is too short
		return;
	length = context->endcreditsforatmost.time_in_ms > window
				 ? window
				 : context->endcreditsforatmost.time_in_ms;
...
void try_to_add_start_credits(struct encoder_ctx *context, LLONG start_ms)
{
	...
	// We have a windows from last_displayed_subs_ms to l - we need to see if it
	// fits

	if (l < context->startcreditsnotbefore.time_in_ms) // Too early
		return;

	if (context->last_displayed_subs_ms + 1 >
		context->startcreditsnotafter.time_in_ms) // Too late
		return;

	st = context->startcreditsnotbefore.time_in_ms >
				 (context->last_displayed_subs_ms + 1)
			 ? context->startcreditsnotbefore.time_in_ms
			 : (context->last_displayed_subs_ms +
				1); // When would credits actually start
	...
	if (context->startcreditsforatleast.time_in_ms >
		window) // Window is too short
		return;

	length = context->startcreditsforatmost.time_in_ms > window
				 ? window
				 : context->startcreditsforatmost.time_in_ms;
...
#define check_ret(filename)                                                    \
	...
	if (cfg->cc_to_stdout == CCX_FALSE && cfg->send_to_srv == CCX_FALSE &&
		cfg->extract == 12 && cfg->write_format != CCX_OF_MCC)
		nb_lang = 2;
	else
		nb_lang = 1;
	...
	if (ctx->generates_file && cfg->cc_to_stdout == CCX_FALSE &&
		cfg->send_to_srv == CCX_FALSE)
	{
		if (cfg->output_filename != NULL)
		{
			...
			if ((cfg->extract == 12) && (cfg->write_format != CCX_OF_MCC))
			{
				...
				ret = init_write(
					&ctx->out[1],
					create_outfilename(basefilename, "_2", extension),
					cfg->with_semaphore);
		...
		else if (cfg->write_format != CCX_OF_NULL)
		{
			...
			if ((cfg->extract == 12) && (cfg->write_format != CCX_OF_MCC))
			{
				ret = init_write(
					&ctx->out[0],
					create_outfilename(basefilename, "_1", extension),
					cfg->with_semaphore);
			...
			{
				ret = init_write(
					ctx->out, create_outfilename(basefilename, NULL, extension),
					cfg->with_semaphore);
	...
	if (cfg->send_to_srv == CCX_TRUE)
	{
		...
		connect_to_srv(ccx_options.srv_addr, ccx_options.srv_port,
					   ccx_options.tcp_desc, ccx_options.tcp_password);
	}
		...
		for (int i = 0; i < CCX_DTVCC_MAX_SERVICES; i++)
		{
			...
			else
			{
				...
				ccx_dtvcc_writer_init(&ctx->dtvcc_writers[i], basefilename,
									  ctx->program_number, i + 1,
									  cfg->write_format, cfg);
				free(basefilename);
			}
		}
	}
	...
	if (ret)
	{
		print_error(cfg->gui_mode_reports,
					"Output filename is same as one of input filenames. Check "
					"output parameters.\n");
...
struct encoder_ctx *init_encoder(struct encoder_cfg *opt)
{
	...
	if (opt->line_terminator_lf)
		ctx->encoded_crlf_length =
			encode_line(ctx, ctx->encoded_crlf, (unsigned char *)"\n");
	else
		ctx->encoded_crlf_length =
			encode_line(ctx, ctx->encoded_crlf, (unsigned char *)"\r\n");

	ctx->encoded_br_length =
		encode_line(ctx, ctx->encoded_br, (unsigned char *)"<br>");
...
{
	return write(ctx->out[lang].fh, ctx->encoded_crlf,
				 ctx->encoded_crlf_length);
}
...
int encode_sub(struct encoder_ctx *context, struct cc_subtitle *sub)
{
	...
	if (sub->type == CC_608)
	{
		...
		{
			// Determine context based on channel. This replaces the code that
			// was above, as this was incomplete (for cases where -12 was used
			// for example)
			...
			if (data->format == SFORMAT_XDS)
			{
				...
				xds_write_transcript_line_prefix(context, out, data->start_time,
												 data->end_time,
												 data->cur_xds_packet_class);
				if (data->xds_len > 0)
				{
			...
			{
			case CCX_OF_SRT:
				if (!context->startcredits_displayed &&
					context->start_credits_text != NULL)
			...
			case CCX_OF_SIMPLE_XML:
				if (ccx_options.keep_output_closed &&
					context->out->temporarily_closed)
				{
	...
	for (int i = 0; i < 15; i++)
	{
		if (data->row_used[i])
		{
			...
			if (!time_reported)
			{
				...
				millis_to_time(
					ms_end - 1, &h2, &m2, &s2,
					&ms2); // -1 To prevent overlapping with next line.
			...
			int length =
				get_decoder_line_encoded_for_gui(context->subline, i, data);
						...
						int track_id)
{
	...
	enc_ctx->out->filename = create_outfilename(
		get_basename(enc_ctx->first_input_file), suffix, ext);
	enc_ctx->out->fh =
		open(enc_ctx->out->filename, O_RDWR | O_CREAT | O_TRUNC | O_BINARY,
			 S_IREAD | S_IWRITE);
