...
static int ccx_demuxer_open(struct ccx_demuxer *ctx, const char *file)
{
	...
	{
		if (ctx->infd !=
			-1) // Means we had already processed stdin. So we're done.
		{
	...
	else if (ccx_options.input_source == CCX_DS_NETWORK)
	{
		...
		ctx->infd = start_upd_srv(ccx_options.udpsrc, ccx_options.udpaddr,
								  ccx_options.udpport);
		if (ctx->infd < 0)
		{
	...
	else if (ccx_options.input_source == CCX_DS_TCP)
	{
		...
		ctx->infd =
			start_tcp_srv(ccx_options.tcpport, ccx_options.tcp_password);
	}
	else
	{
#ifdef _WIN32
	...
	if (ctx->auto_stream == CCX_SM_AUTODETECT)
	{
		...
		case CCX_SM_ELEMENTARY_OR_NOT_FOUND:
			mprint(
				"\rFile seems to be an elementary stream, enabling ES mode\n");
			break;
		...
		case CCX_SM_AUTODETECT:
			fatal(CCX_COMMON_EXIT_BUG_BUG,
				  "In ccx_demuxer_open: Impossible value in stream_mode. "
...
int ccx_demuxer_write_es(struct ccx_demuxer *ctx, unsigned char *buf,
						 size_t len)
{
...
struct ccx_demuxer *init_demuxer(void *parent, struct demuxer_cfg *cfg)
{
	...
	for (int i = 0; i < MAX_PROGRAM; i++)
	{
		...
		ctx->pinfo[i].version = 0xFF; // Not real in a real stream since it's 5
									  // bits. FF => Not initialized
	}
	...
	for (i = 0; i < cfg->nb_ts_cappid; i++)
	{
		if (ctx->codec == CCX_CODEC_ANY)
			update_capinfo(ctx, cfg->ts_cappids[i], cfg->ts_datastreamtype,
						   CCX_CODEC_NONE, 0, NULL);
		else
			update_capinfo(ctx, cfg->ts_cappids[i], cfg->ts_datastreamtype,
						   ctx->codec, 0, NULL);
	}
	...
	{
		if ((ctx->fh_out_elementarystream =
				 fopen(cfg->out_elementarystream_filename, "wb")) == NULL)
		{
			print_error(CCX_COMMON_EXIT_FILE_CREATION_FAILED,
						"Unable to open clean file: %s \nDetails : %s .\n",
						cfg->out_elementarystream_filename,
						strerror(errno)); // printing specific error -
										  // Permission Denied/ Wrong Path etc..
			return NULL;
		}
	}
