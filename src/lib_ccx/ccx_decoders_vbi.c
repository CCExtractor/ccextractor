...
struct ccx_decoder_vbi_ctx *init_decoder_vbi(struct ccx_decoder_vbi_cfg *cfg)
{
	...
	if (cfg == NULL)
	{
		...
		vbi_raw_decoder_add_services(&vbi->zvbi_decoder, VBI_SLICED_CAPTION_525,
									 /* strict */ 0);
	}
	return vbi;
}

int decode_vbi(struct lib_cc_decode *dec_ctx, uint8_t field,
			   unsigned char *buffer, size_t len, struct cc_subtitle *sub)
{
	int i = 0;
	...
	n_lines =
		vbi_raw_decode(&dec_ctx->vbi_decoder->zvbi_decoder, buffer, sliced);
	// n_lines = vbi3_raw_decoder_decode (&dec_ctx->vbi_decoder->zvbi_decoder,
	// sliced, 2, buffer);
	if (n_lines > 0)
	{
		for (i = 0; i < n_lines; ++i)
