						 ...
						 LLONG ms_start, LLONG ms_end)
{
	...
	sprintf(
		timeline,
		"Dialogue: "
		"0,%02u:%02u:%02u.%01u,%02u:%02u:%02u.%02u,Default,,0000,0000,0000,,",
	...
	if (el == NULL || unescaped == NULL)
		fatal(EXIT_NOT_ENOUGH_MEMORY,
			  "In write_stringz_as_ssa() - not enough memory.\n");
	int pos_r = 0;
	int pos_w = 0;
...
int write_cc_bitmap_as_ssa(struct cc_subtitle *sub, struct encoder_ctx *context)
{
	...
	if (str)
	{
		...
		if (context->prev_start != -1 || !(sub->flags & SUB_EOD_MARKER))
		{
			...
			sprintf(timeline,
					"Dialogue: "
					"0,%02u:%02u:%02u.%01u,%02u:%02u:%02u.%02u,Default,,0000,"
					"0000,0000,,",
					h1, m1, s1, ms1 / 10, h2, m2, s2, ms2 / 10);
							 ...
							 struct encoder_ctx *context)
{
	...
	while (sub)
	{
		if (sub->type == CC_TEXT)
		{
			ret = write_stringz_as_ssa(sub->data, context, sub->start_time,
									   sub->end_time);
			freep(&sub->data);
			sub->nb_data = 0;
			ret = 1;
