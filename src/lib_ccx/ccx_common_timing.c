...
double current_fps = (double)30000.0 / 1001;
	/* 29.97 */ // TODO: Get from framerates_values[] instead
...
init_timing_ctx(struct ccx_common_timing_settings_t *cfg)
{
	struct ccx_common_timing_ctx *ctx =
		malloc(sizeof(struct ccx_common_timing_ctx));
	if (!ctx)
		return NULL;
	...
	ctx->fts_now =
		0; // Time stamp of current file (w/ fts_offset, w/o fts_global)
...
void set_current_pts(struct ccx_common_timing_ctx *ctx, LLONG pts)
{
	...
	dbg_print(CCX_DMT_VIDES, "PTS: %s (%8u)",
			  print_mstime_static(ctx->current_pts / (MPEG_CLOCK_FREQ / 1000)),
...
int set_fts(struct ccx_common_timing_ctx *ctx)
{
	...
	// First check for timeline jump (only when min_pts was set (implies
	// sync_pts)).
	int dif = 0;
	if (ctx->pts_set == 2)
	{
		...
		if (dif < -0.2 || dif >= max_dif)
		{
			...
			ccx_common_logging.log_ftn(
				"\nWarning: Reference clock has changed abruptly (%d seconds "
				"filepos=%lld), attempting to synchronize\n",
			...
			ccx_common_logging.log_ftn("Current PTS value: %lld\n",
									   ctx->current_pts);
			ccx_common_logging.log_ftn(
				"Note: You can disable this behavior by adding -ignoreptsjumps "
				"to the parameters.\n");
			pts_jump = 1;
			pts_big_change = 1;
				...
				ctx->current_picture_coding_type != CCX_FRAME_TYPE_I_FRAME)
			{
				...
				ccx_common_logging.log_ftn("Change did not occur on first "
										   "frame - probably a broken GOP\n");
				return CCX_OK;
			}
		}
	}

	// If min_pts was set just before a rollover we compensate by "roll-oving"
	// it too
	if (ctx->pts_set == 2 && !ctx->min_pts_adjusted) // min_pts set
	{
		// We want to be aware of the upcoming rollover, not after it happened,
		// so we don't take the 3 most significant bits but the 3 next ones
		...
		{
			// Huge difference possibly means the first min_pts was actually
			// just over the boundary Take the current_pts (smaller, accounting
			// for the rollover) instead
			ctx->min_pts = ctx->current_pts;
			ctx->min_pts_adjusted = 1;
		}
		else if (cur_pts_big_bits >= 1 &&
				 cur_pts_big_bits <= 6) // Far enough from the boundary
		{
			// Prevent the eventual difference with min_pts to make a bad
			// adjustment
			ctx->min_pts_adjusted = 1;
		}
	}
	...
	if (ctx->pts_set != 0)
	{
		...
		if (ctx->current_pts < ctx->min_pts && !pts_jump)
		{
			...
			{ // It needs to be "+1" because the current frame is
				// not yet counted.
				ctx->fts_offset =
					(LLONG)((total_frames_count - frames_since_ref_time + 1) *
							1000.0 / current_fps);
			}
			...
			ccx_common_logging.debug_ftn(
				CCX_DMT_TIME,
				"Total_frames_count %u frames_since_ref_time %u\n",
	...
	if (1) // CFS: Remove or think decent condition
	{
		if (ctx->pts_set)
		{
			...
			ctx->fts_now = (LLONG)((ctx->current_pts - ctx->min_pts) /
									   (MPEG_CLOCK_FREQ / 1000) +
								   ctx->fts_offset);
		...
		else
		{
			// No PTS info at all!!
			ccx_common_logging.log_ftn(
				"Set PTS called without any global timestamp set\n");
			return CCX_EINVAL;
		}
	}
...
LLONG get_fts(struct ccx_common_timing_ctx *ctx, int current_field)
{
	...
	default:
		ccx_common_logging.fatal_ftn(CCX_COMMON_EXIT_BUG_BUG,
									 "get_fts: unhandled branch");
	}
	//	ccx_common_logging.debug_ftn(CCX_DMT_TIME, "[FTS] "
	//			"fts: %llu, fts_now: %llu, fts_global: %llu, current_field: %llu,
...
void print_debug_timing(struct ccx_common_timing_ctx *ctx)
{
	...
	// for uninitialized min_pts
	LLONG tempmin_pts =
		(ctx->min_pts == 0x01FFFFFFFFLL ? ctx->sync_pts : ctx->min_pts);

	ccx_common_logging.log_ftn(
		"Sync time stamps:  PTS: %s                ",
	...
	LLONG ptslenms =
		(unsigned)((ctx->sync_pts - tempmin_pts) / (MPEG_CLOCK_FREQ / 1000) +
				   ctx->fts_offset);
	...
	ccx_common_logging.log_ftn(
		"Max FTS diff. to   PTS:       %6lldms              GOP:       "
		"%6lldms\n\n",
...
{
	int seconds = (g->time_code_hours * 3600) + (g->time_code_minutes * 60) +
				  g->time_code_seconds;
...
{
	if (!((g->time_code_hours <= 23) && (g->time_code_minutes <= 59) &&
		  (g->time_code_seconds <= 59) && (g->time_code_pictures <= 59)))
		return 0;
