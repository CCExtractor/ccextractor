...
static const char *XDSclasses_short[] = {"CUR", "FUT", "CHN", "MIS",
										 "PUB", "RES", "PRV", "END"};

void xds_write_transcript_line_prefix(struct encoder_ctx *context,
									  struct ccx_s_write *wb, LLONG start_time,
									  LLONG end_time, int cur_xds_packet_class)
{
	...
	if (start_time == -1)
	{
		// Means we entered XDS mode without making a note of the XDS start
		// time. This is a bug.
		ccx_common_logging.fatal_ftn(
			CCX_COMMON_EXIT_BUG_BUG,
			"Bug in timedtranscript (XDS). Please report.");
	}
	...
	if (context->transcript_settings->showStartTime)
	{
		...
		if (context->transcript_settings->relativeTimestamp)
		{
			...
			{
				millis_to_time(start_time + context->subs_delay, &h1, &m1, &s1,
							   &ms1);
				fdprintf(wb->fh, "%02u:%02u:%02u%c%03u|", h1, m1, s1,
						 context->millis_separator, ms1);
			}
			else
			{
				fdprintf(wb->fh, "%lld%c%03d|",
	...
	if (context->transcript_settings->showEndTime)
	{
		...
		if (context->transcript_settings->relativeTimestamp)
		{
			if (utc_refvalue == UINT64_MAX)
			{
				...
				fdprintf(wb->fh, "%02u:%02u:%02u%c%03u|", h2, m2, s2,
						 context->millis_separator, ms2);
			}
			else
			{
				fdprintf(wb->fh, "%lld%s%03d|", end_time / 1000,
						 context->millis_separator, end_time % 1000);
			}
		}
		else
		{
