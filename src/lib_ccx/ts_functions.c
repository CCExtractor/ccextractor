...
#define RAI_MASK                                                               \
	0x40 // byte mask to check if RAI bit is set (random access indicator)
...
char *get_buffer_type_str(struct cap_info *cinfo)
{
	...
	else if (cinfo->stream == CCX_STREAM_TYPE_UNKNOWNSTREAM &&
			 ccx_options.hauppauge_mode)
	{
		return strdup("Hauppage");
	}
	else if (cinfo->stream == CCX_STREAM_TYPE_PRIVATE_MPEG2 &&
			 cinfo->codec == CCX_CODEC_TELETEXT)
	{
...
void pes_header_dump(uint8_t *buffer, long len)
{
	...
	pes_packet_length =
		6 + ((buffer[4] << 8) |
			 buffer[5]); // 5th and 6th byte of the header define the length of
						 // the rest of the packet (+6 is for the prefix, stream
						 // ID and packet length)
...
enum ccx_stream_type get_buffer_type(struct cap_info *cinfo)
{
	...
	else if (cinfo->stream == CCX_STREAM_TYPE_PRIVATE_USER_MPEG2 &&
			 cinfo->codec == CCX_CODEC_ATSC_CC)
	{
		return CCX_PES;
	}
...
int ts_readpacket(struct ccx_demuxer *ctx, struct ts_payload *payload)
{
	...
	if (ctx->m2ts)
	{
		...
		if (result != 4)
		{
			if (result > 0)
				mprint(
					"Premature end of file (incomplete TS packer header, "
					"expected 4 bytes to skip M2TS extra bytes, got %lld).\n",
					result);
			return CCX_EOF;
		}
	}
	...
	{
		if (result > 0)
			mprint("Premature end of file - Transport Stream packet is "
				   "incomplete (expected 188 bytes, got %lld).\n",
				   result);
		return CCX_EOF;
	}
	...
	while (tspacket[0] != 0x47)
	{
		if (printtsprob)
		{
			dbg_print(CCX_DMT_DUMPDEF,
					  "\nProblem: No TS header mark (filepos=%lld). Received "
					  "bytes:\n",
					  ctx->past);
	...
	{
		dbg_print(CCX_DMT_DUMPDEF,
				  "Warning: Defective (error indicator on) TS packet "
				  "(filepos=%lld):\n",
				  ctx->past);
	...
	{
		// Take the PCR (Program Clock Reference) from here, in case PTS is not
		// available (copied from telxcc).
		...
		else
		{
			...
			dbg_print(CCX_DMT_PARSE,
					  "  Reject package - set length to zero.\n");
		}
	}
	else
	...
	dbg_print(CCX_DMT_PARSE,
			  "TS pid: %d  PES start: %d  counter: %u  payload length: %u  "
			  "adapt length: %d\n",
...
void look_for_caption_data(struct ccx_demuxer *ctx, struct ts_payload *payload)
{
	...
	if (payload->length < 4 ||
		ctx->PIDs_seen[payload->pid] ==
			3) // Second thing means we already inspected this PID
		return;
...
struct demuxer_data *
search_or_alloc_demuxer_data_node_by_pid(struct demuxer_data **data, int pid)
{
	struct demuxer_data *ptr;
...
int copy_capbuf_demux_data(struct ccx_demuxer *ctx, struct demuxer_data **data,
						   struct cap_info *cinfo)
{
	...
	vpesdatalen = read_video_pes_header(ctx, ptr, cinfo->capbuf, &pesheaderlen,
										cinfo->capbuflen);
	if (ccx_options.pes_header_to_stdout &&
		cinfo->codec ==
			CCX_CODEC_DVB) // for teletext we have its own header dump
	{
	...
	if (vpesdatalen < 0)
	{
		dbg_print(CCX_DMT_VERBOSE,
				  "Seems to be a broken PES. Terminating file handling.\n");
		return CCX_EOF;
	}
	...
	if (ccx_options.hauppauge_mode)
	{
		...
		if (!haup_capbuflen)
		{
			// Do this so that we always return something until EOF. This will
			// be skipped.
			ptr->buffer[ptr->len++] = 0xFA;
			ptr->buffer[ptr->len++] = 0x80;
			ptr->buffer[ptr->len++] = 0x80;
		...
		for (int i = 0; i < haup_capbuflen; i += 12)
		{
			...
			if (haup_stream_id == 0xbd && haup_capbuf[i + 4] == 0 &&
				haup_capbuf[i + 5] == 6)
			{
				// Because I (CFS) don't have a lot of samples for this, for now
				// I make sure everything is like the one I have: 12 bytes total
				// length, stream id = 0xbd (Private non-video and non-audio),
				// etc
				if (2 > BUFSIZE - ptr->len)
				{
					fatal(CCX_COMMON_EXIT_BUG_BUG,
						  "Remaining buffer (%lld) not enough to hold the 3 "
						  "Hauppage bytes.\n"
						  "Please send bug report!",
						  BUFSIZE - ptr->len);
				}
				if (haup_capbuf[i + 9] == 1 ||
					haup_capbuf[i + 9] == 2) // Field match. // TODO: If
											 // extract==12 this won't work!
				{
					if (haup_capbuf[i + 9] == 1)
				...
				/*
				   if (inbuf>1024) // Just a way to send the bytes to the
				   decoder from time to time, otherwise the buffer will fill up.
				   break;
				   else
				   continue; */
			}
	...
	if (!ccx_options.hauppauge_mode) // in Haup mode the buffer is filled
									 // somewhere else
	{
...
uint64_t get_pts(uint8_t *buffer)
{
	...
	if (pes_prefix == 0x000001)
	{
		pes_packet_length =
			6 + ((buffer[4] << 8) |
				 buffer[5]); // 5th and 6th byte of the header define the length
							 // of the rest of the packet (+6 is for the prefix,
							 // stream ID and packet length)
		...
		if (optional_pes_header_included == YES &&
			optional_pes_header_length > 0 && (buffer[7] & 0x80) > 0)
		{
			// get associated PTS as it exists
			pts = (buffer[9] & 0x0e);
			pts <<= 29;
...
uint64_t get_video_min_pts(struct ccx_demuxer *context)
{
	...
	do
	{
		...
		{
			// Packetized Elementary Stream (PES) 32-bit start code
			uint64_t pes_prefix = (payload.start[0] << 16) |
								  (payload.start[1] << 8) | payload.start[2];
		...
		if (pts != UINT64_MAX)
		{
			...
			pts_array =
				realloc(pts_array, num_of_remembered_pts * sizeof(uint64_t));
...
long ts_readstream(struct ccx_demuxer *ctx, struct demuxer_data **data)
{
	...
	int packet_analysis_mode = 0; // If we can't find any packet with CC based
								  // from PMT, look for captions in all packets
	int ret = CCX_EAGAIN;
	...
	do
	{
		...
		{
			dbg_print(CCX_DMT_VERBOSE,
					  "Packet (pid %u) skipped - transport error.\n",
					  payload.pid);
			continue;
		}
		...
		if (payload.pid == 0) // This is a PAT
		{
			...
			if (ctx->PID_buffers[payload.pid] != NULL &&
				ctx->PID_buffers[payload.pid]->buffer_length > 0)
				parse_PAT(ctx); // Returns 1 if there was some data in the
								// buffer already
			continue;
		}
		...
		if (ccx_options.xmltv >= 1 &&
			payload.pid >= 0x1000) // This may be ATSC EPG packet
			parse_EPG_packet(ctx->parent);

		for (j = 0; j < ctx->nb_program; j++)
		{
			if (ctx->pinfo[j].analysed_PMT_once == CCX_TRUE &&
				ctx->pinfo[j].pcr_pid == payload.pid && payload.have_pcr)
			{
			...
			if (ctx->pinfo[j].pid == payload.pid)
			{
				if (!ctx->PIDs_seen[payload.pid])
					dbg_print(CCX_DMT_PAT,
							  "This PID (%u) is a PMT for program %u.\n",
		...
		if (j != ctx->nb_program)
		{
			...
			if (ctx->PID_buffers[payload.pid] != NULL &&
				ctx->PID_buffers[payload.pid]->buffer_length > 0)
				if (parse_PMT(ctx, ctx->PID_buffers[payload.pid]->buffer + 1,
							  ctx->PID_buffers[payload.pid]->buffer_length - 1,
							  pinfo))
					gotpes = 1; // Signals that something changed and that we
		...
		case 0: // First time we see this PID
			if (ctx->PIDs_programs[payload.pid])
			{
				dbg_print(CCX_DMT_PARSE,
						  ...
						  payload.pid,
						  desc[ctx->PIDs_programs[payload.pid]
			...
			ctx->num_of_PIDs++;
			break;
		case 1: // Saw it before but we didn't know what program it belonged to.
				// Luckier now?
			if (ctx->PIDs_programs[payload.pid])
			{
				dbg_print(
		...
		case 2: // Already seen and reported with correct program
			break;
		case 3: // Already seen, reported, and inspected for CC data (and found
				// some)
			break;
		}

		// PTS calculation
		if (payload.pesstart) // if there is PES Header data in the payload and
							  // we didn't get the first pts of that stream
		{
			// Packetized Elementary Stream (PES) 32-bit start code
			uint64_t pes_prefix = (payload.start[0] << 16) |
								  (payload.start[1] << 8) | payload.start[2];
		...
		if (payload.pid == 8191) // Null packet
			continue;
		if (payload.pid == 1003 && !ctx->hauppauge_warning_shown &&
			!ccx_options.hauppauge_mode)
		{
			// TODO: Change this very weak test for something more decent such
			// as size.
			mprint("\n\nNote: This TS could be a recording from a Hauppage "
				   "card. If no captions are detected, try --hauppauge\n\n");
			ctx->hauppauge_warning_shown = 1;
		}

		if (ccx_options.hauppauge_mode && payload.pid == HAUPPAGE_CCPID)
		{
			// Haup packets processed separately, because we can't mix payloads.
			// So they go in their own buffer copy payload to capbuf
			int haup_newcapbuflen = haup_capbuflen + payload.length;
			if (haup_newcapbuflen > haup_capbufsize)
			{
				haup_capbuf =
					(unsigned char *)realloc(haup_capbuf, haup_newcapbuflen);
				if (!haup_capbuf)
					fatal(EXIT_NOT_ENOUGH_MEMORY,
						  "Not enough memory to store hauppauge packets");
		...
		{
			dbg_print(CCX_DMT_VERBOSE,
					  "Packet (pid %u) skipped - no payload.\n", payload.pid);
			continue;
		}
		...
		if (cinfo == NULL)
		{
			if (!packet_analysis_mode)
				dbg_print(CCX_DMT_PARSE,
						  "Packet (pid %u) skipped - no stream with captions "
						  "identified yet.\n",
						  payload.pid);
			else
		...
		if ((cinfo->prev_counter == 15 ? 0 : cinfo->prev_counter + 1) !=
			payload.counter)
		{
		...
		{
			dbg_print(CCX_DMT_PARSE,
					  "\nPES finished (%ld bytes/%ld PES packets/%ld total "
					  "packets)\n",
	...
	for (int i = 0; i < ctx->nb_program; i++)
	{
		...
		if (!pinfo->has_all_min_pts)
		{
			for (int j = 0; j < ctx->num_of_PIDs; j++)
			{
				...
				{
					if (ctx->PIDs_programs[ctx->have_PIDs[j]]->program_number ==
						pinfo->program_number)
					{
						if (ctx->min_pts[j] != UINT64_MAX)
						{
							...
							if (ctx->stream_id_of_each_pid[j] >= 0xc0 &&
								ctx->stream_id_of_each_pid[j] <= 0xdf)
								if (ctx->min_pts[j] <
									pinfo->got_important_streams_min_pts[AUDIO])
									pinfo
										->got_important_streams_min_pts[AUDIO] =
										ctx->min_pts[j];
							if (ctx->stream_id_of_each_pid[j] >= 0xe0 &&
								ctx->stream_id_of_each_pid[j] <= 0xef)
								if (ctx->min_pts[j] <
									pinfo->got_important_streams_min_pts[VIDEO])
									pinfo
										->got_important_streams_min_pts[VIDEO] =
										ctx->min_pts[j];
							if (pinfo->got_important_streams_min_pts
										[PRIVATE_STREAM_1] != UINT64_MAX &&
								pinfo->got_important_streams_min_pts[AUDIO] !=
									UINT64_MAX &&
								pinfo->got_important_streams_min_pts[VIDEO] !=
									UINT64_MAX)
