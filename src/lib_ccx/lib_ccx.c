...
static struct ccx_decoders_common_settings_t *
init_decoder_setting(struct ccx_s_options *opt)
{
	...
	memcpy(&setting->extraction_start, &opt->extraction_start,
		   sizeof(struct ccx_boundary_time));
	memcpy(&setting->extraction_end, &opt->extraction_end,
		   sizeof(struct ccx_boundary_time));
...
static void
dinit_decoder_setting(struct ccx_decoders_common_settings_t **setting)
{
	freep(setting);
}
...
struct lib_ccx_ctx *init_libraries(struct ccx_s_options *opt)
{
	...
	if (!ctx)
		ccx_common_logging.fatal_ftn(EXIT_NOT_ENOUGH_MEMORY,
									 "init_libraries: Not enough memory "
									 "allocating lib_ccx_ctx context.");
	memset(ctx, 0, sizeof(struct lib_ccx_ctx));

	if (opt->xmltv)
	{
		ctx->epg_inited = 1;
		ctx->epg_buffers = (struct PSI_buffer *)malloc(
			sizeof(struct PSI_buffer) * (0xfff + 1));
		ctx->eit_programs = (struct EIT_program *)malloc(
			sizeof(struct EIT_program) * (TS_PMT_MAP_SIZE + 1));
		ctx->eit_current_events =
			(int32_t *)malloc(sizeof(int32_t) * (TS_PMT_MAP_SIZE + 1));
		ctx->ATSC_source_pg_map = (int16_t *)malloc(sizeof(int16_t) * (0xffff));
		memset(ctx->epg_buffers, 0, sizeof(struct PSI_buffer) * (0xfff + 1));
		memset(ctx->eit_programs, 0,
			   sizeof(struct EIT_program) * (TS_PMT_MAP_SIZE + 1));
		memset(ctx->eit_current_events, 0,
			   sizeof(int32_t) * (TS_PMT_MAP_SIZE + 1));
		memset(ctx->ATSC_source_pg_map, 0, sizeof(int16_t) * (0xffff));
		if (!ctx->epg_buffers || !ctx->eit_programs ||
			!ctx->eit_current_events || !ctx->ATSC_source_pg_map)
	...
	struct ccx_decoder_608_report *report_608 =
		malloc(sizeof(struct ccx_decoder_608_report));
	if (!report_608)
		ccx_common_logging.fatal_ftn(
			EXIT_NOT_ENOUGH_MEMORY,
			"init_libraries: Not enough memory allocating report_608");
	memset(report_608, 0, sizeof(struct ccx_decoder_608_report));

	ccx_decoder_dtvcc_report *report_dtvcc =
		(ccx_decoder_dtvcc_report *)malloc(sizeof(ccx_decoder_dtvcc_report));
	if (!report_dtvcc)
		ccx_common_logging.fatal_ftn(
			EXIT_NOT_ENOUGH_MEMORY,
			"init_libraries: Not enough memory allocating report_dtvcc");
...
void dinit_libraries(struct lib_ccx_ctx **ctx)
{
	...
	int i;
	list_for_each_entry_safe(dec_ctx, dec_ctx1, &lctx->dec_ctx_head, list,
...
struct lib_cc_decode *update_decoder_list(struct lib_ccx_ctx *ctx)
{
	...
	if (list_empty(&ctx->dec_ctx_head))
	{
		...
		if (!dec_ctx)
			fatal(EXIT_NOT_ENOUGH_MEMORY, "In update_decoder_list: Not enough "
										  "memory to init_cc_decode.\n");
												...
												struct cap_info *cinfo)
{
		...
		if (list_empty(&ctx->dec_ctx_head))
		{
			...
			if (!dec_ctx)
				fatal(EXIT_NOT_ENOUGH_MEMORY,
					  "In update_decoder_list_cinfo: Not enough memory "
					  "allocating dec_ctx (multiprogram == false)\n");
			list_add_tail(&(dec_ctx->list), &(ctx->dec_ctx_head));
		}
	}
	else
	{
		...
		if (!dec_ctx)
			fatal(EXIT_NOT_ENOUGH_MEMORY,
				  "In update_decoder_list_cinfo: Not enough memory allocating "
				  "dec_ctx ((multiprogram == true)\n");
		list_add_tail(&(dec_ctx->list), &(ctx->dec_ctx_head));
	}
											  ...
											  struct cap_info *cinfo)
{
	...
	else
	{
		...
		sprintf(ccx_options.enc_cfg.output_filename, "%s_%d%s",
				ctx->basefilename, pn, extension);
		enc_ctx = init_encoder(&ccx_options.enc_cfg);
		if (!enc_ctx)
		{
			freep(&extension);
