							  ...
							  LLONG ms_start, LLONG ms_end)
{
	...
	if (el == NULL || unescaped == NULL)
		fatal(EXIT_NOT_ENOUGH_MEMORY,
			  "In write_stringz_as_smptett() - not enough memory.\n");

	millis_to_time(ms_start, &h1, &m1, &s1, &ms1);
	millis_to_time(ms_end - 1, &h2, &m2, &s2, &ms2);

	sprintf((char *)str,
			"<p begin=\"%02u:%02u:%02u.%03u\" end=\"%02u:%02u:%02u.%03u\">\r\n",
	...
	for (i = sub->nb_data - 1; i >= 0; i--)
	{
		if (rect[i].ocr_text && *(rect[i].ocr_text))
		{
			if (context->prev_start != -1 || !(sub->flags & SUB_EOD_MARKER))
			{
				...
				sprintf((char *)context->buffer,
						"<p begin=\"%02u:%02u:%02u.%03u\" "
						"end=\"%02u:%02u:%02u.%03u\">\n",
						h1, m1, s1, ms1, h2, m2, s2, ms2);
								 ...
								 struct encoder_ctx *context)
{
	...
	while (sub)
	{
		if (sub->type == CC_TEXT)
		{
			write_stringz_as_smptett(sub->data, context, sub->start_time,
									 sub->end_time);
			freep(&sub->data);
			sub->nb_data = 0;
		}
							   ...
							   struct encoder_ctx *context)
{
	...
	for (int row = 0; row < 15; row++)
	{
		if (data->row_used[row])
		{
			...
			{
				int unicode = 0;
				get_char_in_unicode((unsigned char *)&unicode,
									data->characters[row][column]);
			...
			if (firstcol >= 0)
			{
				...
				sprintf(str,
						"      <p begin=\"%02u:%02u:%02u.%03u\" "
						"end=\"%02u:%02u:%02u.%03u\" tts:origin=\"%1.3f%% "
						"%1.3f%%\">\n        <span>",
				...
				used =
					encode_line(context, context->buffer, (unsigned char *)str);
				write(context->out->fh, context->buffer, used);
				...
				char *final = malloc(strlen((const char *)(context->subline)) +
									 1000); // Being overly generous? :P
				char *temp =
					malloc(strlen((const char *)(context->subline)) + 1000);
				*final = 0;
				*temp = 0;
				/*
					final	: stores formatted HTML sentence. This will be
				   written in subtitle file. temp	: stored temporary sentences
				   required while formatting

					+1000 because huge <span> and <style> tags are added. This
				   will just prevent overflow (hopefully).
				*/
				...
				*/
				...
				if (start != NULL) // subtitle has style associated with it,
								   // will need formatting.
				{
					...
					char *end = strstr((const char *)(context->subline),
									   end_tag); // occurrence of closing tag
												 // (</i> OR </b> OR </u>)
					...
					else
					{
						...
						strncat(final, (const char *)(context->subline),
								start_index); // copying content before opening
											  // tag e.g. <i>

						strcat(final,
							   "<span>"); // adding <span> : replacement of <i>

						// The content in italics is between <i> and </i>, i.e.
						// between (start_index + 3) and end_index.

						strncat(temp,
								(const char *)(context->subline) + start_index +
									3,
								end_index - start_index -
									3); // the content in italics
						...
						if (style == 1)
							strcpy(temp,
								   ...
								   "tts:fontSize=\"18px\" "
								   "tts:fontStyle=\"italic\"/> </span>");

						else if (style == 2)
							strcpy(temp,
								   "<style tts:backgroundColor=\"#000000FF\" "
								   "tts:fontSize=\"18px\" "
								   "tts:fontWeight=\"bold\"/> </span>");
							...
							strcpy(
								...
								"<style tts:backgroundColor=\"#000000FF\" "
								"tts:fontSize=\"18px\" "
								"tts:textDecoration=\"underline\"/> </span>");

						strcat(final, temp); // adding appropriate style tag.

						sprintf(temp, "%s",
								(const char *)(context->subline) + end_index +
									4); // finding remaining sentence.
				...
				else // No style or Font Color
				{
					...
					start = strstr((const char *)(context->subline),
								   "<font color"); // spec : <font
												   // color="#xxxxxx"> cc </font>
					if (start != NULL) // font color attribute is present
					{
						char *end =
							strstr((const char *)(context->subline), "</font>");
						if (end == NULL)
						{
							// Incorrect styling, writing as it is
							strcpy(final, (const char *)(context->subline));
						...
						{
							int start_index =
								start - (char *)(context->subline);
							int end_index = end - (char *)(context->subline);

							strncat(final, (const char *)(context->subline),
									start_index); // copying content before
												  // opening tag e.g. <font ..>

							strcat(final, "<span>"); // adding <span> :
													 // replacement of <font ..>

							char *temp_pointer =
								strchr((const char *)(context->subline),
									   '#'); // locating color code

							char color_code[7];
							strncpy(color_code, temp_pointer + 1,
									6); // obtained color code
							color_code[6] = '\0';

							temp_pointer =
								strchr((const char *)(context->subline),
									   '>'); // The content is in between <font
											 // ..> and </font>

							strncat(temp, temp_pointer + 1,
									end_index -
										(temp_pointer -
										 (char *)(context->subline) + 1));
							...
							sprintf(temp,
									...
									"tts:color=\"%s\" "
									"tts:fontSize=\"18px\"/></span>",
									color_code);

							strcat(final, temp); // adding font color tag

							sprintf(temp, "%s",
									(const char *)(context->subline) +
										end_index +
										7); // finding remaining sentence.
				...
				sprintf(str, "        <style tts:backgroundColor=\"#000000FF\" "
							 "tts:fontSize=\"18px\"/></span>\n      </p>\n");
				if (context->encoding != CCX_ENC_UNICODE)
				{
					dbg_print(CCX_DMT_DECODER_608, "\r%s\n", str);
