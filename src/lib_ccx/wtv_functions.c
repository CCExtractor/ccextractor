...
int add_skip_chunks(struct ccx_demuxer *ctx, struct wtv_chunked_buffer *cb,
...
void get_sized_buffer(struct ccx_demuxer *ctx, struct wtv_chunked_buffer *cb,
					  uint32_t size);
void skip_sized_buffer(struct ccx_demuxer *ctx, struct wtv_chunked_buffer *cb,
					   uint32_t size);
...
uint64_t time_to_pes_time(uint64_t time) { return ((time / 10000) * 90); }
...
// skip_sized_buffer. Same as get_sized_buffer, only without actually copying
// any data in to the buffer.
void skip_sized_buffer(struct ccx_demuxer *ctx, struct wtv_chunked_buffer *cb,
					   uint32_t size)
{
	...
	if (cb->skip_chunks[cb->chunk] != -1 &&
		start + size > cb->skip_chunks[cb->chunk])
	{
		buffered_seek(ctx,
					  (int)((cb->skip_chunks[cb->chunk] - start) +
							(WTV_META_CHUNK_SIZE) +
							(size - (cb->skip_chunks[cb->chunk] - start))));
...
// get_sized_buffer will alloc and set a buffer in the passed wtv_chunked_buffer
// struct it will handle any meta data chunks that need to be skipped in the
// file Will print error messages and return a null buffer on error.
void get_sized_buffer(struct ccx_demuxer *ctx, struct wtv_chunked_buffer *cb,
					  uint32_t size)
{
	int64_t result;
	...
	if (cb->skip_chunks[cb->chunk] != -1 &&
		(start + size) > cb->skip_chunks[cb->chunk])
	{
		buffered_read(ctx, cb->buffer,
					  (int)(cb->skip_chunks[cb->chunk] - start));
		cb->filepos += cb->skip_chunks[cb->chunk] - start;
		buffered_seek(ctx, WTV_META_CHUNK_SIZE);
		cb->filepos += WTV_META_CHUNK_SIZE;
		result = buffered_read(
			ctx, cb->buffer + (cb->skip_chunks[cb->chunk] - start),
			(int)(size - (cb->skip_chunks[cb->chunk] - start)));
...
int read_header(struct ccx_demuxer *ctx, struct wtv_chunked_buffer *cb)
{
	...
	ctx->startbytes_avail =
		(int)buffered_read_opt(ctx, ctx->startbytes, STARTBYTESLENGTH);
	...
	while (!end)
	{
		...
		else
		{
			...
			uint32_t text_len;
			memcpy(&text_len, parsebuf,
				   4); // text_len is number of unicode chars, not bytes.
			...
			{
				memcpy(&string[x], parsebuf + 8 + (x * 2),
					   1); // unicode converted to ascii
			}
	...
	// Typically 0x40000
	result = buffered_skip(
		ctx,
		(int)((cb->skip_chunks[cb->chunk] + WTV_META_CHUNK_SIZE) - ctx->past));
			   ...
			   struct demuxer_data *data)
{
	...
	static int alt_stream; // Stream to use for timestamps if the cc stream has
						   // broken timestamps
	...
	while (1)
	{
		...
		pad = len % 8 == 0
				  ? 0
				  : 8 - (len % 8); // Calculate the padding to add to the length
		...
		if (!memcmp(guid, WTV_STREAM2, 16))
		{
			...
			memcpy(&stream_type, cb->buffer + 0xc,
				   16); // Read the stream type GUID
			const void *stream_guid;
			if (ccx_options.wtvmpeg2)
				stream_guid = WTV_STREAM_VIDEO; // We want mpeg2 data if the
												// user set -wtvmpeg2
			else
				stream_guid = WTV_STREAM_MSTVCAPTION; // Otherwise we want the
													  // MSTV captions stream
			if (!memcmp(stream_type, stream_guid, 16))
			{
				video_streams[num_streams] =
					stream_id; // We keep a list of stream ids
		...
		if (!memcmp(guid, WTV_TIMING, 16) &&
			((use_alt_stream < WTV_CC_TIMESTAMP_MAGIC_THRESH &&
			  check_stream_id(stream_id, video_streams, num_streams)) ||
			 (use_alt_stream == WTV_CC_TIMESTAMP_MAGIC_THRESH &&
			  stream_id == alt_stream)))
		{
			...
			{
				use_alt_stream++;
				mprint("WARNING: %i WTV_CC_TIMESTAMP_MAGIC detected in cc "
					   "timestamps. \n",
					   use_alt_stream);
				if (use_alt_stream == WTV_CC_TIMESTAMP_MAGIC_THRESH)
					mprint("WARNING: WTV_CC_TIMESTAMP_MAGIC_THRESH reached. "
						   "Switching to alt timestamps!\n");
			}
			len -= 16;
		}
