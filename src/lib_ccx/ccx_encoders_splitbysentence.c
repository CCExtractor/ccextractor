...
//---------------------------
// BEGIN of #BUG639
// HACK: this is workaround for
// https://github.com/CCExtractor/ccextractor/issues/639 short: the outside
// function, called encode_sub changes ENCODER_CONTEXT when it required.. as
// result SBS loses it internal state...
...
typedef struct
{
	...
	size_t handled_len; /// The length of the string in the SBS-buffer, already
						/// handled, but preserved for DUP-detection.

	// ccx_sbs_utf8_character *sbs_newblock;
	LLONG time_from; // Used by the split-by-sentence code to know when the
...
",
	...
	if (NULL == ____sbs_context)
	{
		...
		____sbs_context->buffer =
			malloc(____sbs_context->capacity * sizeof(unsigned char));
...
char *sbs_find_insert_point_partial(char *old_tail, const char *new_start,
									size_t n, const int maxerr, int *errcount)
{
	...
	dist_r = levenshtein_dist_char(old_tail + len_l, new_start + len_l, len_r,
								   len_r);
...
// ",
	...
	if (dist_r <= few_errors			 // right part almost the same
		&& n > PARTIAL_CHANGE_LENGTH_MIN // the sentence is long enough for
										 // analysis
	)
	{
...
// ",
		...
		partial_shift = 0;
		while (old_tail[partial_shift] != 0 &&
			   old_tail[partial_shift + 1] != 0 &&
			   (0 != sbs_char_equal_CI(old_tail[partial_shift],
									   new_start[partial_shift])))
		{
			partial_shift += 1;
		}

		// LOG_DEBUG("SBS: sbs_find_insert_point_partial: PARTIAL SHIFT,
		// [%d]\n", 	partial_shift
		// );
...
char *sbs_find_insert_point(char *buf, const char *str, int *ilen)
{
	...
	// init errcounter with value, greater than possible amount of errors in
	// string
	best_err = cur_len + 1;
	best_ptr = NULL;
	best_len = 0;
	...
	while (cur_len > 0)
	{
		...
		cur_ptr = sbs_find_insert_point_partial(buffer_tail, prefix, cur_len,
												maxerr, &cur_err);
		if (NULL != cur_ptr)
		{
...
void sbs_strcpy_without_dup(const unsigned char *str, sbs_context_t *context)
{
	...
	LOG_DEBUG(
		"SBS: sbs_strcpy_without_dup: going to append, looking for common part\n\
\tbuffer:          [%p][%s]\n\
\tstring:          [%s]\n\
",
		context->buffer, context->buffer, str);

	buffer_insert_point =
		sbs_find_insert_point(context->buffer, str, &intersect_len);
...
\t handled len     [%4zu]\n\
",
			  context->buffer, str, buffer_insert_point, intersect_len, sbs_len,
			  context->handled_len);

	if (intersect_len > 0)
	{
		// there is a common part (suffix of old sentence equals to prefix of
		// new str)
		//
	...
	if ((sbs_len - intersect_len) >= context->handled_len)
	{
		...
		// It is time to clean the buffer. Excepting the last uncomplete
		// sentence
...
",
	...
	if (!isspace(str[0]) // not a space char in the beginning of new str
		//&& context->handled_len >0  // buffer is not empty (there is
		//uncomplete sentence)
		&& sbs_len > 0 // buffer is not empty (there is uncomplete sentence)
		&& !isspace(context->buffer[sbs_len - 1]) // not a space char at the end
												  // of existing buf
	)
	{
...
/**
 * Appends the function to the sentence buffer, and returns a list of full
 * sentences (if there are any), or NULL
 *
 ...
 * @return           New <struct cc_subtitle *> subtitle, or NULL, if <str>
 * doesn't contain the ending part of the sentence. If there are more than one
 * sentence, the remaining sentences will be chained using <result->next>
 * reference.
 */
struct cc_subtitle *sbs_append_string(unsigned char *str, const LLONG time_from,
									  const LLONG time_trim,
									  sbs_context_t *context)
{
...
\tcur cap:   [%zu]\n\
",
			  context->buffer, context->buffer, context->capacity);
	...
	// ===============================
	required_capacity = strlen(context->buffer) // existing data in buf
...
",
		...
		while (new_capacity < required_capacity)
		{
			...
			new_capacity +=
				(new_capacity > 1048576 * 8) ? 1048576 * 8 : new_capacity;
		}
		...
		if (!context->buffer)
			fatal(EXIT_NOT_ENOUGH_MEMORY,
				  "In sbs_append_string: Not enough memory to append buffer");
...
\tsbs_undone: [%s]\n\
",
			  bp_last_break, context->handled_len, sbs_undone_start);

	for (bp_current = sbs_undone_start; bp_current && *bp_current; bp_current++)
	{
		if (0 < anychar_cur // skip empty!
			&& sbs_is_pointer_on_sentence_breaker(bp_last_break, bp_current))
		{
			...
			tmpsub->nb_data = bp_current - bp_last_break + 1 // terminating '\0'
							  + 1							 // skip '.'
				;
			...
			// tune last break:
			while (*bp_last_break && isspace(*bp_last_break))
			{
				bp_last_break++;
			}
...
\n\tBUFFER:[%s]\
\n",
			  context->handled_len, alphanum_total, anychar_total, str,
			  context->buffer);
										   ...
										   struct encoder_ctx *context)
{
	...
	if (str)
	{
		...
		{
			resub =
				sbs_append_string(str, ms_start, ms_end, sbs_init_context());
		}
		freep(&str);
	}
