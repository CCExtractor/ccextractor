	...
	for (int i = 0; i < data_length; i += 4)
	{
		...
		case 2:
			ccx_common_logging.debug_ftn(
				CCX_DMT_708,
				"[CEA-708] dtvcc_process_data: DTVCC Channel Packet Data\n");
			...
			else
			{
				...
				{
					ccx_common_logging.debug_ftn(
						CCX_DMT_708, "[CEA-708] dtvcc_process_data: "
									 "Warning: Legal packet size exceeded (1), "
									 "data not added.\n");
				}
				else
				{
					dtvcc->current_packet[dtvcc->current_packet_length++] =
						data[i + 2];
					dtvcc->current_packet[dtvcc->current_packet_length++] =
						data[i + 3];
				}
		...
		case 3:
			ccx_common_logging.debug_ftn(
				CCX_DMT_708,
				"[CEA-708] dtvcc_process_data: DTVCC Channel Packet Start\n");
			ccx_dtvcc_process_current_packet(dtvcc);
			if (cc_valid)
			{
				if (dtvcc->current_packet_length >
					CCX_DTVCC_MAX_PACKET_LENGTH - 1)
				{
					ccx_common_logging.debug_ftn(
						CCX_DMT_708, "[CEA-708] dtvcc_process_data: "
									 "Warning: Legal packet size exceeded (2), "
									 "data not added.\n");
				}
				else
				{
...
ccx_dtvcc_ctx *ccx_dtvcc_init(struct ccx_decoder_dtvcc_settings *opts)
{
	...
	memcpy(ctx->services_active, opts->services_enabled,
		   CCX_DTVCC_MAX_SERVICES * sizeof(int));
