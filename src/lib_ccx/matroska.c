...
void skip_bytes(FILE *file, ULLONG n) { FSEEK(file, n, SEEK_CUR); }

void set_bytes(FILE *file, ULLONG n) { FSEEK(file, n, SEEK_SET); }

ULLONG get_current_byte(FILE *file) { return (ULLONG)FTELL(file); }
...
UBYTE mkv_read_byte(FILE *file) { return (UBYTE)fgetc(file); }
...
void parse_ebml(FILE *file)
{
	...
	while (pos + len > get_current_byte(file))
	{
		...
		default:
			...
			{
				mprint(MATROSKA_ERROR "Unknown element 0x%x at position " LLD
									  ", skipping EBML block\n",
					   code, get_current_byte(file) - MATROSKA_MAX_ID_LENGTH);
...
char *generate_timestamp_ass_ssa(ULLONG milliseconds)
{
	...
	sprintf(buf, LLD ":%02" LLD_M ":%02" LLD_M ".%02" LLD_M, hours, minutes,
			seconds, millis);
	return buf;
}
										...
										ULLONG cluster_timecode)
{
	...
	ULLONG pos = get_current_byte(file);
	ULLONG track_number =
		read_vint_length(file); // track number is length, not int
	...
	struct matroska_sub_sentence *sentence =
		malloc(sizeof(struct matroska_sub_sentence));
	ULLONG timestamp = timecode + cluster_timecode;
	sentence->blockaddition = NULL;
	sentence->time_end =
		0; // Initialize time_end so that it is updated if it was not set
	...
	if (strcmp(track->codec_id_string, dvb_codec_id) == 0)
	{
		...
		int ret =
			dvbsub_decode(enc_ctx, dec_ctx, message, size, &mkv_ctx->dec_sub);
		// We use string produced by enc_ctx as a message
		free(message);

		/* Bear in mind that in DVB we are handling the text of the previous
		block. There can be 2 types of DVB in .mkv. One is when each display
		block is followed by empty block in order to allow gaps in time between
		display blocks. Another one is when display block is followed by another
		display block. This code handles both cases but we don't save and use
		empty blocks as sentences, only time_starts of them. */
		...
		if (ret < 0 || dvb_message == NULL)
		{
			...
			if (ret < 0)
				...
				track->last_timestamp =
					timestamp; // We save timestamp because we need to use it
							   // for the next sub as a timestart
			return NULL;
		}
	...
	if (track->sentence_count == 0)
	{
		...
		track->sentences = realloc(track->sentences,
								   (track->sentence_count + 1) *
									   sizeof(struct matroska_sub_sentence *));
	track->sentences[track->sentence_count] = sentence;
	track->sentence_count++;

	mkv_ctx->current_second =
		max(mkv_ctx->current_second, sentence->time_start / 1000);
	activity_progress(
		(int)(get_current_byte(file) * 100 / mkv_ctx->ctx->inputsize),
...
struct matroska_sub_sentence *
parse_segment_cluster_block_group_block_additions(struct matroska_ctx *mkv_ctx,
												  ULLONG cluster_timecode)
{
	FILE *file = mkv_ctx->file;
	ULLONG len = read_vint_length(file);
	...
	struct matroska_sub_sentence *sentence =
		track->sentences[track->sentence_count - 1];
									   ...
									   ULLONG cluster_timecode)
{
	...
	while (pos + len > get_current_byte(file))
	{
		...
		case MATROSKA_SEGMENT_CLUSTER_BLOCK_GROUP_BLOCK:
			new_sentence = parse_segment_cluster_block_group_block(
				mkv_ctx, cluster_timecode);
			if (new_sentence != NULL)
			{
				sentence_list =
					realloc(sentence_list, sizeof(struct matroska_sub_track *) *
											   (sentence_count + 1));
		...
		case MATROSKA_SEGMENT_CLUSTER_BLOCK_GROUP_BLOCK_ADDITIONS:
			parse_segment_cluster_block_group_block_additions(mkv_ctx,
															  cluster_timecode);
			MATROSKA_SWITCH_BREAK(code, code_len);
		...
		default:
			if (code_len == MATROSKA_MAX_ID_LENGTH)
			{
				mprint(MATROSKA_ERROR
					   "Unknown element 0x%x at position " LLD
					   ", skipping segment cluster block group\n",
					   code, get_current_byte(file) - MATROSKA_MAX_ID_LENGTH);
				set_bytes(file, pos + len);
				return;
			}
	...
	{
		// When BlockDuration is not written, the value is assumed to be the
		// difference between the timestamp of this Block and the timestamp of
		// the next Block in "display" order
		if (block_duration == ULONG_MAX)
			...
			sentence_list[i]->time_end =
				sentence_list[i]->time_start + block_duration;
		...
		if (ccx_options.gui_mode_reports)
		{
			...
			while (true)
			{
				...
				if (pos == 0)
				{
					fprintf(stderr, "%02u:%02u#%02u:%02u#", h1 * 60 + m1, s1,
							h2 * 60 + m2, s2);
				}
				else
				{
...
void parse_segment_cluster(struct matroska_ctx *mkv_ctx)
{
	...
	while (pos + len > get_current_byte(file))
	{
		...
		default:
			...
			{
				mprint(MATROSKA_ERROR "Unknown element 0x%x at position " LLD
									  ", skipping segment cluster block\n",
					   code, get_current_byte(file) - MATROSKA_MAX_ID_LENGTH);
				set_bytes(file, pos + len);
				return;
			}
	...
	// We already update activity progress in subtitle block, but we also need
	// to show percents in samples without captions
	activity_progress(
		(int)(get_current_byte(file) * 100 / mkv_ctx->ctx->inputsize),
		(int)(mkv_ctx->current_second / 60),
		(int)(mkv_ctx->current_second % 60));
...
static long bswap32(long v)
{
	...
	long swapped = ((v & 0xFF) << 24) | ((v & 0xFF00) << 8) |
						  ...
						  struct matroska_avc_frame frame)
{
	...
	for (i = 0; i < frame.len;)
	{
		...
		if (nal_length > 0)
			do_NAL(enc_ctx, dec_ctx, (unsigned char *)&(frame.data[i]),
				   nal_length, &mkv_ctx->dec_sub);
		i += nal_length;
	} // outer for
	assert(i == frame.len);
...
{
	for (int i = MATROSKA_TRACK_SUBTITLE_CODEC_ID_UTF8;
		 i <= MATROSKA_TRACK_SUBTITLE_CODEC_ID_KATE; i++)
...
void parse_segment_track_entry(struct matroska_ctx *mkv_ctx)
{
	...
	while (pos + len > get_current_byte(file))
	{
		...
		case MATROSKA_SEGMENT_TRACK_TRACK_TYPE:
			track_type =
				(enum matroska_track_entry_type)read_vint_block_int(file);
			mprint("    Type: %s\n",
		...
		case MATROSKA_SEGMENT_TRACK_CODEC_ID:
			...
			if (strcmp((const char *)codec_id_string,
					   (const char *)avc_codec_id) == 0)
		...
		case MATROSKA_SEGMENT_TRACK_CODEC_PRIVATE:
			...
			if (track_type == MATROSKA_TRACK_TYPE_SUBTITLE &&
				strcmp((const char *)codec_id_string,
					   (const char *)dvb_codec_id) != 0)
				header = read_vint_block_string(file);
			else
				parse_private_codec_data(mkv_ctx, codec_id_string, track_number,
										 lang);
			MATROSKA_SWITCH_BREAK(code, code_len);
		...
		default:
			...
			{
				mprint(MATROSKA_ERROR "Unknown element 0x%x at position " LLD
									  ", skipping segment track entry block\n",
					   code, get_current_byte(file) - MATROSKA_MAX_ID_LENGTH);
				set_bytes(file, pos + len);
				return;
			}
	...
	if (track_type == MATROSKA_TRACK_TYPE_SUBTITLE)
	{
		struct matroska_sub_track *sub_track =
			malloc(sizeof(struct matroska_sub_track));
		...
		for (int i = 0; i < mkv_ctx->sub_tracks_count; i++)
			if (strcmp((const char *)mkv_ctx->sub_tracks[i]->lang,
					   (const char *)lang) == 0)
				sub_track->lang_index++;
		mkv_ctx->sub_tracks =
			realloc(mkv_ctx->sub_tracks, sizeof(struct matroska_sub_track *) *
											 (mkv_ctx->sub_tracks_count + 1));
							  ...
							  char *lang)
{
	...
	if ((strcmp((const char *)codec_id_string, (const char *)avc_codec_id) ==
		 0) &&
		mkv_ctx->avc_track_number == track_number)
	{
		// Skip reserved data
	...
	else if (strcmp((const char *)codec_id_string,
					(const char *)dvb_codec_id) == 0)
	{
								   ...
								   struct matroska_sub_track *track)
{
	...
	if (track->lang_index == 0)
		sprintf(buf, "%s_%s.%s", get_basename(mkv_ctx->filename), track->lang,
				matroska_track_text_subtitle_id_extensions[track->codec_id]);
	else
		sprintf(buf, "%s_%s_" LLD ".%s", get_basename(mkv_ctx->filename),
				track->lang, track->lang_index,
				matroska_track_text_subtitle_id_extensions[track->codec_id]);
	return buf;
}
					...
					struct matroska_sub_track *track)
{
	...
	int desc;
#ifdef WIN32
	desc = open(filename, O_WRONLY | O_CREAT | O_TRUNC | O_APPEND,
				S_IREAD | S_IWRITE);
#else
	desc = open(filename, O_WRONLY | O_CREAT | O_TRUNC | O_APPEND,
				S_IWUSR | S_IRUSR);
	...
	for (int i = 0; i < track->sentence_count; i++)
	{
		...
		if (track->codec_id == MATROSKA_TRACK_SUBTITLE_CODEC_ID_WEBVTT)
		{
			...
			if (blockaddition != NULL)
			{
				...
				{
					write(desc, sentence->blockaddition->comment,
						  sentence->blockaddition->comment_size);
					write(desc, "\n", 1);
				}
			}
			...
			if (blockaddition != NULL)
			{
				...
				{
					write(desc, blockaddition->cue_identifier,
						  blockaddition->cue_identifier_size);
					write(desc, "\n", 1);
				}
			...
			if (i + 1 < track->sentence_count)
				time_end =
					MIN(time_end, track->sentences[i + 1]->time_start - 1);
			...
			if (blockaddition != NULL)
			{
				...
				{
					write(desc, " ", 1);
					write(desc, blockaddition->cue_settings_list,
						  blockaddition->cue_settings_list_size);
				}
			}
			write(desc, "\n", 1);
		...
		else if (track->codec_id == MATROSKA_TRACK_SUBTITLE_CODEC_ID_ASS ||
				 track->codec_id == MATROSKA_TRACK_SUBTITLE_CODEC_ID_SSA)
		{
			char *timestamp_start =
				generate_timestamp_ass_ssa(sentence->time_start);
			ULLONG time_end = sentence->time_end;
			if (i + 1 < track->sentence_count)
				time_end =
					MIN(time_end, track->sentences[i + 1]->time_start - 1);
...
int matroska_loop(struct lib_ccx_ctx *ctx)
{
	...
	{
		mprint(MATROSKA_WARNING
			   "You are using -out=<format>, but Matroska parser extract "
			   "subtitles in a recorded format\n");
