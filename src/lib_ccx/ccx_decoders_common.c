...
uint64_t utc_refvalue =
	UINT64_MAX; /* _UI64_MAX means don't use UNIX, 0 = use current system time
				   as reference, +1 use a specific reference */
extern int in_xds_mode;

/* This function returns a FTS that is guaranteed to be at least 1 ms later than
   the end of the previous screen. It shouldn't be needed obviously but it
   guarantees there's no timing overlap */
...
/* This function returns the current FTS and saves it so it can be used by
 * ctxget_visible_start */
...
int process_cc_data(struct encoder_ctx *enc_ctx, struct lib_cc_decode *dec_ctx,
...
int validate_cc_data_pair(unsigned char *cc_data_pair)
{
	...
	if (cc_type == 0 || cc_type == 1)
	{
		...
		if (!cc608_parity_table[cc_data_pair[1]])
		{
			...
			// blank and process the pair.
			cc_data_pair[1] = 0x7F;
		}
	}
	return 0;
}
int do_cb(struct lib_cc_decode *ctx, unsigned char *cc_block,
		  struct cc_subtitle *sub)
{
	...
	if (ctx->fix_padding && cc_valid == 0 &&
		cc_type <= 1 // Only fix NTSC packets
	...
	if (ctx->write_format != CCX_OF_RAW && // In raw we cannot skip padding
										   // because timing depends on it
		ctx->write_format != CCX_OF_DVDRAW &&
		(cc_block[0] == 0xFA || cc_block[0] == 0xFC || cc_block[0] == 0xFD) &&
		(cc_block[1] & 0x7F) == 0 &&
		(cc_block[2] & 0x7F) ==
			0) // CFS: Skip non-data, makes debugging harder.
		return 1;

	// Print raw data with FTS.
	dbg_print(
		CCX_DMT_CBRAW, "%s   %d   %02X:%c%c:%02X",
		print_mstime_static(ctx->timing->fts_now + ctx->timing->fts_global),
		in_xds_mode, cc_block[0], cc_block[1] & 0x7f, cc_block[2] & 0x7f,
		cc_block[2]);
	...
	if (cc_valid || cc_type == 3)
	{
		...
		case 0:
			...
			if (ctx->extraction_start.set &&
				get_fts(ctx->timing, ctx->current_field) <
					ctx->extraction_start.time_in_ms)
				timeok = 0;
			if (ctx->extraction_end.set &&
				get_fts(ctx->timing, ctx->current_field) >
					ctx->extraction_end.time_in_ms)
			{
				timeok = 0;
		...
		default:
			fatal(CCX_COMMON_EXIT_BUG_BUG,
				  "In do_cb: Impossible value for cc_type, Please file a bug "
				  "report on GitHub.\n");
		} // switch (cc_type)
	}	  // cc_valid
	else
	{
		...
		dbg_print(CCX_DMT_VERBOSE,
				  "Found !(cc_valid || cc_type==3) - ignore this block\n");
	}
...
init_cc_decode(struct ccx_decoders_common_settings_t *setting)
{
	...
	if (setting->codec == CCX_CODEC_ATSC_CC)
	{
		...
		ctx->context_cc608_field_1 = ccx_decoder_608_init_library(
			setting->settings_608, setting->cc_channel, 1,
			&ctx->processed_enough, setting->cc_to_stdout,
			setting->output_format, ctx->timing);
		ctx->context_cc608_field_2 = ccx_decoder_608_init_library(
			setting->settings_608, setting->cc_channel, 2,
			&ctx->processed_enough, setting->cc_to_stdout,
			setting->output_format, ctx->timing);
	}
	else
	...
	memcpy(&ctx->extraction_start, &setting->extraction_start,
		   sizeof(struct ccx_boundary_time));
	memcpy(&ctx->extraction_end, &setting->extraction_end,
		   sizeof(struct ccx_boundary_time));
	...
	ctx->xds_ctx =
		ccx_decoders_xds_init_library(ctx->timing, setting->xds_write_to_file);
...
void flush_cc_decode(struct lib_cc_decode *ctx, struct cc_subtitle *sub)
{
	if (ctx->codec == CCX_CODEC_ATSC_CC)
	{
		if (ctx->extract != 2)
		{
			if (ctx->write_format == CCX_OF_SMPTETT ||
...
struct encoder_ctx *copy_encoder_context(struct encoder_ctx *ctx)
{
	...
	if (ctx->buffer)
	{
		...
		memcpy(ctx_copy->buffer, ctx->buffer,
			   ctx->capacity * sizeof(unsigned char));
	}
	if (ctx->first_input_file)
	{
		ctx_copy->first_input_file =
			malloc(strlen(ctx->first_input_file) * sizeof(char));
		memcpy(ctx_copy->first_input_file, ctx->first_input_file,
			   strlen(ctx->first_input_file) * sizeof(char));
	}
	if (ctx->out)
	{
	...
	if (ctx->timing)
	{
		...
		memcpy(ctx_copy->timing, ctx->timing,
			   sizeof(struct ccx_common_timing_ctx));
	}
	if (ctx->transcript_settings)
	{
		ctx_copy->transcript_settings =
			malloc(sizeof(struct ccx_encoders_transcript_format));
		memcpy(ctx_copy->transcript_settings, ctx->transcript_settings,
			   sizeof(struct ccx_encoders_transcript_format));
	}
	if (ctx->subline)
	{
	...
	{
		ctx_copy->start_credits_text =
			malloc(strlen(ctx->start_credits_text) * sizeof(char));
		memcpy(ctx_copy->start_credits_text, ctx->start_credits_text,
			   (strlen(ctx->start_credits_text) + 1) * sizeof(char));
	}
	if (ctx->end_credits_text)
	{
		ctx_copy->end_credits_text =
			malloc(strlen(ctx->end_credits_text) * sizeof(char));
		memcpy(ctx_copy->end_credits_text, ctx->end_credits_text,
			   (strlen(ctx->end_credits_text) + 1) * sizeof(char));
	}
	return ctx_copy;
}
struct lib_cc_decode *copy_decoder_context(struct lib_cc_decode *ctx)
{
	...
	{
		ctx_copy->context_cc608_field_1 =
			malloc(sizeof(struct ccx_decoder_608_context));
		memcpy(ctx_copy->context_cc608_field_1, ctx->context_cc608_field_1,
			   sizeof(struct ccx_decoder_608_context));
	}
	if (ctx->context_cc608_field_2)
	{
		ctx_copy->context_cc608_field_2 =
			malloc(sizeof(struct ccx_decoder_608_context));
		memcpy(ctx_copy->context_cc608_field_2, ctx->context_cc608_field_2,
			   sizeof(struct ccx_decoder_608_context));
	}
	if (ctx->timing)
	{
	...
	if (ctx->xds_ctx)
	{
		...
		memcpy(ctx_copy->xds_ctx, ctx->xds_ctx,
			   sizeof(struct ccx_decoders_xds_context));
	}
	if (ctx->vbi_decoder)
	{
		ctx_copy->vbi_decoder = malloc(sizeof(struct ccx_decoder_vbi_ctx));
		memcpy(ctx_copy->vbi_decoder, ctx->vbi_decoder,
			   sizeof(struct ccx_decoder_vbi_ctx));
	}
	return ctx_copy;
}
struct cc_subtitle *copy_subtitle(struct cc_subtitle *sub)
{
	...
	if (sub->data)
	{
		...
		memcpy(sub_copy->data, sub->data,
			   sub->nb_data * sizeof(struct eia608_screen));
	}
	return sub_copy;
}
