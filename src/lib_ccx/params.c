...
void print_usage (void)
{
	...
	mprint("              -man:  Generates man page for CCExtractor using help2man\n");
	mprint("                     Dependency: help2man\n");
	mprint("                     Installation(For Ubuntu): sudo apt-get install help2man\n");
}
...
int parse_parameters (struct ccx_s_options *opt, int argc, char *argv[])
{
	...
	for (int i=1; i<argc; i++)
	{
		...
		if(!strcmp(argv[i], "-man"))	//Generate manual page
		{
#ifdef __linux__
			system("bash manscript.sh");
