...
int ps_get_more_data(struct lib_ccx_ctx *ctx, struct demuxer_data **ppdata)
{
	...
	do
	{
		...
		else
		{
			...
			// Search for a header that is not a picture header
			// (nextheader[3]!=0x00)
					 ...
					 nextheader[2] == 0x01 && nextheader[3] != 0x00))
			{
				...
				{
					mprint("\nNot a recognized header. Searching for next "
						   "header.\n");
					dump(CCX_DMT_GENERIC_NOTICES, nextheader, 6, 0, 0);
					// Only print the message once per loop / unrecognized
					// header
					ctx->demux_ctx->strangeheader = 1;
				}

				unsigned char *newheader;
				// The amount of bytes read into nextheader by the buffered_read
				// above
				int hlen = 6;
				// Find first 0x00
				...
				newheader =
					(unsigned char *)memchr(nextheader + 1, 0, hlen - 1);
				if (newheader != NULL)
				{
			...
			else if (nextheader[3] == 0xBD)
			{
				...
				ret = buffered_read(ctx->demux_ctx, nextheader + 6,
									1); // PES header data length (extension)
				...
				// dbg_print(CCX_DMT_VERBOSE, "datalen :%d packetlen :%" PRIu16
				...
				if (nextheader[7] >= 0x20 && nextheader[7] < 0x40)
				{
					dbg_print(CCX_DMT_VERBOSE,
							  "Subtitle found Stream id:%02x\n", nextheader[7]);
					result =
						buffered_read(ctx->demux_ctx, data->buffer, datalen);
			...
			else if (nextheader[3] >= 0xBB && nextheader[3] <= 0xDF)
			{
				...
				dbg_print(CCX_DMT_VERBOSE,
						  "non Video PES (type 0x%2X) - len %u\n",
			...
			else if ((nextheader[3] & 0xf0) == 0xe0)
			{
				...
				int ret;
				size_t peslen;
				ret = read_video_pes_header(ctx->demux_ctx, data, nextheader,
											&hlen, 0);
				if (ret < 0)
				{
					end_of_file = 1;
				...
				// peslen is unsigned since loop would have already broken if it
				// was negative
				int want = (int)((BUFSIZE - data->len) > peslen
									 ? peslen
									 : (BUFSIZE - data->len));
...
int general_get_more_data(struct lib_ccx_ctx *ctx, struct demuxer_data **data)
{
	...
	do
	{
		...
		result = buffered_read(ctx->demux_ctx, ptr->buffer + ptr->len,
							   want); // This is a macro.
...
{
	size_t max =
		(size_t)ctx->inputsize + 1; // Enough for the whole thing. Hex dumps are
									// small so we can be lazy here
	...
	while (fgets(line, max - 1, fr) != NULL)
	{
		...
		*c2 = 0;
		...
		c2++; // Skip blank
		/*
		if (strncmp (c2,mpeg_header,strlen (mpeg_header))) // No idea how to
		deal with this.
		{
			if (!warning_shown)
			{
				warning_shown=1;
				mprint ("\nWarning: This file contains data I can't process:
		Please submit .hex for analysis!\n");
			}
			continue;
		}
		...
		*/
		...
		/*
		if (atoi (line)!=byte_count+strlen (mpeg_header)/3) // Number of bytes
		reported don't match actual contents continue;
			*/
		if (atoi(line) !=
			byte_count) // Number of bytes reported don't match actual contents
			continue;
		if (!byte_count) // Nothing to get from this line except timing info,
						 // already done.
			continue;
		bytes = (unsigned char *)malloc(byte_count);
		if (!bytes)
			fatal(EXIT_NOT_ENOUGH_MEMORY, "In process_hex: Out of memory to "
										  "store processed hex value.\n");
		...
		for (unsigned i = 0; i < byte_count; i++)
		{
			...
			if (value == -1)
				fatal(EXIT_FAILURE, "In process_hex: Incorrect format, "
									"unexpected non-hex string.");
			bytes[i] = value;
			c2 += 3;
		}
		...
		// Always 03 after header, then something unknown (seen 42, 43, c2,
		// c3...), then ff, then data with field info, and terminated with ff.
		if (byte_count > 3 && bytes[0] == 0x03 && bytes[2] == 0xff &&
			bytes[byte_count - 1] == 0xff)
		{
			ok = 1;
		...
		else
		{
			...
			int always_ff = 1;
			int current_field = 0;
			if (filler == 0 && caption_count * 6 ==
								   byte_count - 1) // Note that we are ignoring
												   // the extra field for now...
			{
				ok = 1;
				for (unsigned i = 1; i < byte_count - 2; i += 3)
					if (bytes[i] != 0xff)
					{
						// If we only find FF in the first byte then either
						// there's only field 1 data, OR there's alternating
						// field 1 and field 2 data. Don't know how to tell
						// apart. For now let's assume that always FF means
						// alternating.
						always_ff = 0;
						break;
					}
				...
				{
					inbuf = 3;
					if (always_ff) // Try to tell apart the fields based on the
								   // pattern field.
					{
						ctx->buffer[0] =
							current_field | 4; // | 4 to enable the 'valid' bit
		...
		{
			warning_shown = 1;
			mprint("\nWarning: This file contains data I can't process: Please "
				   "submit .hex for analysis!\n");
		}
		free(bytes);
	}
	fclose(fr);
...
int raw_loop(struct lib_ccx_ctx *ctx)
{
	...
	do
	{
		...
		add_current_pts(dec_ctx->timing,
						cb_field1 * 1001 / 30 * (MPEG_CLOCK_FREQ / 1000));
		set_fts(dec_ctx->timing); // Now set the FTS related variables including
								  // fts_max
...
/* Process inbuf bytes in buffer holding raw caption data (three byte packets,
 * the first being the field). The number of processed bytes is returned. */
size_t process_raw_with_field(struct lib_cc_decode *dec_ctx,
							  struct cc_subtitle *sub, unsigned char *buffer,
							  size_t len)
{
	...
	for (i = 0; i < len; i = i + 3)
	{
		if (!dec_ctx->saw_caption_block && *(buffer + i) == 0xff &&
			*(buffer + i + 1) == 0xff)
		{
				   ...
				   unsigned char *buffer, size_t len)
{
	...
	for (i = 0; i < len; i = i + 2)
	{
		if (!ctx->saw_caption_block && *(buffer + i) == 0xff &&
			*(buffer + i + 1) == 0xff)
		{
			// Skip broadcast header
		}
...
int process_data(struct encoder_ctx *enc_ctx, struct lib_cc_decode *dec_ctx,
				 struct demuxer_data *data_node)
{
	...
	{
		got = process_raw_with_field(dec_ctx, dec_sub, data_node->buffer,
									 data_node->len);
	...
	{
		ret = dvbsub_decode(enc_ctx, dec_ctx, data_node->buffer + 2,
							data_node->len - 2, dec_sub);
		if (ret < 0)
	...
	else if (data_node->bufferdatatype == CCX_PES)
	{
		...
		got = process_m2v(enc_ctx, dec_ctx, data_node->buffer, data_node->len,
						  dec_sub);
	}
	...
	{
		// telxcc_update_gt(dec_ctx->private_data,
		// ctx->demux_ctx->global_timestamp);
		if (enc_ctx)
		{
			ret = tlt_process_pes_packet(dec_ctx, data_node->buffer,
										 data_node->len, dec_sub,
										 enc_ctx->sentence_cap);
			if (ret == CCX_EINVAL)
				return ret;
		}
		got = data_node->len;
	...
	else if (data_node->bufferdatatype ==
			 CCX_RAW) // Raw two byte 608 data from DVR-MS/ASF
	{
		// The asf_get_more_data() loop sets current_pts when possible
		if (dec_ctx->timing->pts_set == 0)
		{
			mprint(
				"DVR-MS/ASF file without useful time stamps - count blocks.\n");
		...
		if (dec_ctx->timing->current_pts != last_pts)
		{
			...
			if (dec_ctx->timing->min_pts == 0x01FFFFFFFFLL)
			{
				...
				fts_at_gop_start =
					get_fts(dec_ctx->timing, dec_ctx->current_field);
		...
		dbg_print(CCX_DMT_VIDES, "PTS: %s (%8u)",
				  print_mstime_static(dec_ctx->timing->current_pts /
	...
	else if (data_node->bufferdatatype == CCX_H264) // H.264 data from TS file
	{
		...
		got = process_avc(enc_ctx, dec_ctx, data_node->buffer, data_node->len,
						  dec_sub);
	}
	...
	else if (data_node->bufferdatatype == CCX_ISDB_SUBTITLE)
	{
		...
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In process_data: datanode->buffer is of unknown data type!");
	...
	//	ctx->demux_ctx->write_es(ctx->demux_ctx, data_node->buffer, (size_t)
	//(data_node->len - got));
	...
	if (data_node)
	{
		if (got > 0)
		{
			memmove(data_node->buffer, data_node->buffer + got,
					(size_t)(data_node->len - got));
			data_node->len -= got;
		}
	}
...
void segment_output_file(struct lib_ccx_ctx *ctx, struct lib_cc_decode *dec_ctx)
{
	...
	if (cur_sec / ctx->out_interval > ctx->segment_counter)
	{
		...
		if (!ctx->segment_on_key_frames_only ||
			dec_ctx->picture_coding_type ==
				CCX_FRAME_TYPE_I_FRAME) // Segment only when an I-Frame is found
		{
			segment_now = 1;
		...
		{
			if (enc_ctx->segment_pending) // We already knew we had to segment
										  // but can we do it?
			{
	...
	{
		{
			...
			if (enc_ctx)
			{
				...
				char *extension =
					get_file_extension(ccx_options.enc_cfg.write_format);
				sprintf(ccx_options.enc_cfg.output_filename, "%s_%06d%s",
						ctx->basefilename, ctx->segment_counter + 1, extension);
...
int general_loop(struct lib_ccx_ctx *ctx)
{
	...
	default:
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In general_loop: Impossible value for stream_mode");
	}

	end_of_file = 0;
	while (!terminate_asap && !end_of_file &&
		   is_decoder_processed_enough(ctx) == CCX_FALSE)
	{
		...
		if (!ctx->multiprogram)
		{
			...
			// Find most promising stream: teletext, DVB, ISDB, ATSC, in that
			// order
			...
			if (ccx_options.analyze_video_stream)
			{
				...
				{
					struct cap_info *cinfo_video = get_cinfo(
						ctx->demux_ctx,
						pid); // Must be pid, not video_pid or DVB crashes
							  // (possibly buffer consumption?) - TODO
					struct lib_cc_decode *dec_ctx_video =
						update_decoder_list_cinfo(ctx, cinfo_video);
					enc_ctx = update_encoder_list_cinfo(ctx, cinfo_video);
					struct cc_subtitle *dec_sub_video = &dec_ctx_video->dec_sub;
					struct demuxer_data *data_node_video =
						get_data_stream(datalist, video_pid);
					if (data_node_video)
					{
						if (data_node_video->pts != CCX_NOPTS)
						{
							...
							LLONG pts;
							if (data_node_video->tb.num != 1 ||
								data_node_video->tb.den != MPEG_CLOCK_FREQ)
							{
								pts = change_timebase(data_node_video->pts,
													  data_node_video->tb, tb);
							}
							else
						...
						size_t got = process_m2v(
							enc_ctx, dec_ctx_video, data_node_video->buffer,
							data_node_video->len, dec_sub_video);
						if (got > 0)
						{
							memmove(data_node_video->buffer,
									data_node_video->buffer + got,
									(size_t)(data_node_video->len - got));
			...
			dec_ctx->dtvcc->encoder =
				(void *)enc_ctx; // WARN: otherwise cea-708 will not work

			if (dec_ctx->timing->min_pts ==
				0x01FFFFFFFFLL) // if we didn't set the min_pts of the program
			{
				int p_index = 0; // program index
				for (int i = 0; i < ctx->demux_ctx->nb_program; i++)
				{
					if (dec_ctx->program_number ==
				...
				if (dec_ctx->codec ==
					CCX_CODEC_TELETEXT) // even if there's no sub data, we still
										// need to set the min_pts
				{
					if (ctx->demux_ctx->pinfo[p_index]
							.got_important_streams_min_pts[PRIVATE_STREAM_1] !=
						UINT64_MAX) // Teletext is synced with subtitle packet
									// PTS
					{
						min_pts = ctx->demux_ctx->pinfo[p_index]
				...
				if (dec_ctx->codec ==
					CCX_CODEC_DVB) // DVB will always have to be in sync with
								   // audio (no matter the min_pts of the other
								   // streams)
				{
					if (ctx->demux_ctx->pinfo[p_index]
							.got_important_streams_min_pts[AUDIO] !=
						UINT64_MAX) // it means we got the first pts for audio
					{
						min_pts = ctx->demux_ctx->pinfo[p_index]
									  .got_important_streams_min_pts[AUDIO];
						set_current_pts(dec_ctx->timing, min_pts);
			...
			if (data_node->pts != CCX_NOPTS)
			{
				struct ccx_rational tb = {1, MPEG_CLOCK_FREQ};
				LLONG pts;
				if (data_node->tb.num != 1 ||
					data_node->tb.den != MPEG_CLOCK_FREQ)
				{
			...
			if (data_node->bufferdatatype == CCX_ISDB_SUBTITLE)
			{
				...
				{
					tstamp = (ctx->demux_ctx->global_timestamp +
							  ctx->demux_ctx->offset_global_timestamp) -
							 ctx->demux_ctx->min_global_timestamp;
				}
				else
				{
			...
			if (data_node->bufferdatatype == CCX_TELETEXT &&
				dec_ctx->private_data) // if we have teletext subs, we set the
									   // min_pts here
				set_tlt_delta(dec_ctx, min_pts);
			ret = process_data(enc_ctx, dec_ctx, data_node);
			if (enc_ctx != NULL)
			{
				if (enc_ctx->srt_counter || enc_ctx->cea_708_counter ||
					dec_ctx->saw_caption_block || ret == 1)
					caps = 1;
			}

			// Process the last subtitle for DVB
			if (!(!terminate_asap && !end_of_file &&
				  is_decoder_processed_enough(ctx) == CCX_FALSE))
			{
				if (data_node->bufferdatatype == CCX_DVB_SUBTITLE &&
					dec_ctx->dec_sub.prev->end_time == 0)
				{
					dec_ctx->dec_sub.prev->end_time =
						(dec_ctx->timing->current_pts -
						 dec_ctx->timing->min_pts) /
						(MPEG_CLOCK_FREQ / 1000);
					if (enc_ctx != NULL)
		...
		else
		{
			...
			struct encoder_ctx *enc_ctx = NULL;
			list_for_each_entry(program_iter, &ptr->pg_stream, pg_stream,
								struct cap_info)
			{
				...
				if (dec_ctx->timing->min_pts ==
					0x01FFFFFFFFLL) // if we didn't set the min_pts of the
									// program
				{
					int p_index = 0; // program index
					for (int i = 0; i < ctx->demux_ctx->nb_program; i++)
					...
					if (dec_ctx->codec ==
						CCX_CODEC_TELETEXT) // even if there's no sub data, we
											// still need to set the min_pts
					{
						if (ctx->demux_ctx->pinfo[p_index]
								.got_important_streams_min_pts
									[PRIVATE_STREAM_1] !=
							UINT64_MAX) // Teletext is synced with subtitle
										// packet PTS
						{
							min_pts =
								ctx->demux_ctx->pinfo[p_index]
									.got_important_streams_min_pts
										[PRIVATE_STREAM_1]; // it means we got
															// the first pts for
															// private stream 1
							set_current_pts(dec_ctx->timing, min_pts);
							set_fts(dec_ctx->timing);
					...
					if (dec_ctx->codec ==
						CCX_CODEC_DVB) // DVB will always have to be in sync
									   // with audio (no matter the min_pts of
									   // the other streams)
					{
						if (ctx->demux_ctx->pinfo[p_index]
								.got_important_streams_min_pts[AUDIO] !=
							UINT64_MAX) // it means we got the first pts for
										// audio
						{
							min_pts = ctx->demux_ctx->pinfo[p_index]
										  .got_important_streams_min_pts[AUDIO];
				...
				if (enc_ctx != NULL)
				{
					if ((enc_ctx && (enc_ctx->srt_counter ||
		...
		{
			int cur_sec =
				(int)(get_fts(dec_ctx->timing, dec_ctx->current_field) / 1000);
			int th = cur_sec / 10;
		...
		{
			if (ctx->total_inputsize >
				255) // Less than 255 leads to division by zero below.
			{
				int progress =
					(int)((((ctx->total_past + ctx->demux_ctx->past) >> 8) *
						   100) /
		...
		// void segment_output_file(struct lib_ccx_ctx *ctx, struct
		// lib_cc_decode *dec_ctx);
	...
	list_for_each_entry(dec_ctx, &ctx->dec_ctx_head, list, struct lib_cc_decode)
	{
		...
		mprint("Number of num_unexpected_sei_length: %ld",
			   dec_ctx->avc_ctx->num_unexpected_sei_length);
		free(dec_ctx->xds_ctx);
	}

	delete_datalist(datalist);
	if (ctx->total_past != ctx->total_inputsize && ctx->binary_concat &&
		is_decoder_processed_enough(ctx))
	{
		mprint("\n\n\n\nATTENTION!!!!!!\n");
		mprint("Processing of %s %d ended prematurely %lld < %lld, please send "
			   "bug report.\n\n",
			   ctx->inputfile[ctx->current_file], ctx->current_file,
			   ctx->demux_ctx->past, ctx->inputsize);
	}
	return caps;
}
...
int rcwt_loop(struct lib_ccx_ctx *ctx)
{
	...
	if (BUFSIZE < (3 * 0xFFFF + 10))
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In rcwt_loop: BUFSIZE too small for RCWT caption block.\n");
	...
	if (!memcmp(parsebuf, "\xCC\xCC\xED", 3))
	{
		...
		dbg_print(CCX_DMT_PARSE,
				  ...
				  "revision: %02X%02X\n",
				  parsebuf[3], parsebuf[4], parsebuf[5], parsebuf[6],
				  parsebuf[7]);
	}
	else
	{
		fatal(EXIT_MISSING_RCWT_HEADER,
			  "In rcwt_loop: Missing RCWT header. Abort.\n");
	}
			...
			if (cbcount * 3 > parsebufsize)
			{
				...
				if (!parsebuf)
					fatal(EXIT_NOT_ENOUGH_MEMORY,
						  "In rcwt_loop: Out of memory allocating parsebuf.");
