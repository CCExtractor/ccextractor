...
#define debug(fmt, ...)                                                        \
	ccx_common_logging.debug_ftn(CCX_DMT_PARSE, "GXF:%s:%d: " fmt,             \
								 __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define log(fmt, ...)                                                          \
	ccx_common_logging.log_ftn("GXF:%d: " fmt, __LINE__, ##__VA_ARGS__)
...
typedef enum
{
	...
	/* A video track encoded using JPEG (ITU-R T.81 or ISO/IEC 10918-1) for 625
	   line material */
	...
	/**
	 * A video track encoded using SMPTE 314M or ISO/IEC 61834-2 DV encoding at
	 * 25 Mb/s for 625/50i.
	 */
 ...
 */
static int parse_packet_header(struct ccx_demuxer *ctx, GXFPktType *type,
							   int *length)
{
...
static int parse_material_sec(struct ccx_demuxer *demux, int len)
{
	...
	while (len > 2)
	{
		...
		{
		case MAT_NAME:
			result = buffered_read(demux, (unsigned char *)ctx->media_name,
								   MIN(tag_len, STR_LEN));
			demux->past += tag_len;
...
static void set_track_frame_rate(struct ccx_gxf_video_track *vid_track,
								 int8_t val)
{
	switch (val)
	{
...
static int parse_mpeg525_track_desc(struct ccx_demuxer *demux, int len)
{
	...
	while (len > 2)
	{
		...
		case TRACK_NAME:
			result =
				buffered_read(demux, (unsigned char *)vid_track->track_name,
							  MIN(tag_len, STR_LEN));
			demux->past += tag_len;
			if (result != MIN(tag_len, STR_LEN))
			{
...
static int parse_ad_track_desc(struct ccx_demuxer *demux, int len)
{
	...
	while (len > 2)
	{
		...
		{
		case TRACK_NAME:
			result = buffered_read(demux, (unsigned char *)ad_track->track_name,
								   MIN(tag_len, STR_LEN));
			demux->past += tag_len;
			if (result != MIN(tag_len, STR_LEN))
			{
		...
		case TRACK_AUX:
			...
			ad_track->field_size =
				*((int16_t *)(auxi_info + 4)); // RB16(auxi_info + 4);
			ad_track->packet_size =
				*((int16_t *)(auxi_info + 6)) * 256; // RB16(auxi_info + 6);
			debug("ad_format %d nb_field %d field_size %d packet_size %d track "
				  "id %d\n",
				  ad_track->ad_format, ad_track->nb_field, ad_track->field_size,
				  ad_track->packet_size, ad_track->id);
			break;
		case TRACK_VER:
...
static int parse_track_sec(struct ccx_demuxer *demux, int len,
						   struct demuxer_data *data)
{
	...
	while (len > 4)
	{
		...
		if (track_type == TRACK_TYPE_ANCILLARY_DATA)
		{
			if (!ctx->ad_track)
				ctx->ad_track =
					malloc(sizeof(struct ccx_gxf_ancillary_data_track));
...
int parse_ad_cdp(unsigned char *cdp, size_t len, struct demuxer_data *data)
{
	...
	{
		log(" Please share sample, newer version of SMPTE-334 specification "
			"are followed\n"
			"New section id 0x%x\n",
			*cdp);
		cdp++;
	...
	if (*cdp == 0x74)
	{
		 ...
		 * packet_checksum – This 8-bit field shall contain the 8-bit value
		 * necessary to make the arithmetic sum of the entire packet (first byte
		 * of cdp_identifier to packet_checksum, inclusive) modulo 256 equal
		 * zero.
		 */
	}
 ...
 */
static int parse_ad_pyld(struct ccx_demuxer *demux, int len,
						 struct demuxer_data *data)
{
	int ret = CCX_OK;
	int result = 0;
	...
	if (((d_id & 0xff) == CLOSED_CAP_DID) &&
		((sd_id & 0xff) == CLOSED_C708_SDID))
	{
	...
	else if (((d_id & 0xff) == CLOSED_CAP_DID) &&
			 ((sd_id & 0xff) == CLOSED_C608_SDID))
	{
		log("Need Sample\n");
	}
	else
 ...
 * of parsing vbi
 */
static int parse_ad_vbi(struct ccx_demuxer *demux, int len,
						struct demuxer_data *data)
{
	int ret = CCX_OK;
	int result = 0;
...
static int parse_ad_field(struct ccx_demuxer *demux, int len,
						  struct demuxer_data *data)
{
	int ret = CCX_OK;
	int result = 0;
	...
	while (len > 28)
	{
		...
		/**
		 * IN GXF video there are 2 pad but if I ignore first pad tag then there
		 * is data inside it There must be data that time but it was not there
		 * or it may be the consequence of pad tag inserted later and data is
		 * still present
		 */
...
static int parse_mpeg_packet(struct ccx_demuxer *demux, int len,
							 struct demuxer_data *data)
{
	int ret = CCX_OK;
	int result = 0;
 ...
 */
static int parse_ad_packet(struct ccx_demuxer *demux, int len,
						   struct demuxer_data *data)
{
	int ret = CCX_OK;
	int result = 0;
	...
	len -= 4;
	/**
	 * The number of fields in the ancillary data field section vector. Shall be
	 * 10 for high definition ancillary data and 14 for standard definition
	 * ancillary data.
	 */
	...
	len -= 4;
	/**
	 * The length of the ancillary data field descriptions, ancillary data
	 * sample payloads, and the associated padding. This field shall be set to
	 * 6340 for high definition video formats and to 4676 for standard
 ...
 *    +-----------------------------+-----------------------------+
 */
static void set_mpeg_frame_desc(struct ccx_gxf_video_track *vid_track,
								unsigned char mpeg_frame_desc_flag)
{
	vid_track->p_code = mpeg_frame_desc_flag & 0x03;
	vid_track->p_struct = (mpeg_frame_desc_flag >> 2) & 0x03;
}
static int parse_media(struct ccx_demuxer *demux, int len,
					   struct demuxer_data *data)
{
	int ret = CCX_OK;
	int result = 0;
	...
	// struct ccx_gxf_video_track *vid_track;
	/**
	 * The track number is a reference into the track description section of the
	 * map packets. It identifies the media file to which the current media
	 * packet belongs. Track descriptions shall be considered as consecutive
	 * elements of a vector and track numbers are the index into that vector.
	 * Clips shall have 1 to 48 tracks in any combination of types. Tracks shall
	 * be numbered from 0 to n–1. Media packets shall be inserted in the stream
	 * starting with track n-1 and proceeding to track 0. Time code packets
	 * shall have the greatest track numbers. Ancillary data packets shall have
	 * track numbers less than time code packets. Audio packets shall have track
	 * numbers less than ancillary data packets. Video packet track numbers
	 * shall be less than audio track numbers.
	 */
	unsigned char track_nb;
	unsigned int media_field_nb;

	/**
	 * For ancillary data, the field information contains the first valid
	 * ancillary data field number (inclusive) and the last valid ancillary data
	 * field number (exclusive). The first and last valid ancillary data field
	 * numbers apply to the current packet only. If the entire ancillary data
	 * packet is valid, the first and last valid ancillary data field numbers
	 * shall be 0 and 10 for high definition ancillary data, and 0 and 14 for
	 * standard definition ancillary data. These values shall be sent starting
	 * with the ancillary data field from the lowest number video line
	 * continuing to higher video line numbers. Within each line the ancillary
	 * data fields shall not be reordered.
	 */
 ...
 */
static int parse_map(struct ccx_demuxer *ctx, int len,
					 struct demuxer_data *data)
{
	int result = 0;
 ...
 *            Stream Starts with Map packet which is known by looking at offset
 * 0x05 of packet header.
 *            TODO Map packet are sent per 100 packets so search MAP packet,
 * there might be no MAP header at start if GXF is sliced at unknown region
 */
