 ...
 *
 * @param ctx ccx_demuxer context properly initilaized ccx_demuxer with some
 * input Not to be NULL, since ctx is derefrenced inside this function
 *
 * @param buffer if buffer then it must be allocated to at;east bytes len as
 *               passed in third argument, If buffer is NULL then those number
 * of bytes are skipped from input.
 * @param bytes number of bytes to be read from file buffer.
 *
 * @return 0 or number of bytes, if returned 0 then op should check error number
 * to know details of error
 */
 ...
 *
 */
static size_t inline buffered_read(struct ccx_demuxer *ctx,
...
/**
 * Read single byte from file buffer and if needed also read file for number of
 * bytes.
 *
 */
static size_t inline buffered_read_byte(struct ccx_demuxer *ctx,
