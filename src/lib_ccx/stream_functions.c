...
void detect_stream_type(struct ccx_demuxer *ctx)
{
	...
	if (ctx->startbytes_avail >= 4)
	{
		...
		if (ctx->startbytes[0] == 0x30 && ctx->startbytes[1] == 0x26 &&
		...
		ctx->startbytes_avail >= 4)
	{
		...
		if (ctx->startbytes[0] == 0x1a && ctx->startbytes[1] == 0x45 &&
			ctx->startbytes[2] == 0xdf && ctx->startbytes[3] == 0xa3)
			ctx->stream_mode = CCX_SM_MKV;
		// Check for Matroska magic bytes - Segment
		if (ctx->startbytes[0] == 0x18 && ctx->startbytes[1] == 0x53 &&
			ctx->startbytes[2] == 0x80 && ctx->startbytes[3] == 0x67)
			ctx->stream_mode = CCX_SM_MKV;
	}
	if (ctx->stream_mode == CCX_SM_ELEMENTARY_OR_NOT_FOUND)
	{
	...
	if (ctx->stream_mode == CCX_SM_ELEMENTARY_OR_NOT_FOUND &&
		ctx->startbytes_avail >= 4)
	{
		if (ctx->startbytes[0] == 0xb7 && ctx->startbytes[1] == 0xd8 &&
			ctx->startbytes[2] == 0x00 && ctx->startbytes[3] == 0x20)
			ctx->stream_mode = CCX_SM_WTV;
	}
#ifdef WTV_DEBUG
	if (ctx->stream_mode == CCX_SM_ELEMENTARY_OR_NOT_FOUND &&
		ctx->startbytes_avail >= 6)
	{
		// Check for hexadecimal dump generated by wtvccdump
		// ; CCHD
		if (ctx->startbytes[0] == ';' && ctx->startbytes[1] == ' ' &&
			ctx->startbytes[2] == 'C' && ctx->startbytes[3] == 'C' &&
			ctx->startbytes[4] == 'H' && ctx->startbytes[5] == 'D')
		...
		ctx->startbytes_avail >= 11)
	{
		...
		if (ctx->startbytes[0] == 0xCC && ctx->startbytes[1] == 0xCC &&
			ctx->startbytes[2] == 0xED && ctx->startbytes[8] == 0 &&
		...
		ctx->startbytes_avail >= 4) // Still not found
	{
		...
		while (idx < ctx->startbytes_avail - 8)
		{
			...
			if (isValidMP4Box(ctx->startbytes, idx, &nextBoxLocation,
							  &boxScore))
			{
				idx = nextBoxLocation; // If the box is valid, a new box should
									   // be found on the next location... Not
			...
			{
				// Not a valid box, reset score. We need a couple of successive
				// boxes to identify a MP4 file.
				boxScore = 0;
				idx++;
			}
		}
		if (boxScore > 1)
		{
			// We had at least one box (or multiple) at the end to "claim" this
			// is MP4. A single valid box at the end is doubtful...
			ctx->stream_mode = CCX_SM_MP4;
		}
	}

	if (ctx->stream_mode ==
		CCX_SM_ELEMENTARY_OR_NOT_FOUND) // Search for MXF header
	{
		...
		if (ctx->startbytes_avail > 188 * 8) // Otherwise, assume no TS
		{
			...
			if (ctx->stream_mode == CCX_SM_TRANSPORT)
			{
				dbg_print(CCX_DMT_PARSE,
						  "detect_stream_type: detected as TS\n");
				return_to_buffer(ctx, ctx->startbytes,
								 (unsigned int)ctx->startbytes_avail);
				return;
			}
			...
			{
				dbg_print(CCX_DMT_PARSE,
						  "detect_stream_type: detected as M2TS\n");
				return_to_buffer(ctx, ctx->startbytes,
								 (unsigned int)ctx->startbytes_avail);
				return;
			}
			...
			for (unsigned i = 0; i < (unsigned)(ctx->startbytes_avail < 50000
			...
			{
				dbg_print(CCX_DMT_PARSE,
						  "detect_stream_type: detected as PS\n");
			}
			...
			{
				// The TiVo header is longer, but the PS loop will find the
				// beginning
				dbg_print(CCX_DMT_PARSE,
						  "detect_stream_type: detected as Tivo PS\n");
				ctx->startbytes_pos = 187;
				ctx->stream_mode = CCX_SM_PROGRAM;
				ctx->strangeheader =
					1; // Avoid message about unrecognized header
			}
		}
		else
		{
						  ...
						  int sbuflen)
{
	...
	static LLONG current_pts_33 =
		0; // Last PTS from the header, without rollover bits
	...
	{
		if (data->bufferdatatype == CCX_DVB_SUBTITLE && peslen == 1 &&
			nextheader[6] == 0xFF)
		{
	...
	*headerlength += (int)nextheader[8];
	...
	else if ((nextheader[7] & 0xC0) == 0xC0)
	{
		...
		if ((nextheader[9] & 0xF1) != 0x31 || (nextheader[11] & 0x01) != 0x01 ||
			...
			(nextheader[16] & 0x01) != 0x01 || (nextheader[18] & 0x01) != 0x01)
		{
			falsepes = 1;
	...
	{ // ESCR
		if ((nextheader[9 + hskip] & 0xC4) != 0x04 ||
			!(nextheader[11 + hskip] & 0x04) ||
			!(nextheader[13 + hskip] & 0x04) ||
			!(nextheader[14 + hskip] & 0x01))
		{
			falsepes = 1;
			mprint("False ESCR\n");
		}
	...
	if (!falsepes && nextheader[7] & 0x80)
	{
		...
		LLONG bits_9 = ((LLONG)nextheader[9] & 0x0E)
					   << 29; // PTS 32..30 - Must be LLONG to prevent overflow
		...
		{
			if (!bits_9 &&
				((current_pts_33 >> 30) & 7) == 7) // PTS about to rollover
				data->rollover_bits++;
			if ((bits_9 >> 30) == 7 &&
				((current_pts_33 >> 30) & 7) ==
					0) // PTS rollback? Rare and if happens it would mean out of
					   // order frames
				data->rollover_bits--;
		}
	...
	dbg_print(CCX_DMT_VERBOSE,
			  "PES header length: %d (%d verified)  Data length: %d\n",
...
/*
 * Structure for a MP4 box type. Contains the name of the box type and the score
 * we want to assign when a given box type is found.
 */
typedef struct ccx_stream_mp4_box
{
	char boxType[5]; // Name of the box structure
	int score; // The weight of how heavy a box structure should be counted in
			   // order to "recognize" an MP4 file.
} ccx_stream_mp4_box;

/* The box types below are taken from table 1 from ISO/IEC 14496-12_2012 (page
 * 12 and further). An asterisk (*) marks a mandatory box for a regular file.
...
ccx_stream_mp4_box ccx_stream_mp4_boxes[16] = {
	...
	{"meta", 1}, // Metadata
	{"wide",
	 1}, // For boxes that are > 2^32 bytes
...
/*
 * Checks if the buffer, beginning at the given position, contains a valid MP4
 * box (defined in the ccx_stream_mp4_boxes array). If it does contain one, it
 * will set the nextBoxLocation to position + current box size (this should be
 * the place in the buffer where the next box should be in case of an mp4) and
 * increment the box score with the defined score for the found box.
 *
				  ...
				  size_t *nextBoxLocation, int *boxScore)
{
	...
	{
		if (buffer[position + 4] == ccx_stream_mp4_boxes[idx].boxType[0] &&
			buffer[position + 5] == ccx_stream_mp4_boxes[idx].boxType[1] &&
			buffer[position + 6] == ccx_stream_mp4_boxes[idx].boxType[2] &&
			buffer[position + 7] == ccx_stream_mp4_boxes[idx].boxType[3])
		{
			mprint("Detected MP4 box with name: %s\n",
				   ccx_stream_mp4_boxes[idx].boxType);
			// Box name matches. Do crude validation of possible box size, and
			// if valid, add points for "valid" box
			...
			*boxScore += ccx_stream_mp4_boxes[idx].score;
			...
			{
				// If 0, runs to end of file. If 1, next field contains int_64
				// containing size, which is definitely bigger than the cache.
				// Both times we can skip further checks by setting
				// nextBoxLocation to the max possible.
				*nextBoxLocation = UINT32_MAX;
			}
			else
			{
				// Set nextBoxLocation to current position + boxSize, as that
				// will be the location of the next box.
