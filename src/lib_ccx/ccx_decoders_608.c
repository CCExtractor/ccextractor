...
static const int rowdata[] = {11, -1, 1, 2, 3, 4, 12, 13,
							  14, 15, 5, 6, 7, 8, 9,  10};
...
static const char *command_type[] = {"Unknown",
...
static const char *font_text[] = {"regular", "italics", "underlined",
								  "underlined italics"};
...
const char *color_text[MAX_COLOR][2] = {{"white", ""},
...
void clear_eia608_cc_buffer(ccx_decoder_608_context *context,
							struct eia608_screen *data)
{
	for (int i = 0; i < 15; i++)
	{
		...
		memset(data->colors[i], context->settings->default_color,
			   CCX_DECODER_608_SCREEN_WIDTH + 1);
...
ccx_decoder_608_context *ccx_decoder_608_init_library(
	struct ccx_decoder_608_settings *settings, int channel, int field,
	int *halt, int cc_to_stdout, enum ccx_output_format output_format,
...
struct eia608_screen *get_writing_buffer(ccx_decoder_608_context *context)
{
	...
	case MODE_PAINTON:
	case MODE_TEXT:
		// TODO: Fix this. Text uses a different buffer, and contains
	...
	default:
		ccx_common_logging.fatal_ftn(CCX_COMMON_EXIT_BUG_BUG,
									 "Caption mode has an illegal value at "
									 "get_writing_buffer(), this is a bug.\n");
	}
	return use_buffer;
}
...
void delete_to_end_of_row(ccx_decoder_608_context *context)
{
	if (context->mode != MODE_TEXT)
	{
		...
		for (int i = context->cursor_column;
			 i <= CCX_DECODER_608_SCREEN_WIDTH - 1; i++)
		{
			...
			use_buffer->colors[context->cursor_row][i] =
				context->settings->default_color;
...
void write_char(const unsigned char c, ccx_decoder_608_context *context)
{
	if (context->mode != MODE_TEXT)
	{
		...
		use_buffer->colors[context->cursor_row][context->cursor_column] =
			context->current_color;
		use_buffer->fonts[context->cursor_row][context->cursor_column] =
			context->font;
		...
		if (use_buffer->empty)
		{
			if (MODE_POPON != context->mode)
				context->current_visible_start_ms =
					get_visible_start(context->timing, context->my_field);
		}
		use_buffer->empty = 0;
		...
		if (context->ts_start_of_current_line == -1)
			context->ts_start_of_current_line =
				get_fts(context->timing, context->my_field);
		context->ts_last_char_received =
			get_fts(context->timing, context->my_field);
	}
}

/* Handle MID-ROW CODES. */
void handle_text_attr(const unsigned char c1, const unsigned char c2,
					  ccx_decoder_608_context *context)
{
	...
	ccx_common_logging.debug_ftn(CCX_DMT_DECODER_608,
								 "\r608: text_attr: %02X %02X", c1, c2);
	if (((c1 != 0x11 && c1 != 0x19) || (c2 < 0x20 || c2 > 0x2f)))
	{
		ccx_common_logging.debug_ftn(CCX_DMT_DECODER_608,
									 "\rThis is not a text attribute!\n");
	}
	else
	{
		int i = c2 - 0x20;
		...
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_608, "  --  Color: %s,  font: %s\n",
			color_text[context->current_color][0], font_text[context->font]);
		// Mid-row codes should put a non-transparent space at the current
...
get_current_visible_buffer(ccx_decoder_608_context *context)
{
	struct eia608_screen *data;
	if (context->visible_buffer == 1)
...
int write_cc_buffer(ccx_decoder_608_context *context, struct cc_subtitle *sub)
{
	...
	if (context->mode == MODE_FAKE_ROLLUP_1 && // Use the actual start of data
											   // instead of last buffer change
	...
	{
		sub->data = (struct eia608_screen *)realloc(
			sub->data, (sub->nb_data + 1) * sizeof(*data));
		if (!sub->data)
		{
		...
		memcpy(((struct eia608_screen *)sub->data) + sub->nb_data, data,
			   sizeof(*data));
		sub->nb_data++;
		wrote_something = 1;
		if (start_time < end_time)
		{
			...
			{
				data->start_time =
					start_time + (((end_time - start_time) / nb_data) * i);
				data->end_time =
					start_time +
					(((end_time - start_time) / nb_data) * (i + 1));
				data++;
			}
			sub->got_output = 1;
		}
...
int check_roll_up(ccx_decoder_608_context *context)
{
	...
	case MODE_ROLLUP_4:
		...
		break;
	case MODE_TEXT:
		keep_lines = 7; // CFS: can be 7 to 15 according to the handbook. No
						// idea how this is selected.
		break;
	default: // Shouldn't happen
		return 0;
		break;
	}
	if (use_buffer->row_used[0]) // If top line is used it will go off the
	...
	if ((firstrow - 1) <=
		context->cursor_row - keep_lines) // Roll up will delete those lines.
		return 1;
	return 0;
}

// Roll-up: Returns true if a line was rolled over the visible area (it
// disappears from screen), false if the rollup didn't delete any line.
int roll_up(ccx_decoder_608_context *context)
{
	...
	ccx_common_logging.debug_ftn(
		CCX_DMT_DECODER_608,
		"\rIn roll-up: %d lines used, first: %d, last: %d\n", rows_orig,
		firstrow, lastrow);
	...
	for (int j = lastrow - keep_lines + 1; j < lastrow; j++)
	{
		if (j >= 0)
		{
			memcpy(use_buffer->characters[j], use_buffer->characters[j + 1],
				   CCX_DECODER_608_SCREEN_WIDTH + 1);
			memcpy(use_buffer->colors[j], use_buffer->colors[j + 1],
				   CCX_DECODER_608_SCREEN_WIDTH + 1);
			memcpy(use_buffer->fonts[j], use_buffer->fonts[j + 1],
				   CCX_DECODER_608_SCREEN_WIDTH + 1);
	...
	for (int j = 0; j < (1 + context->cursor_row - keep_lines); j++)
	{
		...
		memset(use_buffer->colors[j], context->settings->default_color,
	...
	memset(use_buffer->colors[lastrow], context->settings->default_color,
		   CCX_DECODER_608_SCREEN_WIDTH);
...
void handle_command(unsigned char c1, const unsigned char c2,
					ccx_decoder_608_context *context, struct cc_subtitle *sub)
{
	int changes = 0;
	...
	if ((command == COM_ROLLUP2 || command == COM_ROLLUP3 ||
		 command == COM_ROLLUP4) &&
		context->settings->force_rollup == 1)
		command = COM_FAKE_RULLUP1;

	if ((command == COM_ROLLUP3 || command == COM_ROLLUP4) &&
		context->settings->force_rollup == 2)
		command = COM_ROLLUP2;
	else if (command == COM_ROLLUP4 && context->settings->force_rollup == 3)
		command = COM_ROLLUP3;

	ccx_common_logging.debug_ftn(CCX_DMT_DECODER_608,
								 "\rCommand begin: %02X %02X (%s)\n", c1, c2,
								 command_type[command]);
	ccx_common_logging.debug_ftn(
		CCX_DMT_DECODER_608,
		"\rCurrent mode: %d  Position: %d,%d  VisBuf: %d\n", context->mode,
	...
	case COM_BACKSPACE:
		...
		{
			context->cursor_column--;
			get_writing_buffer(context)
				->characters[context->cursor_row][context->cursor_column] = ' ';
		}
		break;
	case COM_TABOFFSET1:
		if (context->cursor_column < CCX_DECODER_608_SCREEN_WIDTH - 1)
	...
	case COM_ROLLUP4:
		...
		// If the reception of data for a row is interrupted by data for the
		// alternate data channel or for text mode, the display of caption text
		// will resume from the same cursor position if a roll-up caption
		// command is received and no PAC is given [...]
		if (context->mode != MODE_TEXT && context->have_cursor_position == 0)
		{
			// If no Preamble Address Code  is received, the base row shall
			// default to row 15
			context->cursor_row =
				14; // Default if the previous mode wasn't roll up already.
	...
	case COM_CARRIAGERETURN:
		if (context->mode == MODE_PAINTON) // CR has no effect on painton mode
										   // according to zvbis' code
			break;
		if (context->mode == MODE_POPON) // CFS: Not sure about this. Is there a
										 // valid reason for CR in popup?
		{
			context->cursor_column = 0;
		...
		if (changes)
		{
			// Only if the roll up would actually cause a line to disappear we
			// write the buffer
			if (context->output_format != CCX_OF_TRANSCRIPT)
			{
				if (write_cc_buffer(context, sub))
				...
				if (context->settings->no_rollup)
					erase_memory(context, true); // Make sure the lines we just
												 // wrote aren't written again
			}
		}
	...
	case COM_ERASEDISPLAYEDMEMORY:
		...
		if (context->output_format == CCX_OF_TRANSCRIPT &&
			(context->mode == MODE_FAKE_ROLLUP_1 ||
			 ...
			 context->mode == MODE_ROLLUP_4))
		{
			// In transcript mode we just write the cursor line. The previous
			// lines should have been written already, so writing everything
	...
	case COM_ALARMOFF:
	case COM_ALARMON:
		// These two are unused according to Robson's, and we wouldn't be able
		// to do anything useful anyway
		break;
	case COM_RESUMEDIRECTCAPTIONING:
		context->mode = MODE_PAINTON;
		// ccx_common_logging.log_ftn ("\nWarning: Received
		// ResumeDirectCaptioning, this mode is almost impossible.\n");
	...
	ccx_common_logging.debug_ftn(
		...
		context->cursor_row, context->cursor_column, context->visible_buffer);
	ccx_common_logging.debug_ftn(CCX_DMT_DECODER_608,
								 "\rCommand end: %02X %02X (%s)\n", c1, c2,
								 command_type[command]);
}

void flush_608_context(ccx_decoder_608_context *context,
...
void handle_double(const unsigned char c1, const unsigned char c2,
				   ccx_decoder_608_context *context)
{
	unsigned char c;
	if (context->channel != context->my_channel)
	...
	if (c2 >= 0x30 && c2 <= 0x3f)
	{
		...
		ccx_common_logging.debug_ftn(
			CCX_DMT_DECODER_608, "\rDouble: %02X %02X  -->  %c\n", c1, c2, c);
		write_char(c, context);
	}
}

/* Process EXTENDED CHARACTERS */
unsigned char handle_extended(unsigned char hi, unsigned char lo,
							  ccx_decoder_608_context *context)
{
	// Handle channel change
...
void handle_pac(unsigned char c1, unsigned char c2,
				ccx_decoder_608_context *context)
{
	// Handle channel change
	if (context->new_channel > 2)
	...
	ccx_common_logging.debug_ftn(CCX_DMT_DECODER_608, "\rPAC: %02X %02X", c1,
								 c2);
	...
	ccx_common_logging.debug_ftn(
		CCX_DMT_DECODER_608, "  --  Position: %d:%d, color: %s,  font: %s\n",
		row, indent, color_text[context->current_color][0],
		font_text[context->font]);
	if (context->settings->default_color == COL_USERDEFINED &&
		(context->current_color == COL_WHITE ||
	...
	{
		/* In roll-up, delete lines BELOW the PAC. Not sure (CFS) this is
		   correct (possibly we may have to move the
		   buffer around instead) but it's better than leaving old characters in
		   the buffer */
		struct eia608_screen *use_buffer =
			get_writing_buffer(context); // &wb->data608->buffer1;

		for (int j = row; j < 15; j++)
		{
			if (use_buffer->row_used[j])
			{
				memset(use_buffer->characters[j], ' ',
					   CCX_DECODER_608_SCREEN_WIDTH);
				memset(use_buffer->colors[j], context->settings->default_color,
					   CCX_DECODER_608_SCREEN_WIDTH);
				memset(use_buffer->fonts[j], FONT_REGULAR,
...
void erase_both_memories(ccx_decoder_608_context *context,
						 struct cc_subtitle *sub)
{
	erase_memory(context, false);
...
int check_channel(unsigned char c1, ccx_decoder_608_context *context)
{
	...
	if (newchan != context->channel)
	{
		...
		if (context->channel !=
			3) // Don't delete memories if returning from XDS.
		{
			// erase_both_memories (wb); // 47cfr15.119.pdf, page 859, part f
			// CFS: Removed this because the specs say memories should be
			// deleted if THE USER changes the channel.
		}
	}
	return newchan;
}
...
int disCommand(unsigned char hi, unsigned char lo,
			   ccx_decoder_608_context *context, struct cc_subtitle *sub)
{
...
int process608(const unsigned char *data, int length, void *private_data,
			   struct cc_subtitle *sub)
{
	...
	for (i = 0; i < length; i = i + 2)
	{
		...
		if (hi >= 0x10 && hi <= 0x1e)
		{
			...
			if (context == NULL ||
				context->my_field ==
					2) // Originally: current_field from sequencing.c. Seems to
					   // be just to change channel, so context->my_field seems
					   // good.
				ch += 2;
		...
		if (hi >= 0x01 && hi <= 0x0E &&
			(context == NULL ||
			 context->my_field == 2)) // XDS can only exist in field 2.
		{
			if (context)
				context->channel = 3;
			if (!in_xds_mode)
		...
		if (hi == 0x0F && in_xds_mode &&
			(context == NULL || context->my_field == 2)) // End of XDS block
		{
			in_xds_mode = 0;
			do_end_of_xds(sub, dec_ctx->xds_ctx, lo);
			if (context)
				context->channel =
					context->new_channel; // Switch from channel 3
			continue;
		}
		if (hi >= 0x10 &&
			hi <=
					  ...
					  // http://www.theneitherworld.com/mcpoodle/SCC_TOOLS/DOCS/CC_CHARS.HTML
		{
			...
			if (!context) // Not XDS and we don't have a writebuffer, nothing
						  // else would have an effect
				continue;
			...
			if (context->last_c1 == hi && context->last_c2 == lo)
			{
				...
				ccx_common_logging.debug_ftn(
					CCX_DMT_DECODER_608,
					"Skipping command %02X,%02X Duplicate\n", hi, lo);
		...
		else
		{
			...
			if (!context) // No XDS code after this point, and user doesn't want
						  // captions.
				continue;
			...
			if (!context->textprinted &&
				context->channel == context->my_channel)
			{ // Current FTS information after the characters are shown
				ccx_common_logging.debug_ftn(
					CCX_DMT_DECODER_608, "Current FTS: %s\n",
					print_mstime_static(
						get_fts(dec_ctx->timing, context->my_field)));
			...
			if (wrote_to_screen &&
				context->settings
					->direct_rollup && // If direct_rollup is enabled and
				(context->mode ==
					 MODE_FAKE_ROLLUP_1 || // we are in rollup mode, write now.
				 context->mode == MODE_ROLLUP_2 ||
				 context->mode == MODE_ROLLUP_3 ||
				 context->mode == MODE_ROLLUP_4))
			{
