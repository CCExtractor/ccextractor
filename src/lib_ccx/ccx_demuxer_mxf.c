...
#define debug(fmt, ...)                                                        \
	ccx_common_logging.debug_ftn(CCX_DMT_PARSE, "MXF:%s:%d: " fmt,             \
								 __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define log(fmt, ...)                                                          \
	ccx_common_logging.log_ftn("MXF:%d: " fmt, __LINE__, ##__VA_ARGS__)
...
static const MXFCodecUL mxf_caption_essence_container[] = {
	{{0x6, 0xE, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x09, 0xD, 0x1, 0x3, 0x1, 0x2,
	  0xD, 0x0, 0x0},
	 MXF_CT_VBI},
	{{0x6, 0xE, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x09, 0xD, 0x1, 0x3, 0x1, 0x2,
	  0xE, 0x0, 0x0},
...
static const struct ccx_rational framerate_rationals[16] = {
	{0, 0},	 {1001, 24000}, {1, 24},	   {1, 25}, {1001, 30000},
	{1, 30}, {1, 50},		{1001, 60000}, {1, 60}, {0, 0}};
...
void update_tid_lut(struct MXFContext *ctx, uint32_t track_id,
					uint8_t *track_number, struct ccx_rational edit_rate)
{
	int i;
	...
	for (i = 0; i < ctx->nb_tracks; i++)
	{
		if (ctx->tracks[i].track_id == track_id)
		{
			...
			debug("Key "
				  ...
				  "02X%02X essence_key\n",
				  ctx->cap_essence_key[0], ctx->cap_essence_key[1],
				  ctx->cap_essence_key[2], ctx->cap_essence_key[3],
				  ctx->cap_essence_key[4], ctx->cap_essence_key[5],
				  ctx->cap_essence_key[5], ctx->cap_essence_key[7],
				  ctx->cap_essence_key[8], ctx->cap_essence_key[9],
				  ctx->cap_essence_key[10], ctx->cap_essence_key[11],
				  ctx->cap_essence_key[12], ctx->cap_essence_key[13],
				  ctx->cap_essence_key[14], ctx->cap_essence_key[15]);
			return;
		}
	}
}
static int mxf_read_header_partition_pack(struct ccx_demuxer *demux,
										  uint64_t size)
{
	int ret;
	int len = 0;
	...
	{
		log("Partition pack has invalid essense container count(%d) to fit in "
			"partition size(%d)\n",
	...
	for (i = 0; i < nb_essence_container; i++)
	{
		...
		demux->past += 16;
		len += 16;
		for (j = 0; j < sizeof(mxf_caption_essence_container) /
							sizeof(*mxf_caption_essence_container);
			 j++)
		{
...
static int mxf_read_timeline_track_metadata(struct ccx_demuxer *demux,
											uint64_t size)
{
...
static int mxf_read_cdp_data(struct ccx_demuxer *demux, int size,
 ...
 */
static int mxf_read_vanc_data(struct ccx_demuxer *demux, uint64_t size,
							  struct demuxer_data *data)
{
	int len = 0;
	int ret;
...
static int mxf_read_essence_element(struct ccx_demuxer *demux, uint64_t size,
									struct demuxer_data *data)
{
	int ret;
	struct MXFContext *ctx = demux->private_data;
...
static const MXFReadTableEntry mxf_read_table[] = {
	/* According to section 7.1 of S377 partition key byte 14 and 15 have
	   variable values */
	{{0x06, 0x0e, 0x2b, 0x34, 0x02, 0x05, 0x01, 0x01, 0x0d, 0x01, 0x02, 0x01,
	  0x01, 0x02, 0x04, 0x00},
	 mxf_read_header_partition_pack},
	/* Structural Metadata Sets */
	{{0x06, 0x0e, 0x2b, 0x34, 0x02, 0x53, 0x01, 0x01, 0x0d, 0x01, 0x01, 0x01,
	  0x01, 0x01, 0x3B, 0x00},
	 mxf_read_timeline_track_metadata},
	{{0x06, 0x0e, 0x2b, 0x34, 0x02, 0x53, 0x01, 0x01, 0x0d, 0x01, 0x01, 0x01,
	  0x01, 0x01, 0x5c, 0x00},
	 mxf_read_vanc_vbi_desc},
	/* Generic Track */
...
static int mxf_read_sync(struct ccx_demuxer *ctx, const uint8_t *key,
						 size_t size)
{
	size_t i;
	unsigned char b;
...
static int read_packet(struct ccx_demuxer *demux, struct demuxer_data *data)
{
	...
	while ((ret = klv_read_packet(&klv, demux)) == 0)
	{
		debug("Key "
			  ...
			  "02X size %" PRIu64 "\n",
			  klv.key[0], klv.key[1], klv.key[2], klv.key[3], klv.key[4],
			  klv.key[5], klv.key[5], klv.key[7], klv.key[8], klv.key[9],
			  klv.key[10], klv.key[11], klv.key[12], klv.key[13], klv.key[14],
			  klv.key[15], klv.length);
