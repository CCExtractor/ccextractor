...
uint32_t asf_readval(void *val, int ltype)
{
	...
	default:
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In asf_readval(): Invalid ltype, cannot continue processing "
			  "this stream.\n");
		break;
	}
	return rval;
...
char *gui_data_string(void *val)
{
	...
	sprintf(sbuf, "%08lX-%04X-%04X-", (long)*((uint32_t *)((char *)val + 0)),
...
int asf_get_more_data(struct lib_ccx_ctx *ctx, struct demuxer_data **ppdata)
{
	...
	int payload_parser_size =
		0; // Inferred (PacketLType + sequence_type + padding_l_type + 6);
	...
	{
		asf_data_container =
			(asf_data){.parsebuf = (unsigned char *)malloc(1024),
					   .parsebufsize = 1024,
					   .FileSize = 0,
					   .PacketSize = 0,
					   .StreamProperties =
						   {// Make sure the stream numbers are invalid when a
							// new file begins
		...
		{
			fatal(EXIT_MISSING_ASF_HEADER,
				  "Missing ASF header. Could not read ASF file. Abort.\n");
		}
		asf_data_container.HeaderObjectSize =
			*((int64_t *)(asf_data_container.parsebuf + 16));
		dbg_print(CCX_DMT_PARSE, "Length: %lld\n",
				  asf_data_container.HeaderObjectSize);
		dbg_print(CCX_DMT_PARSE, "\nNumber of header objects: %ld\n",
				  (long)*((uint32_t *)(asf_data_container.parsebuf + 24)));

		if (asf_data_container.HeaderObjectSize >
			asf_data_container.parsebufsize)
		{
			asf_data_container.parsebuf = (unsigned char *)realloc(
				asf_data_container.parsebuf,
				(size_t)asf_data_container.HeaderObjectSize);
			if (!asf_data_container.parsebuf)
				fatal(EXIT_NOT_ENOUGH_MEMORY,
					  "In asf_getmoredata: Out of memory requesting buffer for "
					  "data container.");
			asf_data_container.parsebufsize =
				(long)asf_data_container.HeaderObjectSize;
		}
		...
		result =
			buffered_read(ctx->demux_ctx, current_position, (int)get_bytes);
		ctx->demux_ctx->past += result;
		if (result != get_bytes)
		{
			mprint("Premature end of file!\n");
		...
		while (current_position < asf_data_container.parsebuf +
									  asf_data_container.HeaderObjectSize)
		{
			int64_t hpobjectsize =
				*((int64_t *)(current_position + 16)); // Local
			...
			if (!memcmp(current_position, ASF_FILE_PROPERTIES, 16))
			{
				...
				asf_data_container.FileSize =
					*((int64_t *)(current_position + 40));
			...
			else if (!memcmp(current_position, ASF_STREAM_PROPERTIES, 16))
			{
				...
				if (!memcmp(current_position + 24, ASF_VIDEO_MEDIA, 16))
				{
					...
					dbg_print(
						CCX_DMT_PARSE, "Video Stream Number: %d\n",
						asf_data_container.StreamProperties.VideoStreamNumber);
				}
				else if (!memcmp(current_position + 24, ASF_AUDIO_MEDIA, 16))
				{
					...
					dbg_print(
						CCX_DMT_PARSE, "Audio Stream Number: %d\n",
						asf_data_container.StreamProperties.AudioStreamNumber);
				}
				else
				{
			...
			else if (!memcmp(current_position, ASF_HEADER_EXTENSION, 16))
			{
				...
				int32_t header_extension_data_size =
					*((uint32_t *)(current_position + 42));
				// Process Header Extension Data
				if (header_extension_data_size)
				{
					unsigned char *header_current_position =
						current_position + 46;

					if (header_extension_data_size != hpobjectsize - 46)
						fatal(
							EXIT_NOT_CLASSIFIED,
							"In asf_getmoredata: Incorrect "
							"HeaderExtensionDataSize value, cannot continue.");

					dbg_print(CCX_DMT_PARSE,
							  "\nReading Header Extension Sub-Objects\n");
					while (header_current_position <
						   current_position + 46 + header_extension_data_size)
					{
						int64_t header_object_size = *(
							(int64_t *)(header_current_position + 16)); // Local

						if (!memcmp(header_current_position,
									ASF_EXTENDED_STREAM_PROPERTIES, 16))
						{
							dbg_print(CCX_DMT_PARSE,
									  ...
									  header_object_size);
							int stream_number =
								*((uint16_t *)(header_current_position + 72));
							int stream_name_count =
								*((uint16_t *)(header_current_position + 84));
							int payload_ext_system_count =
								*((uint16_t *)(header_current_position + 86));

							unsigned char *stream_prop_position =
								header_current_position + 88;

							int stream_name_length;

							dbg_print(CCX_DMT_PARSE,
									  "Stream Number: %d NameCount: %d  "
									  "ESCount: %d\n",
									  stream_number, stream_name_count,
									  payload_ext_system_count);

							if (stream_number >= STREAMNUM)
								fatal(
									CCX_COMMON_EXIT_BUG_BUG,
									"In asf_getmoredata: STREAMNUM too small. "
									"Please file a bug report on GitHub.\n");

							for (int i = 0; i < stream_name_count; i++)
							{
								dbg_print(CCX_DMT_PARSE,
										  "%2d. Stream Name Field\n", i);
								stream_name_length =
									*((uint16_t *)(stream_prop_position + 2));
							...
							if (payload_ext_system_count > PAYEXTNUM)
								fatal(
									CCX_COMMON_EXIT_BUG_BUG,
									"In asf_getmoredata: PAYEXTNUM too small. "
									"Please file a bug report on GitHub.\n");

							for (int i = 0; i < payload_ext_system_count; i++)
							{
								ext_system_data_size =
									*((uint16_t *)(stream_prop_position + 16));
								ext_system_info_length =
									*((uint32_t *)(stream_prop_position + 18));

								asf_data_container
									.PayloadExtSize[stream_number][i] =
									ext_system_data_size;

								dbg_print(
									CCX_DMT_PARSE,
									"%2d. Payload Extension GUID: %s Size %d "
									"Info Length %d\n",
									i,
									gui_data_string(stream_prop_position + 0),
									ext_system_data_size,
									ext_system_info_length);

								// For DVR-MS presentation timestamp
								if (!memcmp(stream_prop_position, DVRMS_PTS,
											16))
								{
									dbg_print(CCX_DMT_PARSE,
											  "Found DVRMS_PTS\n");
									asf_data_container
										.PayloadExtPTSEntry[stream_number] = i;
								}

								stream_prop_position +=
									22 + ext_system_info_length;
							}

							// Now, there can be a Stream Properties Object. The
							// only way to find out is to check if there are
							// bytes left in the current object.
							if ((stream_prop_position -
								 header_current_position) < header_object_size)
							{
								int64_t stream_prop_object_size =
									*((int64_t *)(stream_prop_position +
												  16)); // Local
								if (memcmp(stream_prop_position,
										   ASF_STREAM_PROPERTIES, 16))
									fatal(
										EXIT_NOT_CLASSIFIED,
										"Stream Properties Object expected\n");

								if (!memcmp(stream_prop_position + 24,
											ASF_VIDEO_MEDIA, 16))
								{
									dbg_print(CCX_DMT_PARSE,
									...
									asf_data_container.StreamProperties
										.VideoStreamNumber = stream_number;
								}
								else if (!memcmp(stream_prop_position + 24,
												 ASF_AUDIO_MEDIA, 16))
								{
									dbg_print(CCX_DMT_PARSE,
											  "Stream Type: ASF_Audio_Media "
											  "(size: %lld)\n",
											  stream_prop_object_size);
									asf_data_container.StreamProperties
										.AudioStreamNumber = stream_number;
								}
								else if (!memcmp(stream_prop_position + 24,
												 ASF_BINARY_MEDIA, 16))
								{
									// dvr-ms files identify audio streams as
									// binary streams but use the "major media
									// type" accordingly to identify the steam.
									// (There might be other audio identifiers.)
									if (!memcmp(stream_prop_position + 78,
												DVRMS_AUDIO, 16))
									{
										dbg_print(CCX_DMT_PARSE,
												  "Binary media: DVR-MS Audio "
												  "Stream (size: %lld)\n",
												  stream_prop_object_size);
									}
									else if (!memcmp(stream_prop_position + 78,
													 DVRMS_NTSC, 16))
									{
										dbg_print(CCX_DMT_PARSE,
												  "Binary media: NTSC captions "
												  "(size: %lld)\n",
												  stream_prop_object_size);
										asf_data_container.StreamProperties
											.CaptionStreamNumber =
									...
									else
									{
										dbg_print(
											CCX_DMT_PARSE,
											"Binary media: Major Media Type "
											"GUID: %s (size: %lld)\n",
											gui_data_string(
												stream_prop_position + 78),
											stream_prop_object_size);
									}
								}
								else
								{
									dbg_print(
										CCX_DMT_PARSE,
										"Unknown Type GUID: %s (size: %lld)\n",
										gui_data_string(stream_prop_position +
														24),
										stream_prop_object_size);
								}
							}
							...
							{
								dbg_print(CCX_DMT_PARSE,
										  "No Stream Properties Object\n");
							}
						}
						else if (!memcmp(header_current_position, ASF_METADATA,
										 16))
						{
							dbg_print(CCX_DMT_PARSE,
						...
						else if (!memcmp(header_current_position,
										 ASF_METADATA_LIBRARY, 16))
						{
							dbg_print(
								CCX_DMT_PARSE,
						...
						else if (!memcmp(header_current_position, ASF_PADDING,
										 16))
						{
							dbg_print(CCX_DMT_PARSE,
						...
						{
							dbg_print(CCX_DMT_PARSE, "\nGUID: %s  size: %lld\n",
									  gui_data_string(header_current_position),
									  header_object_size);
							dump(CCX_DMT_PARSE, header_current_position, 16, 0,
								 0);
						}

						header_current_position += header_object_size;
					}
					if (header_current_position - (current_position + 46) !=
						header_extension_data_size)
						fatal(EXIT_NOT_CLASSIFIED,
							  "Header Extension Parsing problem: read bytes "
							  "%ld != header length %lld\nAbort!\n",
							  (long)(header_current_position -
									 (current_position + 46)),
							  header_extension_data_size);
				}
			...
			else if (!memcmp(current_position, ASF_EXTENDED_CONTENT_DESCRIPTION,
							 16))
			{
				dbg_print(
					CCX_DMT_PARSE,
				...
				int content_descriptor_count =
					*((uint16_t *)(current_position + 24));
				...
				for (int i = 0; i < content_descriptor_count; i++)
				{
					descriptor_name_length =
						*((uint16_t *)(ext_content_position));
					descriptor_value_data_type =
						*((uint16_t *)(ext_content_position + 2 +
									   descriptor_name_length));
					descriptor_value_length =
						*((uint16_t *)(ext_content_position + 4 +
									   descriptor_name_length));
					extended_description_value =
						ext_content_position + 6 + descriptor_name_length;

					dbg_print(CCX_DMT_PARSE, "%3d. %ls = ", i,
							  (wchar_t *)(ext_content_position + 2));
					...
					case 1: // byte string
						dbg_print(CCX_DMT_PARSE, ":");
						for (int ii = 0; ii < descriptor_value_length && ii < 9;
							 ii++)
						{
							dbg_print(
								CCX_DMT_PARSE, "%02X:",
						...
						if (descriptor_value_length > 8)
							dbg_print(CCX_DMT_PARSE, "skipped %d more",
									  descriptor_value_length - 8);
					...
					case 5: // WORD
						dbg_print(
							CCX_DMT_PARSE, "%u (WORD)\n",
							(int)*((uint16_t *)extended_description_value));
						break;
					default:
						fatal(CCX_COMMON_EXIT_BUG_BUG,
							  "In asf_getmoredata: Impossible value for "
							  "DescriptorValueDataType. Please file a bug "
							  "report in GitHub.\n");
						break;
					}

					if (!memcmp(ext_content_position + 2,
								...
								descriptor_name_length))
					{
						...
						asf_data_container.VideoClosedCaptioningFlag =
							*((int32_t *)extended_description_value);
						dbg_print(CCX_DMT_PARSE,
								  "Found WM/VideoClosedCaptioning flag: %d\n",
								  asf_data_container.VideoClosedCaptioningFlag);
					}

					ext_content_position +=
						6 + descriptor_name_length + descriptor_value_length;
				}
			}
			else if (!memcmp(current_position, ASF_STREAM_BITRATE_PROPERTIES,
							 16))
			{
				dbg_print(
					CCX_DMT_PARSE,
		...
		if (current_position - asf_data_container.parsebuf !=
			asf_data_container.HeaderObjectSize)
			fatal(EXIT_NOT_CLASSIFIED,
				  "Header Object Parsing problem: read bytes %ld != header "
				  "length %lld\nAbort!\n",
				  (long)(current_position - asf_data_container.parsebuf),
				  asf_data_container.HeaderObjectSize);

		if (asf_data_container.StreamProperties.VideoStreamNumber == 0)
			fatal(EXIT_NOT_CLASSIFIED, "No Video Stream Properties Object "
									   "found.  Unable to continue ...\n");

		// Wouldn't it be nice if  VideoClosedCaptioningFlag  would be usable,
		// unfortunately it is not reliable.
		...
		if (asf_data_container.StreamProperties.CaptionStreamNumber > 0 &&
			(asf_data_container.StreamProperties.CaptionStreamStyle == 1 ||
			 (asf_data_container.StreamProperties.CaptionStreamStyle == 2 &&
			  !ccx_options.wtvconvertfix)))
		{
			mprint("\nNote: If you converted a WTV into a DVR-MS and "
				   "CCExtractor finds no captions, try passing -wtvconvertfix "
				   "to work around bug in the conversion process.");
		}
		if (asf_data_container.StreamProperties.CaptionStreamNumber > 0 &&
			(asf_data_container.StreamProperties.CaptionStreamStyle == 1 ||
			 (asf_data_container.StreamProperties.CaptionStreamStyle == 2 &&
		...
		else if (asf_data_container.StreamProperties.CaptionStreamNumber > 0 &&
				 asf_data_container.StreamProperties.CaptionStreamStyle == 2)
		{
			// if (debug_parse)
			mprint("\nATSC captions (probably) in stream #%d - Decode the "
				   "video stream #%d instead\n\n",
				   asf_data_container.StreamProperties.CaptionStreamNumber,
				   asf_data_container.StreamProperties.VideoStreamNumber);
			asf_data_container.StreamProperties.DecodeStreamNumber =
				asf_data_container.StreamProperties.VideoStreamNumber;
		...
		else
		{
			asf_data_container.StreamProperties.DecodeStreamNumber =
				asf_data_container.StreamProperties.VideoStreamNumber;
			// if (debug_parse)
			mprint("\nAssume CC info in video stream #%d (No caption stream "
				   "found)\n\n",
				   asf_data_container.StreamProperties.DecodeStreamNumber);
		}

		// When reading "Payload parsing information" it occured that "Packet
		// Lenght" was not set (Packet Length Type 00) and for "Single Payloads"
		// this means the Payload Data Length cannot be infered.  Use
		// min_packet_size, max_packet_size instead.
		...
		result = buffered_read(ctx->demux_ctx, asf_data_container.parsebuf,
							   50); // No realloc needed.
		ctx->demux_ctx->past += result;
		if (result != 50)
		{
			mprint("Premature end of file!\n");
		...
		{
			fatal(EXIT_NOT_CLASSIFIED,
				  "In asf_getmoredata: Missing ASF Data Object. Abort.\n");
		}

		asf_data_container.DataObjectSize =
			*((int64_t *)(asf_data_container.parsebuf + 16));
		asf_data_container.TotalDataPackets =
			*((uint32_t *)(asf_data_container.parsebuf + 40));
	...
	while (asf_data_container.datapacketcur <
			   asf_data_container.TotalDataPackets &&
		   !enough)
	{
		...
		if (!reentry)
		{
			...
			dbg_print(CCX_DMT_PARSE, "\nReading packet %d/%d\n",
					  asf_data_container.datapacketcur + 1,
					  asf_data_container.TotalDataPackets);
			...
			{
				int ecdatalength = *asf_data_container.parsebuf &
								   0x0F; // Small, no realloc needed
				if (*asf_data_container.parsebuf & 0x60)
				{
					fatal(EXIT_NOT_CLASSIFIED,
						  "Error Correction Length Type not 00 - reserved - "
						  "aborting ...\n");
				}
				result = buffered_read(ctx->demux_ctx,
									   asf_data_container.parsebuf + 1,
									   ecdatalength);
				ctx->demux_ctx->past += result;
				asf_data_container.dobjectread += result;
				...
				if (asf_data_container.parsebuf[1] & 0x0F)
					fatal(
						EXIT_NOT_CLASSIFIED,
						"Error correction present.  Unable to continue ...\n");
			}
			else
			{
			...
			result = buffered_read(ctx->demux_ctx,
								   asf_data_container.parsebuf + ecinfo,
								   2 - ecinfo); // No realloc needed
			ctx->demux_ctx->past += result;
			asf_data_container.dobjectread += result;
			if (result != 2)
			{
			...
			asf_data_container.MultiplePayloads =
				*asf_data_container.parsebuf & 0x01;
			...
			asf_data_container.PacketLType =
				(*asf_data_container.parsebuf >> 5) & 0x03;
			asf_data_container.PacketLType =
				ASF_TypeLength(asf_data_container.PacketLType);

			asf_data_container.ReplicatedLType =
				(asf_data_container.parsebuf[1]) & 0x03;
			asf_data_container.ReplicatedLType =
				ASF_TypeLength(asf_data_container.ReplicatedLType);

			asf_data_container.OffsetMediaLType =
				(asf_data_container.parsebuf[1] >> 2) & 0x03;
			asf_data_container.OffsetMediaLType =
				ASF_TypeLength(asf_data_container.OffsetMediaLType);

			asf_data_container.MediaNumberLType =
				(asf_data_container.parsebuf[1] >> 4) & 0x03;
			asf_data_container.MediaNumberLType =
				ASF_TypeLength(asf_data_container.MediaNumberLType);

			asf_data_container.StreamNumberLType =
				(asf_data_container.parsebuf[1] >> 6) & 0x03;
			asf_data_container.StreamNumberLType =
				ASF_TypeLength(asf_data_container.StreamNumberLType);

			payload_parser_size = asf_data_container.PacketLType +
								  sequence_type + padding_l_type + 6;

			result =
				buffered_read(ctx->demux_ctx, asf_data_container.parsebuf + 2,
							  payload_parser_size); // No realloc needed
			ctx->demux_ctx->past += result;
			asf_data_container.dobjectread += result;
			...
			asf_data_container.PacketLength =
				asf_readval(asf_data_container.parsebuf + 2,
							asf_data_container.PacketLType);
			sequence = asf_readval(asf_data_container.parsebuf + 2 +
									   asf_data_container.PacketLType,
								   sequence_type);
			asf_data_container.PaddingLength =
				asf_readval(asf_data_container.parsebuf + 2 +
								asf_data_container.PacketLType + sequence_type,
							padding_l_type);
			// Data Packet ms time stamp
			send_time = *((uint32_t *)(asf_data_container.parsebuf + 2 +
									   asf_data_container.PacketLType +
									   sequence_type + padding_l_type));
			...
			if (asf_data_container.PacketLength == 0)
			{
				...
				if (asf_data_container.PacketLength == 0 &&
					asf_data_container.MultiplePayloads == 0)
					fatal(EXIT_NOT_CLASSIFIED,
						  "In asf_getmoredata: Cannot determine packet length. "
						  "Unable to continue processing this file.\n");
			}

			dbg_print(CCX_DMT_PARSE,
					  "Lengths - Packet: %d / sequence %d / Padding %d\n",
					  asf_data_container.PacketLength, sequence,
					  asf_data_container.PaddingLength);

			asf_data_container.PayloadLType =
				0; // Payload Length Type. <>0 for multiple payloads
			asf_data_container.PayloadLength =
				0; // Payload Length (for multiple payloads)
			...
			if (asf_data_container.MultiplePayloads != 0)
			{
				...
				asf_data_container.PayloadLType =
					ASF_TypeLength(asf_data_container.PayloadLType);
		...
		while (asf_data_container.payloadcur <
				   asf_data_container.NumberOfPayloads &&
			   !enough)
		{
			// Skip reading the Payload headers the first time when reentering
			// the loop
			if (!reentry)
			{
				if (asf_data_container.NumberOfPayloads < 2)
					...
					dbg_print(CCX_DMT_PARSE, "\nMultiple payloads %d/%d\n",
							  asf_data_container.payloadcur + 1,
							  asf_data_container.NumberOfPayloads);

				int payload_header_size = 1 +
										  asf_data_container.MediaNumberLType +
										  asf_data_container.OffsetMediaLType +
										  asf_data_container.ReplicatedLType;

				result =
					buffered_read(ctx->demux_ctx, asf_data_container.parsebuf,
								  payload_header_size); // No realloc needed
				ctx->demux_ctx->past += result;
				asf_data_container.dobjectread += result;
				...
				asf_data_container.PayloadStreamNumber =
					*asf_data_container.parsebuf & 0x7F;
				asf_data_container.KeyFrame =
					(*asf_data_container.parsebuf & 0x80) && 1;

				asf_data_container.PayloadMediaNumber =
					asf_readval(asf_data_container.parsebuf + 1,
								asf_data_container.MediaNumberLType);
				offset_media_length =
					asf_readval(asf_data_container.parsebuf + 1 +
									asf_data_container.MediaNumberLType,
				...
				if (replicated_length == 1)
					fatal(EXIT_NOT_CLASSIFIED,
						  "Cannot handle compressed data...\n");

				if ((long)replicated_length > asf_data_container.parsebufsize)
				{
					asf_data_container.parsebuf = (unsigned char *)realloc(
						asf_data_container.parsebuf, replicated_length);
					if (!asf_data_container.parsebuf)
						fatal(EXIT_NOT_ENOUGH_MEMORY,
							  "In asf_getmoredata: Not enough memory for "
							  "buffer, unable to continue.\n");
					asf_data_container.parsebufsize = replicated_length;
				}
				result =
					buffered_read(ctx->demux_ctx, asf_data_container.parsebuf,
								  (long)replicated_length);
				ctx->demux_ctx->past += result;
				asf_data_container.dobjectread += result;
				...
				media_object_size =
					*((uint16_t *)(asf_data_container.parsebuf));
				presentation_time_millis =
					*((uint16_t *)(asf_data_container.parsebuf + 4));
				replicate_position += 8;

				dbg_print(CCX_DMT_PARSE,
						  "Stream# %d[%d] Media# %d Offset/Size: %d/%d\n",
						  asf_data_container.PayloadStreamNumber,
						  asf_data_container.KeyFrame,
						  asf_data_container.PayloadMediaNumber,
				...
				for (int i = 0;
					 i < asf_data_container.PayloadExtPTSEntry
							 [asf_data_container.PayloadStreamNumber];
					 i++)
				{
					if (asf_data_container.PayloadExtSize
							[asf_data_container.PayloadStreamNumber][i] ==
						0xffff)
					{
					...
					{
						extsize =
							asf_data_container.PayloadExtSize
								[asf_data_container.PayloadStreamNumber][i];
					}
					replicate_position += extsize;
					// printf("%2d. Ext. System - size: %d\n", i, extsize);
				}
				if (asf_data_container.PayloadExtPTSEntry
						[asf_data_container.PayloadStreamNumber] > 0)
				{
				...
				dbg_print(CCX_DMT_PARSE, "   dvr-ms PTS: %s+%lld\n",
						  print_mstime_static(rtStart / 10000),
						  (rtEnd - rtStart) / 10000);
				...
				if (asf_data_container.MultiplePayloads != 0)
				{
					...
					result = buffered_read(ctx->demux_ctx, plheader,
										   asf_data_container.PayloadLType);
					ctx->demux_ctx->past += result;
					asf_data_container.dobjectread += result;
					...
					asf_data_container.PayloadLength =
						asf_readval(plheader, asf_data_container.PayloadLType);
				}
				else
				{
					asf_data_container.PayloadLength =
						asf_data_container.PacketLength - data_packet_length -
						asf_data_container.PaddingLength;
				}
				dbg_print(CCX_DMT_PARSE,
						  "Size - Replicated %d + Payload %d = %d\n",
						  replicated_length, asf_data_container.PayloadLength,
						  replicated_length + asf_data_container.PayloadLength);

				// Remember the last video time stamp - only when captions are
				// separate from video stream.
				if (asf_data_container.PayloadStreamNumber ==
						asf_data_container.StreamProperties.VideoStreamNumber &&
					asf_data_container.StreamProperties.DecodeStreamNumber !=
						asf_data_container.StreamProperties.VideoStreamNumber &&
					offset_media_length == 0)
				{
					asf_data_container.StreamProperties.prevVideoStreamMS =
						asf_data_container.StreamProperties.currVideoStreamMS;
					asf_data_container.StreamProperties.currVideoStreamMS =
						asf_data_container.StreamProperties.VideoStreamMS;

					// Use presentation_time_millis (send_time might also work)
					// when the dvr-ms time stamp is not present.
					if (asf_data_container.PayloadExtPTSEntry
							[asf_data_container.PayloadStreamNumber] > 0)
					{
					...
					else
					{
						...
						asf_data_container.StreamProperties.VideoStreamMS =
							presentation_time_millis + 1;
					}
					// This checks if there is a video time jump in the timeline
					// between caption information.
					if (abs(asf_data_container.StreamProperties
								.currVideoStreamMS -
							asf_data_container.StreamProperties
								.prevVideoStreamMS) > 500)
					{
				...
				if (asf_data_container.PayloadStreamNumber ==
					...
					offset_media_length == 0)
				{
					asf_data_container.StreamProperties.prevDecodeStreamPTS =
						asf_data_container.StreamProperties.currDecodeStreamPTS;
					asf_data_container.StreamProperties.currDecodeStreamPTS =
						asf_data_container.StreamProperties.DecodeStreamPTS;
					...
					if (asf_data_container.StreamProperties
							.DecodeStreamNumber !=
						asf_data_container.StreamProperties.VideoStreamNumber)
					{
							...
							500)
						{
							...
							if (!asf_data_container.StreamProperties.VideoJump)
								ccx_common_timing_settings.disable_sync_check =
									1;
							else
								ccx_common_timing_settings.disable_sync_check =
									0;
					...
					data->pts = asf_data_container.StreamProperties
									.currDecodeStreamPTS *
			...
			{
				if (asf_data_container.StreamProperties.DecodeStreamNumber ==
					asf_data_container.StreamProperties.CaptionStreamNumber)
					dbg_print(CCX_DMT_PARSE, "\nCaption stream object");
				else
					dbg_print(CCX_DMT_PARSE, "\nVideo stream object");
				dbg_print(
					CCX_DMT_PARSE, " read with PTS: %s\n",
					print_mstime_static(asf_data_container.StreamProperties
											.currDecodeStreamPTS));
			...
			if (asf_data_container.PayloadStreamNumber ==
				asf_data_container.StreamProperties.DecodeStreamNumber)
			{
				current_media_number =
					asf_data_container
						.PayloadMediaNumber; // Remember current value
				...
				int want = (long)((BUFSIZE - data->len) >
										  asf_data_container.PayloadLength
									  ? asf_data_container.PayloadLength
									  : (BUFSIZE - data->len));
				if (want < (long)asf_data_container.PayloadLength)
					fatal(CCX_COMMON_EXIT_BUG_BUG,
						  "Buffer size too small for ASF payload!\nPlease file "
						  "a bug report!\n");
				result = buffered_read(ctx->demux_ctx, data->buffer + data->len,
									   want);
				...
				asf_data_container.dobjectread +=
					asf_data_container.PayloadLength;
			}
			else
			{
				...
				dbg_print(CCX_DMT_PARSE, "Skipping Stream #%d data ...\n",
						  asf_data_container.PayloadStreamNumber);
				result = buffered_skip(ctx->demux_ctx,
									   (int)asf_data_container.PayloadLength);
				ctx->demux_ctx->past += result;
				if (result != asf_data_container.PayloadLength)
				{
					mprint("Premature end of file!\n");
		...
		dbg_print(CCX_DMT_PARSE, "Skip %d padding\n",
				  asf_data_container.PaddingLength);
		result = buffered_skip(ctx->demux_ctx,
							   (long)asf_data_container.PaddingLength);
		ctx->demux_ctx->past += result;
	...
	if (asf_data_container.datapacketcur == asf_data_container.TotalDataPackets)
	{
		...
		dbg_print(CCX_DMT_PARSE, "Skip the rest: %d\n",
				  (int)(asf_data_container.FileSize -
						asf_data_container.HeaderObjectSize -
						asf_data_container.DataObjectSize));
		result = buffered_skip(ctx->demux_ctx,
							   (int)(asf_data_container.FileSize -
