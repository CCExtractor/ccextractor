...
// Fills given string with given (event.*_time_string) ATSC time converted to
// XMLTV style time string
void EPG_ATSC_calc_time(char *output, uint32_t time)
{
	...
	timeinfo.tm_isdst = -1;
	mktime(&timeinfo);
	sprintf(output, "%02d%02d%02d%02d%02d%02d +0000", timeinfo.tm_year + 1900,
...
void EPG_DVB_calc_start_time(struct EPG_event *event, uint64_t time)
{
	...
	if (mjd > 0)
	{
		...
		sprintf(event->start_time_string, "%02ld%02ld%02ld%06" PRIu64 "+0000",
				y, m, d, time & 0xffffff);
	}
}

// Fills event.end_time_string in XMLTV with passed DVB time + duration
void EPG_DVB_calc_end_time(struct EPG_event *event, uint64_t time,
						   uint32_t duration)
{
	uint64_t mjd = time >> 24;
	...
	if (mjd > 0)
	{
		...
		timeinfo.tm_sec = (time & 0x0f) + (10 * ((time & 0xf0) >> 4)) +
						  (duration & 0x0f) + (10 * ((duration & 0xf0) >> 4));
		timeinfo.tm_min =
			((time & 0x0f00) >> 8) + (10 * ((time & 0xf000) >> 4) >> 8) +
			((duration & 0x0f00) >> 8) + (10 * ((duration & 0xf000) >> 4) >> 8);
		timeinfo.tm_hour = ((time & 0x0f0000) >> 16) +
						   (10 * ((time & 0xf00000) >> 4) >> 16) +
						   ((duration & 0x0f0000) >> 16) +
						   (10 * ((duration & 0xf00000) >> 4) >> 16);
		timeinfo.tm_isdst = -1;
		mktime(&timeinfo);
		sprintf(event->end_time_string, "%02d%02d%02d%02d%02d%02d +0000",
...
void EPG_output_net(struct lib_ccx_ctx *ctx)
{
	...
	{
		if (ctx->demux_ctx->pinfo[i].program_number ==
			ccx_options.demux_cfg.ts_forced_program)
			break;
	}
	...
	for (j = 0; j < ctx->eit_programs[i].array_len; j++)
	{
		...
		net_send_epg(event->start_time_string, event->end_time_string,
					 ...
					 event->ISO_639_language_code, category);
	}
}
...
void EPG_output_live(struct lib_ccx_ctx *ctx)
{
	...
	sprintf(filename, "%s_%i.xml.part", ctx->basefilename,
			ctx->epg_last_live_output);
	f = fopen(filename, "w");
	...
	for (i = 0; i < ctx->demux_ctx->nb_program; i++)
	{
		for (j = 0; j < ctx->eit_programs[i].array_len; j++)
			if (ctx->eit_programs[i].epg_events[j].live_output == false)
			{
				...
				EPG_print_event(&ctx->eit_programs[i].epg_events[j],
								ctx->demux_ctx->pinfo[i].program_number, f);
			}
	}
	fprintf(f, "</tv>");
	fclose(f);
...
void EPG_output(struct lib_ccx_ctx *ctx)
{
	...
	{ // print all events
		...
		if (ctx->demux_ctx->nb_program ==
			0) // Stream has no PMT, fall back to unordered events
			for (j = 0; j < ctx->eit_programs[TS_PMT_MAP_SIZE].array_len; j++)
				EPG_print_event(
					&ctx->eit_programs[TS_PMT_MAP_SIZE].epg_events[j],
					ctx->eit_programs[TS_PMT_MAP_SIZE].epg_events[j].service_id,
					f);
	}
	else
...
{
	if (e1->id != e2->id ||
		(strcmp(e1->start_time_string, e2->start_time_string) != 0) ||
		(strcmp(e1->end_time_string, e2->end_time_string) != 0))
		return false;
...
int EPG_add_event(struct lib_ccx_ctx *ctx, int32_t pmt_map,
				  struct EPG_event *event)
{
	int isnew = true, j;
	...
	for (j = 0; j < ctx->eit_programs[pmt_map].array_len; j++)
	{
		if (ctx->eit_programs[pmt_map].epg_events[j].id == event->id)
		{
			...
			{ // event with this id is already in the array but something has
			  // changed. Update it.
				event->count = ctx->eit_programs[pmt_map].epg_events[j].count;
				EPG_free_event(&ctx->eit_programs[pmt_map].epg_events[j]);
				memcpy(&ctx->eit_programs[pmt_map].epg_events[j], event,
					   sizeof(struct EPG_event));
				return true;
			}
		}
	}
	...
	event->count = 0;
	memcpy(&ctx->eit_programs[pmt_map]
				.epg_events[ctx->eit_programs[pmt_map].array_len],
		   event, sizeof(struct EPG_event));
...
void EPG_decode_content_descriptor(uint8_t *offset, uint32_t descriptor_length,
								   struct EPG_event *event)
{
	int i;
	...
	if (num_items == 0)
	{
		dbg_print(
			...
			"\rWarning: Invalid EIT content_descriptor length detected.\n");
		return;
	}
...
void EPG_decode_parental_rating_descriptor(uint8_t *offset,
										   ...
										   struct EPG_event *event)
{
	...
	if (num_items == 0)
	{
		dbg_print(CCX_DMT_GENERIC_NOTICES,
				  "\rWarning: Invalid EIT parental_rating length detected.\n");
		return;
	}
...
char *EPG_DVB_decode_string(uint8_t *in, size_t size)
{
	...
	{
		dbg_print(CCX_DMT_GENERIC_NOTICES,
				  "\rWarning: EPG_DVB_decode_string(): Reserved encoding "
				  "detected: %02x.\n",
				  in[0]);
		size--;
		in++;
	...
	else
	{
		...
		if (!skipiconv)
			dbg_print(CCX_DMT_GENERIC_NOTICES,
					  "\rWarning: EPG_DVB_decode_string(): Failed to convert "
					  "codepage.\n");
		/*
			...
			ALSO:
		   http://lists.gnu.org/archive/html/bug-gnu-libiconv/2009-09/msg00000.html
		*/
		for (x = 0; x < size; x++)
		{
			if (in[x] <= (uint8_t)127)
...
void EPG_decode_short_event_descriptor(uint8_t *offset,
									   uint32_t descriptor_length,
									   struct EPG_event *event)
{
	uint8_t text_length;
	...
	{
		dbg_print(CCX_DMT_GENERIC_NOTICES,
				  "\rWarning: Invalid short_event_descriptor size detected.\n");
		return;
	}
	...
	event->text =
		EPG_DVB_decode_string(&offset[5 + event_name_length], text_length);
}

// EN 300 468 V1.3.1 (1998-02)
// 6.2.9 Extended event descriptor
void EPG_decode_extended_event_descriptor(uint8_t *offset,
										  uint32_t descriptor_length,
										  struct EPG_event *event)
{
	...
	if (length_of_items > descriptor_length - 5)
	{
		dbg_print(
			CCX_DMT_GENERIC_NOTICES,
			"\rWarning: Invalid extended_event_descriptor size detected.\n");
		return;
	}
	...
	if (text_length > descriptor_length - 5 - length_of_items - 1)
	{
		dbg_print(
			CCX_DMT_GENERIC_NOTICES,
			"\rWarning: Invalid extended_event_text_length size detected.\n");
		return;
	}
	...
	if (descriptor_number == last_descriptor_number)
	{
		...
		event->extended_text = EPG_DVB_decode_string(
			event->extended_text, strlen(event->extended_text));
		free(old);
	}
}
...
void EPG_ATSC_decode_multiple_string(uint8_t *offset, uint32_t length,
									 struct EPG_event *event)
{
...
#define CHECK_OFFSET(val)                                                      \
	...
	for (i = 0; i < number_strings; i++)
	{
		...
		for (j = 0; j < number_segments; j++)
		{
			...
			{
				dbg_print(CCX_DMT_GENERIC_NOTICES,
						  "\rWarning: Unsupported ATSC multiple_string "
						  "encoding detected!.\n");
			}
			offset += number_bytes;
		}
...
void EPG_ATSC_decode_EIT(struct lib_ccx_ctx *ctx, uint8_t *payload_start,
						 uint32_t size)
{
	uint8_t table_id;
	struct EPG_event event;
	...
	{
		if (ctx->demux_ctx->pinfo[i].program_number ==
			ctx->ATSC_source_pg_map[source_id])
			pmt_map = i;
	}
...
#define CHECK_OFFSET(val)                                                      \
	...
	for (j = 0; j < num_events_in_section && offset < payload_start + size; j++)
	{
		...
		start_time = (offset[2] << 24) | (offset[3] << 16) | (offset[4] << 8) |
					 (offset[5] << 0);
		EPG_ATSC_calc_time(event.start_time_string, start_time);
		emt_location = (offset[6] & 0x30) >> 4;
		length_in_seconds =
			(((offset[6] & 0x0F) << 16) | (offset[7] << 8) | (offset[8] << 0));
		EPG_ATSC_calc_time(event.end_time_string,
		...
		descriptors_loop_length = ((offset[10 + title_length] & 0x0f) << 8) |
								  offset[10 + title_length + 1];
	...
	if ((ccx_options.xmltv == 1 || ccx_options.xmltv == 3) &&
		ccx_options.xmltvoutputinterval == 0 && hasnew)
		EPG_output(ctx);
#undef CHECK_OFFSET
}

// decode ATSC VCT table.
void EPG_ATSC_decode_VCT(struct lib_ccx_ctx *ctx, uint8_t *payload_start,
						 uint32_t size)
{
	uint8_t table_id;
...
void EPG_DVB_decode_EIT(struct lib_ccx_ctx *ctx, uint8_t *payload_start,
						uint32_t size)
{
	...
	// For any service we don't have an PMT for (yet), store it in the special
	// last array pos.
	if (pmt_map == -1)
		pmt_map = TS_PMT_MAP_SIZE;
	...
	{
		dbg_print(CCX_DMT_GENERIC_NOTICES,
				  "\rWarning: Invalid EIT packet size detected.\n");
	...
	while (remaining > 4)
	{
		...
		// 40 bits
		start_time = ((uint64_t)offset[16] << 32) |
					 ((uint64_t)offset[17] << 24) |
					 ((uint64_t)offset[18] << 16) |
					 ((uint64_t)offset[19] << 8) | ((uint64_t)offset[20] << 0);
		...
		if (descriptors_loop_length > remaining - 16)
		{
			dbg_print(
				CCX_DMT_GENERIC_NOTICES,
				"\rWarning: Invalid EIT descriptors_loop_length detected.\n");
			return;
		}
		while (descp < &(offset[26]) + descriptors_loop_length)
		{
			...
			{
				dbg_print(CCX_DMT_GENERIC_NOTICES,
						  "\rWarning: Invalid EIT descriptor_loop_length "
						  "detected.\n");
			...
			if (descp[0] == 0x4e)
				EPG_decode_extended_event_descriptor(descp + 2, descp[1],
													 &event);
			if (descp[0] == 0x54)
				EPG_decode_content_descriptor(descp + 2, descp[1], &event);
			if (descp[0] == 0x55)
				EPG_decode_parental_rating_descriptor(descp + 2, descp[1],
													  &event);
			if (descp[1] + 2 == 0)
			{
				dbg_print(
					CCX_DMT_GENERIC_NOTICES,
					"\rWarning: Invalid EIT descriptor_length detected.\n");
				return;
			}
...
{
	int cur_sec = (int)((ctx->demux_ctx->global_timestamp -
						 ctx->demux_ctx->min_global_timestamp) /
						1000);
	if (ccx_options.xmltv == 1 || ccx_options.xmltv == 3)
	{ // full output
		if (ccx_options.xmltvoutputinterval != 0 &&
			cur_sec > ctx->epg_last_output + ccx_options.xmltvliveinterval)
		{
...
void parse_EPG_packet(struct lib_ccx_ctx *ctx)
{
	...
	if (payload_start_indicator)
	{
		if (ctx->epg_buffers[buffer_map].ccounter > 0)
		{
			...
			EPG_parse_table(ctx, ctx->epg_buffers[buffer_map].buffer,
							ctx->epg_buffers[buffer_map].buffer_length);
		}
		...
		memcpy(ctx->epg_buffers[buffer_map].buffer, payload_start,
			   payload_length);
		ctx->epg_buffers[buffer_map].buffer_length = payload_length;
		ctx->epg_buffers[buffer_map].ccounter++;
	}
	else if (ccounter == ctx->epg_buffers[buffer_map].prev_ccounter + 1 ||
			 (ctx->epg_buffers[buffer_map].prev_ccounter == 0x0f &&
			  ccounter == 0))
	{
		ctx->epg_buffers[buffer_map].prev_ccounter = ccounter;
		ctx->epg_buffers[buffer_map].buffer = (uint8_t *)realloc(
			ctx->epg_buffers[buffer_map].buffer,
			ctx->epg_buffers[buffer_map].buffer_length + payload_length);
		memcpy(ctx->epg_buffers[buffer_map].buffer +
				   ctx->epg_buffers[buffer_map].buffer_length,
