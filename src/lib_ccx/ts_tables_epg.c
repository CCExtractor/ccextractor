...
void EPG_output_live(struct lib_ccx_ctx *ctx) {
	...
	for(int i=0; i<pmt_array_length; i++) {
		for(int j=0; j<ctx->eit_programs[i].array_len; j++)
...
int EPG_add_event(struct lib_ccx_ctx *ctx, int32_t pmt_map, struct EPG_event *event) {
	...
	for(int j=0; j<ctx->eit_programs[pmt_map].array_len; j++) {
...
void EPG_decode_short_event_descriptor(uint8_t *offset, uint32_t descriptor_length, struct EPG_event *event) {
	...
	uint8_t event_name_length = offset[3];
	...
	uint8_t text_length = offset[4+event_name_length];
...
void EPG_ATSC_decode_EIT(struct lib_ccx_ctx *ctx, uint8_t *payload_start, uint32_t size) {
	...
	uint8_t num_events_in_section = payload_start[9];
	uint8_t *offset=&payload_start[10];
	int hasnew=false;
	for(int j=0; j<num_events_in_section; j++) {
		uint16_t event_id = ((offset[0]&0x3F) << 8) | offset[1];
		uint32_t full_id = (source_id << 16) | event_id;
		event.id=full_id;
		uint32_t start_time = (offset[2] << 24) | (offset[3] << 16) | (offset[4] << 8)| (offset[5] << 0);
		EPG_ATSC_calc_time(event.start_time_string, start_time);
		uint8_t emt_location = (offset[6]&0x30)>>4;
		uint32_t length_in_seconds = (((offset[6]&0x0F) << 16) | (offset[7] << 8) | (offset[8] << 0));
		EPG_ATSC_calc_time(event.end_time_string, start_time+length_in_seconds);
		uint8_t title_length = offset[9];
		EPG_ATSC_decode_multiple_string(&offset[10], title_length, &event);

		uint16_t descriptors_loop_length = ((offset[10+title_length] & 0x0f) << 8) | offset[10+title_length+1];
...
void EPG_DVB_decode_EIT(struct lib_ccx_ctx *ctx, uint8_t *payload_start, uint32_t size) {
	...
	while(remaining>4) {	
		...
		event.live_output=false;
		//40 bits
		uint64_t start_time = ((uint64_t)offset[16] << 32) | ((uint64_t)offset[17] << 24) | ((uint64_t)offset[18] << 16) | ((uint64_t)offset[19] << 8)| ((uint64_t)offset[20] << 0);
		EPG_DVB_calc_start_time(&event, start_time);
		//24 bits
		uint32_t duration = (offset[21] << 16 ) | (offset[22] << 8 ) | (offset[23] << 0 );
		EPG_DVB_calc_end_time(&event, start_time, duration);
		event.running_status = (offset[24] & 0xE0) >> 5;
		event.free_ca_mode = (offset[24] & 0x10) >> 4;
		//12 bits
		uint16_t descriptors_loop_length = ((offset[24] & 0x0f) << 8) | offset[25];
