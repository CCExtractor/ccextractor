...
/*
   This variable (ccx_decoder_608_report) holds data on the cc channels & xds
   packets that are encountered during file parse. This can be interesting if
   you just want to know what kind of data a file holds that has 608 packets.
   CCExtractor uses it for the report functionality.
 */
...
{
	int direct_rollup; // Write roll-up captions directly instead of line by
					   // line?
...
typedef struct ccx_decoder_608_context
{
	...
	LLONG current_visible_start_ms; // At what time did the current visible
									// buffer became so?
	enum cc_modes mode;
	...
	int rollup_base_row;
	LLONG ts_start_of_current_line; /* Time at which the first character for
									   current line was received, =-1 no
									   character received yet */
	LLONG ts_last_char_received; /* Time at which the last written character was
									received, =-1 no character received yet */
	...
	int *halt; // Can be used to halt the feeding of caption data. Set to 1 if
			   // screens_to_progress != -1 && screenfuls_counter >=
			   // screens_to_process
 ...
 *
 * @param private_data context of cc608 where important information related to
 * 608 are stored.
 *
