...
static const char *DayOfWeekStr[7] = {"Sunday",	   "Monday",   "Tuesday",
									  "Wednesday", "Thursday", "Friday",
									  "Saturday"};

static const uint32 framerate_translation[16] = {
	0, 2397, 2400, 2500, 2997, 3000, 5000, 5994, 6000, 0, 0, 0, 0, 0};
...
static void ms_to_frame(struct encoder_ctx *ctx,
						ccx_mcc_caption_time *caption_time_ptr, int fr_code,
						int dropframe_flag);
static uint8 *add_boilerplate(struct encoder_ctx *ctx, unsigned char *cc_data,
							  int cc_count, int fr_code);
static uint16 count_compressed_chars(uint8 *data_ptr, uint16 num_elements);
static void compress_data(uint8 *data_ptr, uint16 num_elements,
						  uint8 *out_data_ptr);
...
boolean mcc_encode_cc_data(struct encoder_ctx *enc_ctx,
						   struct lib_cc_decode *dec_ctx,
						   unsigned char *cc_data, int cc_count)
{
	ASSERT(cc_data);
	ASSERT(enc_ctx);
	ASSERT(dec_ctx);

	ccx_mcc_caption_time caption_time = convert_to_caption_time(
		enc_ctx->timing->fts_now + enc_ctx->timing->fts_global);
	...
	if (enc_ctx->header_printed_flag == CCX_FALSE)
	{
		...
		generate_mcc_header(enc_ctx->out->fh, dec_ctx->current_frame_rate,
							enc_ctx->force_dropframe);
		...
		uint64 frame_number =
			caption_time.millisecond *
			framerate_translation[dec_ctx->current_frame_rate];
		frame_number = frame_number / 100000;
		if (frame_number >
			(framerate_translation[dec_ctx->current_frame_rate] / 100))
		{
			LOG("WARN: Normalized Frame Number %d to %d", frame_number,
				(framerate_translation[dec_ctx->current_frame_rate] / 100));
			frame_number =
				framerate_translation[dec_ctx->current_frame_rate] / 100;
		}
		...
		LOG("Captions start at: %02d:%02d:%02d:%02d / %02d:%02d:%02d,%03d",
			caption_time.hour, caption_time.minute, caption_time.second,
			frame_number, caption_time.hour, caption_time.minute,
			caption_time.second, caption_time.millisecond);
	}

#if MORE_DEBUG
	LOG("MCC Encoding %d byte packet at time: %02d:%02d:%02d;%02d", cc_count,
		caption_time.hour, caption_time.minute, caption_time.second,
		caption_time.frame);
#endif

	uint8 *w_boilerplate_buffer = add_boilerplate(enc_ctx, cc_data, cc_count,
												  dec_ctx->current_frame_rate);
	uint16 w_boilerplate_buff_size = ((cc_count * 3) + 16);

	ms_to_frame(enc_ctx, &caption_time, dec_ctx->current_frame_rate,
				enc_ctx->force_dropframe);

	uint16 num_chars_needed =
		count_compressed_chars(w_boilerplate_buffer, w_boilerplate_buff_size);

#if MORE_DEBUG
	LOG("With CDP Boiler Plate %d byte packet at time: %02d:%02d:%02d;%02d "
		"requires %d bytes compressed",
	...
	sprintf(compressed_data_buffer, "%02d:%02d:%02d:%02d\t", caption_time.hour,
			caption_time.minute, caption_time.second, caption_time.frame);

	compress_data(w_boilerplate_buffer, w_boilerplate_buff_size,
				  (uint8 *)&compressed_data_buffer[12]);
	free(w_boilerplate_buffer);

#if MORE_DEBUG
	LOG("Writing Compressed %d byte packet at time: %02d:%02d:%02d;%02d",
		(num_chars_needed + 13), caption_time.hour, caption_time.minute,
		caption_time.second, caption_time.frame);
#endif

	strcat(compressed_data_buffer, "\n");

	write(enc_ctx->out->fh, compressed_data_buffer,
		  strlen(compressed_data_buffer));
...
static void generate_mcc_header(int fh, int fr_code, int dropframe_flag)
{
	...
	ASSERT(tm.tm_wday < 7);
	ASSERT(tm.tm_mon < 12);
	sprintf(date_str, "Creation Date=%s, %s %d, %d\n", DayOfWeekStr[tm.tm_wday],
			MonthStr[tm.tm_mon], tm.tm_mday, tm.tm_year + 1900);
	...
	write_string(fh, "//   1. This ANC Transfer file format is used on an "
					 "as-is basis and no warranty is given, and\n");
	write_string(fh, "//   2. This entire descriptive information text is "
					 "included in a generated .mcc file.\n");
	write_string(fh, "//\n// General file format:\n");
	write_string(fh, "//   HH:MM:SS:FF(tab)[Hexadecimal ANC data in groups of "
					 "2 characters]\n");
	write_string(fh, "//     Hexadecimal data starts with the Ancillary Data "
					 "Packet DID (Data ID defined in S291M)\n");
	write_string(fh, "//       and concludes with the Check Sum following the "
					 "User Data Words.\n");
	write_string(fh, "//     Each time code line must contain at most one "
					 "complete ancillary data packet.\n");
	write_string(fh, "//     To transfer additional ANC Data successive lines "
					 "may contain identical time code.\n");
	write_string(
		fh, "//     Time Code Rate=[24, 25, 30, 30DF, 50, 60, 60DF]\n//\n");
	write_string(fh, "//   ANC data bytes may be represented by one ASCII "
					 "character according to the following schema:\n");
...
static void ms_to_frame(struct encoder_ctx *ctx,
						...
						int dropframe_flag)
{
	int64 actual_time_in_ms =
		(((caption_time_ptr->hour * 3600) + (caption_time_ptr->minute * 60) +
	...
	if ((dropframe_flag == CCX_TRUE) && (ctx->next_caption_time.second == 0) &&
		(ctx->next_caption_time.frame == 0) &&
		((ctx->next_caption_time.minute % 10) != 0))
	{
	...
	if (delta_in_ms > 1000)
	{
		LOG("ERROR: Larger than expected delta in Caption Time Conversion: "
			"%lld, DF%d, %dfps",
							  ...
							  int cc_count, int fr_code)
{
	...
	//   This function encodes the Ancillary Data (ANC) Packet, which wraps the
	//   Caption Distribution Packet (CDP), including the Closed Captioning Data
	//   (ccdata_section) as described in the CEA-708 Spec. Below is the list of
	//   specs that were leveraged for this encode:
	//
	//   SMPTE ST 334-1 - Vertical Ancillary Data Mapping of Caption Data and
	//   Other Related Data
	//                    (Specifically: SMPTE ST 334-1:2015 - Revision of SMPTE
	//                    334-1-2007)
	//   SMPTE ST 334-2 - Caption Distribution Packet (CDP) Definition
	//                    (Specifically: SMPTE ST 334-2:2015 - Revision of SMPTE
	//                    334-2-2007)
	//   CEA-708-D - CEA Standard - Digital Television (DTV) Closed Captioning -
	//   August 2008
	...
	buff_ptr[7] = 0x43; // Timecode not Present; Service Info not Present;
						// Captions Present
...
static uint16 count_compressed_chars(uint8 *data_ptr, uint16 num_elements)
{
	...
	while (num_elements > 0)
	{
		...
		{
		case 0xFA:
			if ((num_elements >= 3) && (data_ptr[1] == 0x00) &&
				(data_ptr[2] == 0x00))
			{
				uint8 numFaoos = 0;
				while ((num_elements >= 3) && (data_ptr[0] == 0xFA) &&
		...
		case 0xFC:
		case 0xFD:
			if ((num_elements >= 3) && (data_ptr[1] == 0x80) &&
				(data_ptr[2] == 0x80))
			{
				data_ptr = &data_ptr[3];
				num_elements = num_elements - 3;
		...
		case 0x61:
			...
			break;
		case 0xE1:
			if ((num_elements >= 4) && (data_ptr[1] == 0x00) &&
				(data_ptr[2] == 0x00) && (data_ptr[3] == 0x00))
			{
				data_ptr = &data_ptr[4];
...
static ccx_mcc_caption_time convert_to_caption_time(LLONG mstime)
{
	...
	retval.millisecond =
		(mstime -
		 1000 * (retval.second + 60 * (retval.minute + 60 * retval.hour)));
