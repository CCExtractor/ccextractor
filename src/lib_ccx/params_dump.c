...
void params_dump(struct lib_ccx_ctx *ctx)
{
	...
	case CCX_DS_FILE:
		...
		for (int i = 0; i < ctx->num_input_files; i++)
			mprint("%s%s", ctx->inputfile[i],
				   i == (ctx->num_input_files - 1) ? "" : ", ");
		break;
	case CCX_DS_STDIN:
		mprint("stdin");
		break;
	case CCX_DS_NETWORK:
		if (ccx_options.udpsrc != NULL)
			mprint("Network, %s@%s:%d", ccx_options.udpsrc, ccx_options.udpaddr,
				   ccx_options.udpport);
	...
	if (ccx_options.settings_dtvcc.enabled)
	{
		...
		if (ccx_options.settings_dtvcc.active_services_count ==
	...
	mprint("] ");
	mprint("[Debug: %s] ",
		   (ccx_options.debug_mask & CCX_DMT_VERBOSE) ? "Yes" : "No");
	mprint("[Buffer input: %s]\n", ccx_options.buffer_input ? "Yes" : "No");
	mprint("[Use pic_order_cnt_lsb for H.264: %s] ",
		   ccx_options.usepicorder ? "Yes" : "No");
	...
	mprint("[Add font color data: %s] ",
		   ccx_options.nofontcolor ? "No" : "Yes");
	...
	{
		mprint("%s", ccx_options.enc_cfg.sentence_cap
						 ? "Yes, but only built-in words"
						 : "No");
	}
	mprint("]");
	...
	if (ccx_options.extraction_end.set == 0)
		...
		mprint("%02d:%02d:%02d", ccx_options.extraction_end.hh,
			   ccx_options.extraction_end.mm, ccx_options.extraction_end.ss);
	mprint("]\n");
	mprint("[Live stream: ");
	...
	mprint("[Start credits text: %s]\n",
		   ccx_options.enc_cfg.start_credits_text
			   ? ccx_options.enc_cfg.start_credits_text
			   : "None");
	if (ccx_options.enc_cfg.start_credits_text)
	{
		mprint(
			"Start credits time: Insert between [%ld] and [%ld] seconds\n",
		...
		mprint("                    Display for at least [%ld] and at most "
			   "[%ld] seconds\n",
			   (long)(ccx_options.enc_cfg.startcreditsforatleast.time_in_ms /
					  1000),
			   (long)(ccx_options.enc_cfg.startcreditsforatmost.time_in_ms /
					  1000));
	}
	if (ccx_options.enc_cfg.end_credits_text)
	{
		mprint("End credits text: [%s]\n",
			   ccx_options.enc_cfg.end_credits_text
				   ? ccx_options.enc_cfg.end_credits_text
				   : "None");
		mprint(
			"                    Display for at least [%ld] and at most [%ld] "
			"seconds\n",
...
void print_file_report(struct lib_ccx_ctx *ctx)
{
	...
	list_for_each_entry(program, &demux_ctx->cinfo_tree.pg_stream, pg_stream,
						struct cap_info)
	{
		...
		if (dec_ctx->in_bufferdatatype == CCX_PES &&
			(info->stream == CCX_SM_TRANSPORT ||
			 info->stream == CCX_SM_PROGRAM || info->stream == CCX_SM_ASF ||
