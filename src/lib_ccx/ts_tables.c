...
int update_pinfo(struct ccx_demuxer *ctx, int pid, int program_number)
{
	...
	{
		ctx->pinfo[ctx->nb_program].got_important_streams_min_pts[i] =
			UINT64_MAX;
	}
	ctx->nb_program++;
...
int parse_PMT(struct ccx_demuxer *ctx, unsigned char *buf, int len,
			  struct program_info *pinfo)
{
	...
	/* TO-DO: We're currently parsing the PMT making assumptions that there's
	only one section with table_id=2, but that doesn't have to be the case.
	There's a sample (friends_tbs.ts) that shows a previous section with
	table_id = 0xc0. I can't find any place that says that 0xc0 (Program
	Information Table) must come before table_id = 2, so we should process
	sections in any order. Check
	https://github.com/CCExtractor/ccextractor/issues/385 for more info
	*/
	if (table_id == 0xC0)
	{
		/*
		...
		dbg_print(CCX_DMT_PMT,
				  "PMT: PROGRAM INFORMATION Table need implementation");
		// For now, just parse its length and remove it from the buffer
		unsigned c0length = (buf[1] << 8 | buf[2]) & 0xFFF; // 12 bytes
		dbg_print(CCX_DMT_PMT, "Program information table length: %u",
				  c0length);
		memmove(buf, buf + c0length + 3,
				len - c0length - 3); // First 3 bytes are for the table_id and
									 // the length, don't count
		table_id = buf[0];
		// return 0;
	}
	...
	{
		return 0; // We don't have the full section yet. We will parse again
				  // when we have it.
	}
	...
	if (!current_next_indicator &&
		pinfo->version != 0xFF) // 0xFF means we don't have one yet
		return 0;

	memcpy(pinfo->saved_section, buf, len);

	if (pinfo->analysed_PMT_once == CCX_TRUE &&
	...
	if (12 + pi_length > len)
	{
		...
		mprint("program_info_length cannot be longer than the payload_length - "
			   "skipped\n");
		return 0;
	}
	buf += 12 + pi_length;
	...
	unsigned stream_data =
		section_length - 9 - pi_length - 4; // prev. bytes and CRC
	...
	dbg_print(CCX_DMT_PMT,
			  "  PCR_PID: %u  data length: %u  payload_length: %u\n",
			  pinfo->pcr_pid, stream_data, len);

	if (!pmt_warning_shown && stream_data + 4 > len)
	{
		dbg_print(CCX_DMT_GENERIC_NOTICES,
				  "\rWarning: Probably parsing incomplete PMT, expected data "
				  "longer than available payload.\n");
		pmt_warning_shown = 1;
	}
	...
	for (unsigned i = 0; i < stream_data && (i + 4) < len; i += 5)
	{
		...
		{
			ctx->PIDs_programs[elementary_PID] =
				(struct PMT_entry *)malloc(sizeof(struct PMT_entry));
			if (ctx->PIDs_programs[elementary_PID] == NULL)
				fatal(EXIT_NOT_ENOUGH_MEMORY,
					  "Not enough memory to process PMT.");
		}
		...
		ctx->PIDs_programs[elementary_PID]->printable_stream_type =
			get_printable_stream_type(stream_type);
		dbg_print(
			CCX_DMT_VERBOSE, "%6u | %3X (%3u) | %s\n", elementary_PID,
	...
	for (unsigned i = 0; i < stream_data && (i + 4) < len; i += 5)
	{
		...
		if (!ccx_options.print_file_reports ||
			stream_type != CCX_STREAM_TYPE_PRIVATE_MPEG2 || !ES_info_length)
		{
			i += ES_info_length;
			continue;
		}
		...
		for (desc_len = 0; (buf + i + 5 + ES_info_length) > es_info;
			 es_info += desc_len)
		{
			enum ccx_mpeg_descriptor descriptor_tag =
				(enum ccx_mpeg_descriptor)(*es_info++);
			desc_len = (*es_info++);
	...
	for (unsigned i = 0; i < stream_data && (i + 4) < len; i += 5)
	{
		...
		if (stream_type == CCX_STREAM_TYPE_PRIVATE_MPEG2 && ES_info_length)
		{
			unsigned char *es_info = buf + i + 5;
			for (desc_len = 0; (buf + i + 5 + ES_info_length) > es_info;
				 es_info += desc_len)
			{
				...
				if (CCX_MPEG_DESC_DATA_COMP == descriptor_tag)
				{
					...
					if (!IS_FEASIBLE(ctx->codec, ctx->nocodec,
									 CCX_CODEC_ISDB_CC))
						continue;
					if (desc_len < 2)
						break;
					...
					if (ptr == NULL)
						break;
					update_capinfo(ctx, elementary_PID, stream_type,
								   CCX_CODEC_ISDB_CC, program_number, ptr);
				}
				if (CCX_MPEG_DSC_DVB_SUBTITLE == descriptor_tag)
				{
					...
					{
						mprint("DVB subtitles detected, OCR subsystem not "
							   "present. Use -out=spupng for graphic output\n");
						continue;
					}
#endif
					...
					if (ptr == NULL)
						break;
					update_capinfo(ctx, elementary_PID, stream_type,
								   CCX_CODEC_DVB, program_number, ptr);
					max_dif = 30;
				}
			}
		}
		else if (stream_type == CCX_STREAM_TYPE_PRIVATE_USER_MPEG2 &&
				 ES_info_length)
		{
			// if this any generally used video stream tyoe get clashed with
			// ATSC/SCTE standard then this code can go in some atsc flag
			unsigned char *es_info = buf + i + 5;
			for (desc_len = 0; (buf + i + 5 + ES_info_length) > es_info;
				 es_info += desc_len)
			{
				...
				if (CCX_MPEG_DSC_CAPTION_SERVICE == descriptor_tag)
				{
					...
					{
						dbg_print(
							CCX_DMT_PMT, "CC SERVICE %d: language (%c%c%c)",
							nb_service, es_info[1], es_info[2], es_info[3]);
						is_608 = es_info[4] >> 7;
						dbg_print(CCX_DMT_PMT, "%s",
								  is_608 ? " CEA-608" : " CEA-708");
						dbg_print(CCX_DMT_PMT, "%s",
								  is_608 ? " CEA-608" : " CEA-708");
					}
				}
				update_capinfo(ctx, elementary_PID, stream_type,
							   CCX_CODEC_ATSC_CC, program_number, NULL);
			}
		}
		...
		if (IS_FEASIBLE(ctx->codec, ctx->nocodec, CCX_CODEC_TELETEXT) &&
			ES_info_length &&
			stream_type ==
				CCX_STREAM_TYPE_PRIVATE_MPEG2) // MPEG-2 Packetized Elementary
											   // Stream packets containing
											   // private data
		{
			unsigned char *es_info = buf + i + 5;
			for (desc_len = 0; (buf + i + 5 + ES_info_length) - es_info;
				 es_info += desc_len)
			{
				...
				if (!IS_VALID_TELETEXT_DESC(descriptor_tag))
					continue;
				update_capinfo(ctx, elementary_PID, stream_type,
							   CCX_CODEC_TELETEXT, program_number, NULL);
				mprint("VBI/teletext stream ID %u (0x%x) for SID %u (0x%x)\n",
					   elementary_PID, elementary_PID, program_number,
					   program_number);
			}
		}
											   ...
											   // private data
		{
			...
			if (descriptor_tag == 0x45)
			{
				update_capinfo(ctx, elementary_PID, stream_type,
							   CCX_CODEC_ATSC_CC, program_number, NULL);
				// mprint ("VBI stream ID %u (0x%x) for SID %u (0x%x) - teletext
				// is disabled, will be processed as closed captions.\n",
				//		elementary_PID, elementary_PID, program_number,
				//program_number);
			}
		}

		if (stream_type == CCX_STREAM_TYPE_VIDEO_H264 ||
			stream_type == CCX_STREAM_TYPE_VIDEO_MPEG2)
		{
			update_capinfo(ctx, elementary_PID, stream_type, CCX_CODEC_ATSC_CC,
						   program_number, NULL);
			// mprint ("Decode captions from program %d - %s stream [0x%02x]  -
			// PID: %u\n", 	program_number , desc[stream_type], stream_type,
			//elementary_PID);
		}

		if (need_cap_info_for_pid(ctx, elementary_PID) == CCX_TRUE)
		{
			// We found the user selected CAPPID in PMT. We make a note of its
			// type and don't touch anything else
			if (stream_type >= 0x80 && stream_type <= 0xFF)
			{
				mprint("I can't tell the stream type of the manually selected "
					   "PID.\n");
				mprint("Please pass -streamtype to select manually.\n");
				fatal(EXIT_FAILURE, "-streamtype has to be manually selected.");
			}
			update_capinfo(ctx, elementary_PID, stream_type, CCX_CODEC_NONE,
						   program_number, NULL);
			continue;
		}
		...
		dbg_print(CCX_DMT_VERBOSE, "  %s stream [0x%02x]  -  PID: %u\n",
				  desc[tmp_stream_type], stream_type, elementary_PID);
		i += ES_info_length;
	}
...
void ts_buffer_psi_packet(struct ccx_demuxer *ctx)
{
	...
	// skip the packet if the adaptation field length or payload length are out
	// of bounds or broken
	if (adaptation_field_length > 184 || payload_length > 184)
	{
		...
		dbg_print(
			...
			"\rWarning: Bad packet, adaptation field too long, skipping.\n");
	}
	...
	else if (ccounter == ctx->PID_buffers[pid]->prev_ccounter + 1 ||
			 (ctx->PID_buffers[pid]->prev_ccounter == 0x0f && ccounter == 0))
	{
		ctx->PID_buffers[pid]->prev_ccounter = ccounter;
		ctx->PID_buffers[pid]->buffer = (uint8_t *)realloc(
			ctx->PID_buffers[pid]->buffer,
			ctx->PID_buffers[pid]->buffer_length + payload_length);
		memcpy(ctx->PID_buffers[pid]->buffer +
				   ctx->PID_buffers[pid]->buffer_length,
	...
	{
		dbg_print(CCX_DMT_GENERIC_NOTICES,
				  "\rWarning: Out of order packets detected for PID: %u.\n\
       ctx->PID_buffers[pid]->prev_ccounter:%" PRIu32
				  " ctx->ctx->PID_buffers[pid]->ccounter:%" PRIu32 "\n",
				  pid, ctx->PID_buffers[pid]->prev_ccounter,
				  ctx->PID_buffers[pid]->ccounter);
	}
}
...
int parse_PAT(struct ccx_demuxer *ctx)
{
	...
	unsigned section_length =
		(((payload_start[1] & 0x0F) << 8) | payload_start[2]);
	...
	if (section_number > last_section_number) // Impossible: Defective PAT
	{
		dbg_print(CCX_DMT_PAT,
				  "Skipped defective PAT packet, section_number=%u but "
	...
	if (ctx->last_pat_payload != NULL &&
		payload_length == ctx->last_pat_length &&
	...
	{
		ctx->last_pat_payload = (unsigned char *)realloc(
			ctx->last_pat_payload,
			payload_length + 8); // Extra 8 in case memcpy copies dwords, etc
		if (ctx->last_pat_payload == NULL)
		{
			fatal(EXIT_NOT_ENOUGH_MEMORY,
				  "Not enough memory to process PAT.\n");
			return -1;
		}
	}
	...
	for (unsigned i = 0; i < programm_data; i += 4)
	{
		unsigned program_number =
			((payload_start[i] << 8) | payload_start[i + 1]);
		if (!program_number)
			continue;
	...
	for (unsigned int i = 0; i < programm_data; i += 4)
	{
		unsigned program_number =
			((payload_start[i] << 8) | payload_start[i + 1]);
		unsigned prog_map_pid =
			((payload_start[i + 2] << 8) | payload_start[i + 3]) & 0x1FFF;
		int j = 0;
		 ...
		 * so we break if program_number already exist and make j !=
		 * ctx->nb_program
		 *
		...
		for (j = 0; j < ctx->nb_program; j++)
		{
			...
			{
				if (ctx->flag_ts_forced_pn == CCX_TRUE &&
					ctx->pinfo[j].pid == CCX_UNKNOWN)
				{
	...
	{
		mprint("\nThis TS file has more than one program. These are the "
...
void process_ccx_mpeg_descriptor(unsigned char *data, unsigned length)
{
	...
	case CCX_MPEG_DSC_TELETEXT_DESCRIPTOR:
		...
		for (i = 0; i < l; i += 5)
		{
			...
			dbg_print(CCX_DMT_PMT,
					  "                 Teletext type: %s (%02X)\n",
					  (teletext_type < 6 ? txt_teletext_type[teletext_type]
	...
	default:
		if (data[0] == CCX_MPEG_DSC_REGISTRATION) // Registration descriptor,
												  // could be useful eventually
			break;
		if (data[0] ==
			CCX_MPEG_DSC_DATA_STREAM_ALIGNMENT) // Data stream alignment
												// descriptor
			break;
		...
		if (data[0] >= 0x40 && data[0] <= 0xFF) // User private
			break;
		// mprint ("Still unsupported MPEG descriptor type=%d
		// (%02X)\n",data[0],data[0]);
		break;
	}
}

void decode_service_descriptors(struct ccx_demuxer *ctx, uint8_t *buf,
								uint32_t length, uint32_t service_id)
{
	...
	while (offset + 5 < length)
	{
		...
		{ // service descriptor
			...
			{
				dbg_print(CCX_DMT_GENERIC_NOTICES,
						  "\rWarning: Invalid SDT service_provider_name_length "
						  "detected.\n");
				return;
			}
			offset +=
				service_provider_name_length; // Service provider name. Not sure
			...
			{
				// Not sure if programs can have multiple names (in different
				// encodings?) Need more samples. For now just assume the last
				// one in the loop is as good as any if there are multiple.
				if (ctx->pinfo[x].program_number == service_id &&
					service_name_length < 199)
				{
					char *s = EPG_DVB_decode_string(
						&buf[offset],
						service_name_length); // String encoding is the same as
											  // for EPG
...
void decode_SDT_services_loop(struct ccx_demuxer *ctx, uint8_t *buf,
							  uint32_t length)
{
	...
	while (offset + 5 < length)
	{
		...
		uint32_t descriptors_loop_length =
			(((buf[offset + 3] & 0x0F) << 8) | buf[offset + 4]);
		offset += 5;
		...
		decode_service_descriptors(ctx, &buf[offset], descriptors_loop_length,
								   serive_id);
...
void parse_SDT(struct ccx_demuxer *ctx)
{
	...
	payload_length =
		ctx->PID_buffers[0x11]->buffer_length - (pointer_field + 1);
	...
	// uint16_t original_network_id = ((payload_start[8]) << 8) |
	// payload_start[9];
