...
typedef struct
{
	...
	uint8_t g2_char_present[25][40]; // 0- Supplementary G2 character set NOT
									 // used at this position 1-Supplementary G2
									 // character set used at this position
...
struct TeletextCtx
{
	...
	uint8_t nonempty : 1; // produce at least one (dummy) frame
						  // uint8_t se_mode : 1; // search engine compatible
						  // mode => Uses CCExtractor's write_format uint64_t
						  // utc_refvalue; // UTC referential value => Moved to
						  // ccx_decoders_common, so can be used for other
						  // decoders (608/xds) too
	...
	// Current and previous page buffers. This is the output written to file
	// when the time comes.
	...
	// subtitle type pages bitmap, 2048 bits = 2048 possible pages in teletext
	// (excl. subpages)
	uint8_t cc_map[256];
