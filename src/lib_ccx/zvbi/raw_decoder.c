...
const _vbi_service_par
	...
	},
	{
	    ...
	    5642787,
	    5642787, /* 14/11 x FSC (color subcarrier) */
	    0x00AAAAE5,
	    0xFFFF,
	    18,
	    6,
...
void _vbi3_raw_decoder_dump(const vbi3_raw_decoder *rd,
			    FILE *fp)
{
	const vbi_sampling_par *sp;
	unsigned int i;
	...
	for (i = 0; i < ((unsigned int)sp->count[0] + (unsigned int)sp->count[1]); ++i)
	{
		fputs("  ", fp);
...
cpr1204_crc(const vbi_sliced *sliced)
{
	...
	crc = (+(sliced->data[0] << 12) + (sliced->data[1] << 4) + (sliced->data[2]));
      ...
      const uint8_t *raw)
{
	...
	{
		return vbi3_bit_slicer_slice_with_points(&job->slicer,
							 sliced->data,
							 sizeof(sliced->data),
	...
	{
		return vbi3_bit_slicer_slice(&job->slicer,
					     sliced->data,
					     sizeof(sliced->data),
					     raw);
	}
	...
	for (pat = pattern;; ++pat)
	{
		...
		if (j > 0)
		{
			...
			if (i >= (unsigned int)sp->count[0])
			{
				if (sp->synchronous && 0 != sp->start[1])
					sliced->line = sp->start[1] + i - sp->count[0];
			}
			else
			{
		...
		else if (pat == pattern)
		{
			...
			if (0 == rd->readjust)
			{
				...
				size = sizeof(*pattern) * (_VBI3_RAW_DECODER_MAX_WAYS - 1);
 ...
 */
void vbi3_raw_decoder_reset(vbi3_raw_decoder *rd)
{
	assert(NULL != rd);
		      ...
		      const _vbi_service_par *par)
{
	...
	for (field = 0; field < 2; ++field)
	{
		...
		if (0 == par->first[field] || 0 == par->last[field])
		{
		...
		if (first > 0 && sp->count[field] > 0)
		{
			...
			if ((unsigned int)par->first[field] > last || (unsigned int)par->last[field] < first)
				continue;
	...
	for (par = _vbi_service_table; par->id; ++par)
	{
		...
		for (j = 0; j < rd->n_jobs; ++j)
		{
			...
			if (0 == (id & ~VBI_SLICED_TELETEXT_B)
			    /* Field 1 and 2 */
			    || 0 == (id & ~VBI_SLICED_CAPTION_525) || 0 == (id & ~VBI_SLICED_CAPTION_625) || 0 == (id & ~(VBI_SLICED_VPS | VBI_SLICED_VPS_F2)))
				break;
		...
		if (!vbi3_bit_slicer_set_params(&job->slicer,
						sp->sp_sample_format,
						sp->sampling_rate,
						sample_offset,
						samples_per_line,
...
/* Attn: strict must be int for compatibility with libzvbi 0.2 (-1 == 0) */
vbi_service_set
vbi3_raw_decoder_set_sampling_par(vbi3_raw_decoder *rd,
				  const vbi_sampling_par *sp,
				  int strict)
{
 ...
 */
void vbi3_raw_decoder_get_sampling_par(const vbi3_raw_decoder *rd,
				       vbi_sampling_par *sp)
{
	assert(NULL != rd);
	assert(NULL != sp);
...
void vbi3_raw_decoder_set_log_fn(vbi3_raw_decoder *rd,
				 vbi_log_fn *log_fn,
				 void *user_data,
				 vbi_log_mask mask)
{
 ...
 */
void _vbi3_raw_decoder_destroy(vbi3_raw_decoder *rd)
{
	vbi3_raw_decoder_reset(rd);
 ...
 */
void vbi3_raw_decoder_delete(vbi3_raw_decoder *rd)
{
	if (NULL == rd)
		return;
