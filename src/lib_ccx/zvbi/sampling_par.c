			    ...
			    _vbi_log_hook *log)
{
	...
	if (VBI_VIDEOSTD_SET_525_60 & videostd_set)
	{
		...
		if (0 != sp->start[0] && !range_check(sp->start[0], sp->count[0], 1, 262))
			goto bad_range;

		if (0 != sp->start[1] && !range_check(sp->start[1], sp->count[1], 263, 525))
			goto bad_range;
	}
	else if (VBI_VIDEOSTD_SET_625_50 & videostd_set)
	{
		if (0 != sp->start[0] && !range_check(sp->start[0], sp->count[0], 1, 311))
			goto bad_range;

		if (0 != sp->start[1] && !range_check(sp->start[1], sp->count[1], 312, 625))
			goto bad_range;
	}
	else
	{
	...
	if (sp->interlaced && (sp->count[0] != sp->count[1] || 0 == sp->count[0]))
	{
		info(log,
...
static vbi_bool
_vbi_sampling_par_permit_service(const vbi_sampling_par *sp,
				 ...
				 _vbi_log_hook *log)
{
	...
	{
		if ((par->first[0] > 0 && unknown == (unsigned int)sp->start[0]) || (par->first[1] > 0 && unknown == (unsigned int)sp->start[1]))
		{
			info(log,
	...
	signal = par->cri_bits / (double)par->cri_rate + (par->frc_bits + par->payload) / (double)par->bit_rate;
	...
	if ((par->flags & _VBI_SP_FIELD_NUM) && !sp->synchronous)
	{
		info(log,
		     "Service 0x%08x (%s) requires "
				    ...
				    _vbi_log_hook *log)
{
	...
	{
		if (0 == (VBI_VIDEOSTD_SET_ALL & videostd_set_req) || ((VBI_VIDEOSTD_SET_525_60 & videostd_set_req) && (VBI_VIDEOSTD_SET_625_50 & videostd_set_req)))
		{
			warning(log,
				"Ambiguous videostd_set 0x%lx.",
	...
	for (par = _vbi_service_table; par->id; ++par)
	{
		...
		if (0 == videostd_set_req)
		{
			...
			if (0 == (set & ~VBI_VIDEOSTD_SET_525_60) || 0 == (set & ~VBI_VIDEOSTD_SET_625_50))
		...
		for (i = 0; i < 2; ++i)
			...
			{
				sp->start[i] = MIN((unsigned int)sp->start[i],
						   (unsigned int)par->first[i]);
				sp->count[i] = MAX((unsigned int)sp->start[i] + sp->count[i],
