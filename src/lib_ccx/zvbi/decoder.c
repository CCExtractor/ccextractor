			 ...
			 vbi_modulation modulation, vbi_pixfmt fmt)
{
	...
	slicer->cri_bytes = raw_samples - ((long long)sampling_rate * (payload + frc_bits)) / bit_rate;
	...
	switch (modulation)
	{
		...
		case VBI_MODULATION_NRZ_LSB:
			slicer->phase_shift = (int)(sampling_rate * 256.0 / cri_rate * .5 + sampling_rate * 256.0 / bit_rate * .5 + 128);
			break;
		...
		case VBI_MODULATION_BIPHASE_LSB:
			...
			slicer->phase_shift = (int)(sampling_rate * 256.0 / cri_rate * .5 + sampling_rate * 256.0 / bit_rate * .25 + 128);
			break;
	}
}
			    ...
			    unsigned int *count)
{
	...
	{
		if ((rd->start[0] == start[0]) && (rd->start[1] == start[1]) && (rd->count[0] == (int)count[0]) && (rd->count[1] == (int)count[1]))
		{
			return;
		}
			       ...
			       int strict)
{
	...
	{
		service_set = vbi_sampling_par_check_services((vbi_sampling_par *)rd, service_set, strict);
	}
			   ...
			   int *max_rate)
{
	...
	{
		service_set = vbi_sampling_par_from_services((vbi_sampling_par *)rd,
