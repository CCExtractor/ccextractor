...
struct encoder_ctx
{
  ...
  /* Keep output file closed when not actually writing to it and start over each
   * time (add headers, etc) */
  ...
  int generates_file;
  struct ccx_encoders_transcript_format* transcript_settings; // Keeps the
                                                              // settings for
                                                              // generating
                                                              // transcript
                                                              // output files.
  ...
  int splitbysentence;
  LLONG sbs_newblock_start_time; // Used by the split-by-sentence code to know
                                 // when the current block starts...
...
int write_cc_bitmap_as_transcript(struct cc_subtitle* sub,
                                  struct encoder_ctx* context);
int write_cc_bitmap_to_sentence_buffer(struct cc_subtitle* sub,
                                       struct encoder_ctx* context);
