...
struct ccx_common_logging_t {
  ...
  void (*fatal_ftn)(int exit_code, const char *fmt,
                    ...);  // Used when an unrecoverable error happens. This
                           // should log output/save the error and then exit the
                           // program.
  void (*debug_ftn)(LLONG mask, const char *fmt, ...);  // Used to process debug
                                                        // output. Mask can be
                                                        // ignored (custom set
                                                        // by debug_mask).
  void (*log_ftn)(const char *fmt, ...);  // Used to print things. Replacement
                                          // of standard printf, to allow more
                                          // control.
  void (*gui_ftn)(enum ccx_common_logging_gui message_type,
                  ...);  // Used to display things in a gui (if appropriate). Is
                         // called with the message_type and appropriate
