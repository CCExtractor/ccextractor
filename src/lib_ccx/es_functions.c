...
static int es_video_sequence(struct encoder_ctx *enc_ctx,
...
static int sequence_header(struct lib_cc_decode *ctx,
						   struct bitstream *esstream);
static int sequence_ext(struct lib_cc_decode *ctx, struct bitstream *esstream);
static int read_gop_info(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
						 struct bitstream *esstream, struct cc_subtitle *sub);
static int gop_header(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
					  struct bitstream *esstream, struct cc_subtitle *sub);
static int read_pic_info(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
						 struct bitstream *esstream, struct cc_subtitle *sub);
static int pic_header(struct lib_cc_decode *ctx, struct bitstream *esstream);
static int pic_coding_ext(struct lib_cc_decode *ctx,
						  struct bitstream *esstream);
static int read_eau_info(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
						 struct bitstream *esstream, int udtype,
						 struct cc_subtitle *sub);
static int extension_and_user_data(struct encoder_ctx *enc_ctx,
								   struct lib_cc_decode *ctx,
								   struct bitstream *esstream, int udtype,
								   struct cc_subtitle *sub);
...
size_t process_m2v(struct encoder_ctx *enc_ctx, struct lib_cc_decode *dec_ctx,
				   unsigned char *data, size_t length, struct cc_subtitle *sub)
{
...
static uint8_t search_start_code(struct bitstream *esstream)
{
	...
	while (1)
	{
		...
		tstr = (unsigned char *)memchr(tstr, 0x00, esstream->end - tstr);
		if (tstr == NULL)
		{
							 ...
							 struct cc_subtitle *sub)
{
	...
	do
	{
		...
		{
			if (!read_eau_info(enc_ctx, dec_ctx, esstream,
							   dec_ctx->saw_seqgoppic - 1, sub))
			{
				if (esstream->error)
					dec_ctx->no_bitstream_error = 0;
				return 0;
		...
		if (startcode == 0xBA)
		{
			mprint("\nFound PACK header in ES data.  Probably wrong stream "
				   "mode!\n");
		}
		else
		{
...
static int read_seq_info(struct lib_cc_decode *ctx, struct bitstream *esstream)
{
	...
	if (next_u32(esstream) != 0xB3010000) // LSB first (0x000001B3)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In read_seq_info: next_u32(esstream) != 0xB3010000. Please file "
						   ...
						   struct bitstream *esstream)
{
	...
	if (read_u32(esstream) != 0xB3010000) // LSB first (0x000001B3)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In sequence_header: read_u32(esstream) != 0xB3010000. Please "
		...
		frame_rate != ctx->current_frame_rate)
	{
		...
		// whole sequence info.
		if (vert_size >= 288 && vert_size <= 1088 && hor_size >= 352 &&
			hor_size <= 1920 &&
			(hor_size * 100) / vert_size >=
				(352 * 100) / 576 && // The weird *100 is to avoid using floats
			hor_size / vert_size <= 2 &&
			frame_rate > 0 && frame_rate < 9 && aspect_ratio > 0 &&
			aspect_ratio < 5)
		{
			mprint("\n\nNew video information found");
			mprint("\n");
			mprint("[%u * %u] [AR: %s] [FR: %s]", hor_size, vert_size,
...
// point to after the problem, in case we run out of data the bitstream
// will point to where we want to restart after getting more.
static int read_gop_info(struct encoder_ctx *enc_ctx,
						 struct lib_cc_decode *dec_ctx,
						 struct bitstream *esstream, struct cc_subtitle *sub)
{
	debug("Read GOP Info\n");
	...
	if (next_u32(esstream) != 0xB8010000) // LSB first (0x000001B8)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In read_gop_info: next_u32(esstream) != 0xB8010000. Please file "
			  "a bug report on GitHub.\n");
...
// estream->pos is advanced. Data is only processed if esstream->error
// is FALSE, parsing can set esstream->error to TRUE.
static int gop_header(struct encoder_ctx *enc_ctx,
					  struct lib_cc_decode *dec_ctx, struct bitstream *esstream,
					  struct cc_subtitle *sub)
{
	debug("GOP header\n");
	...
	if (read_u32(esstream) != 0xB8010000) // LSB first (0x000001B8)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In gop_header: read_u32(esstream) != 0xB8010000. Please file a "
	...
	if (gop_accepted(&gtc))
	{
		...
		if ((dec_ctx->current_pulldownfields > 0) !=
			...
			first_gop_time.inited)
		{
			...
			mprint("  (old) %s", print_mstime_static(gop_time.ms));
			mprint("  +  %s (%uF)",
		...
		if (first_gop_time.inited == 0)
		{
			...
			else
			{
				dec_ctx->timing->fts_fc_offset =
					(LLONG)((total_frames_count + 1) * 1000.0 / current_fps);
			...
			dbg_print(CCX_DMT_TIME,
					  ...
					  gtc.time_code_hours, gtc.time_code_minutes,
		...
		else
		{
			...
			// for DVD captions.
			fts_at_gop_start =
				get_fts_max(dec_ctx->timing) + (LLONG)(1000.0 / current_fps);
		}
...
// point to after the problem, in case we run out of data the bitstream
// will point to where we want to restart after getting more.
static int read_pic_info(struct encoder_ctx *enc_ctx,
						 struct lib_cc_decode *dec_ctx,
						 struct bitstream *esstream, struct cc_subtitle *sub)
{
	...
	if (next_u32(esstream) != 0x00010000) // LSB first (0x00000100)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In read_pic_info: next_u32(esstream) != 0x00010000. Please file "
			  "a bug report on GitHub.\n");
	...
	if (dec_ctx->picture_coding_type == CCX_FRAME_TYPE_I_FRAME ||
		dec_ctx->picture_coding_type == CCX_FRAME_TYPE_P_FRAME)
	{
		if (((dec_ctx->picture_structure != 0x1) &&
			 (dec_ctx->picture_structure != 0x2)) ||
	...
	/*
	dbg_print(CCX_DMT_VIDES, "  frametype: %d (%c) t:%d r:%d p:%d",
	dec_ctx->picture_coding_type,
		frame_type_to_char[dec_ctx->picture_coding_type],
	dec_ctx->top_field_first, dec_ctx->repeat_first_field,
	dec_ctx->progressive_frame); dbg_print(CCX_DMT_VIDES, "  FTS: %lld  (%s)\n",
	dec_ctx->timing->current_pts, print_mstime_static(get_fts(dec_ctx->timing,
	dec_ctx->current_field)));
		*/
	...
	if (dec_ctx->temporal_reference == 0)
	{
		...
		if (dec_ctx->saw_gop_header)
		{
			...
			frames_since_ref_time =
				dec_ctx->frames_since_last_gop; // Should this be 0?
		}
		else
		{
	...
	if (!dec_ctx->saw_gop_header &&
		dec_ctx->picture_coding_type == CCX_FRAME_TYPE_I_FRAME)
	{
	...
	if (dec_ctx->repeat_first_field)
	{
		...
		if (dec_ctx->current_progressive_sequence ||
			!(dec_ctx->total_pulldownfields % 2))
			extraframe = 1;
		if (dec_ctx->current_progressive_sequence && dec_ctx->top_field_first)
			extraframe = 2;
		dbg_print(CCX_DMT_VIDES,
				  "Pulldown: total pd fields: %d - %d extra frames\n",
...
static int pic_header(struct lib_cc_decode *ctx, struct bitstream *esstream)
{
	...
	if (read_u32(esstream) != 0x00010000) // LSB first (0x00000100)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In pic_header: read_u32(esstream) != 0x00010000. Please file a "
			  "bug report on GitHub.\n");

	ctx->temporal_reference = (int)read_bits(esstream, 10);
	ctx->picture_coding_type = (enum ccx_frame_type)read_bits(esstream, 3);

	if (ctx->picture_coding_type ==
		1) // Write I-Frame in ffprobe format for easy comparison
	{
		ctx->num_key_frames++;
...
// point to after the problem, in case we run out of data the bitstream
// will point to where we want to restart after getting more.
static int read_eau_info(struct encoder_ctx *enc_ctx,
						 struct lib_cc_decode *dec_ctx,
						 struct bitstream *esstream, int udtype,
						 struct cc_subtitle *sub)
{
	...
	if (!tst || tst[0] != 0x00 || tst[1] != 0x00 || tst[2] != 0x01 ||
		...
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In read_eau_info: Impossible values for tst. Please file a bug "
			  "report on GitHub.\n");
