...
const uint8_t crop_tab[256 + 2 * MAX_NEG_CROP] = {
	times256(0x00), 0x00, 0x01, 0x02, 0x03, 0x04,		   0x05, 0x06, 0x07,
...
static __inline int get_bits_count(const GetBitContext *s) { return s->index; }
...
static __inline unsigned int get_bits(GetBitContext *s, int n)
{
	...
	re_index =
		((re_size_plus8 < re_index + (n)) ? (re_size_plus8) : (re_index + (n)));
										 ...
										 int top_bottom, int non_mod)
{
	...
	while (!parseerror && buf < buf_end)
	{
		...
		{
			mprint("In dvbsub_parse_pixel_data_block(): Invalid object "
				   "location. %d-%d %d-%d %02x\n",
		...
		case 0x10:
			...
			if (x_pos < 0)
			{
				mprint("In dvbsub_parse_pixel_data_block(): "
					   "dvbsub_read_2bit_string() returned error.\n");
				parseerror = 1;
				goto exitfunc;
			}
			break;
		case 0x11:
			if (region->depth < 4)
			{
				mprint("In dvbsub_parse_pixel_data_block(): 4-bit pixel string "
					   "in %d-bit region.\n",
					   region->depth);
				parseerror = 1;
				goto exitfunc;
			...
			{
				mprint("In dvbsub_parse_pixel_data_block(): "
					   "dvbsub_read_4bit_string() returned error.\n");
				parseerror = 1;
				goto exitfunc;
			}
			break;
		case 0x12:
			if (region->depth < 8)
			{
				mprint("In dvbsub_parse_pixel_data_block(): 8-bit pixel string "
					   "in %d-bit region.\n",
					   region->depth);
				return -1;
			}
			...
			{
				mprint("In dvbsub_parse_pixel_data_block(): "
					   "dvbsub_read_8bit_string() returned error.\n");
				parseerror = 1;
				goto exitfunc;
			}
			break;
		...
		case 0xf0:
			...
			break;
		default:
			mprint("In dvbsub_parse_pixel_data_block(): Unknown/unsupported "
				   "pixel block 0x%x\n",
				   *(buf - 1));
			/* no break */
		}
									   ...
									   int buf_size)
{
	...
	if (coding_method == 0)
	{
		...
		{
			mprint(
				"dvbsub_parse_object_segment(): Field data size too large\n");
			return -1;
		}
			 ...
			 display = display->object_list_next)
		{
			...
			if (dvbsub_parse_pixel_data_block(dvb_ctx, display, block,
											  ...
											  non_modifying_color))
			{
				mprint("dvbsub_parse_object_segment(): Something went wrong. "
					   "Giving up on block (1).\n");
									 ...
									 int buf_size)
{
	...
	if (clut->version != version)
	{
		...
		while (buf + 4 < buf_end)
		{
			...
			{
				mprint(
					"dvbsub_parse_clut_segment(): Invalid clut depth 0x%x.\n",
					*buf);
				return -1;
			}
			...
			{
				mprint("dvbsub_parse_clut_segment(): More than one bit level "
					   "marked: %x\n",
					   depth);
			}
										...
										int buf_size)
{
	...
	{
		mprint(
			" dvbsub_parse_region_segment(): [buf_size < 10, leaving region] ");
		return;
	}
													...
													int buf_size)
{
	...
	{
		display_def =
			(struct DVBSubDisplayDefinition *)malloc(sizeof(*display_def));
...
static int write_dvb_sub(struct lib_cc_decode *dec_ctx, struct cc_subtitle *sub)
{
	...
	// TODO: if different regions have different cluts, only the last one will
	// be saved. Don't know if it will affect anything.

	// The first loop, to determine the size of the whole subtitle (made up of
	...
	{
		mprint(
			"write_dvb_sub: failed to alloc memory, need %d * %d = %d bytes\n",
			width, height, width * height);
		return -1;
	}
	...
	for (display = ctx->display_list; display; display = display->next)
	{
		...
		for (int y = 0; y < region->height; y++)
		{
			for (int x = 0; x < region->width; x++)
			{
				...
				{
					mprint("write_dvb_sub(): Offset %d (out of bounds!) "
						   "ignored.\n",
						   offset);
					mprint("  Formula: offset=((y + y_off) * width) + x_off + "
						   "x\n");
	...
	sub->nb_data =
		1; // Set nb_data to 1 since we have merged the images into one image.
								   ...
								   struct cc_subtitle *sub)
{
	if (!enc_ctx)
		return;
	if (enc_ctx->write_previous) // this condition is used for the first
								 // subtitle - write_previous will be 0 first so
								 // we don't encode a non-existing previous sub
	{
		enc_ctx->prev->last_string = NULL; // Reset last recognized sub text
		sub->prev->end_time =
			(dec_ctx->timing->current_pts - dec_ctx->timing->min_pts) /
			(MPEG_CLOCK_FREQ /
			 1000); // we set the end time of the previous sub the current pts
		encode_sub(enc_ctx->prev, sub->prev); // we encode it

		enc_ctx->last_string =
			enc_ctx->prev->last_string; // Update last recognized string (used
										// in Matroska)
		enc_ctx->prev->last_string = NULL;

		enc_ctx->srt_counter =
			enc_ctx->prev
				->srt_counter; // for dvb subs we need to update the current srt
							   // counter because we always encode the previous
							   // subtitle (and the counter is increased for the
							   // previous context)
		enc_ctx->prev_start = enc_ctx->prev->prev_start;
		sub->prev->got_output = 0;
		if (enc_ctx->write_format == CCX_OF_WEBVTT)
		{ // we already wrote header, but since we encoded last sub, we must
		  // prevent multiple headers in future
	...
	memcpy(dec_ctx->prev->private_data, dec_ctx->private_data,
		   sizeof(struct DVBSubContext));
	...
	sub->prev->start_time =
		(dec_ctx->timing->current_pts - dec_ctx->timing->min_pts) /
		(MPEG_CLOCK_FREQ /
		 1000); // we set the start time of the previous sub the current pts

	write_dvb_sub(
		dec_ctx->prev,
		sub->prev); // we write the current dvb sub to update decoder context
	enc_ctx->write_previous =
		1; // we update our boolean value so next time the program reaches this
		   // block of code, it encodes the previous sub
#ifdef ENABLE_OCR
	if (sub->prev)
	{
		...
		dbg_print(
			...
			sub->prev,
			content_prev
				? (content_prev->ocr_text ? content_prev->ocr_text : "NULL OCR")
				: "NULL DATA",
	...
	dbg_print(CCX_DMT_DVB,
			  ...
			  sub,
			  content ? (content->ocr_text ? content->ocr_text : "NULL OCR")
					  : "NULL DATA",
 ...
 * @param buf        buffer containing segment data, first sync byte need to
 * 0x0f. does not include data_identifier and subtitle_stream_id.
	...
	while (p_end - p >= 6 && *p == 0x0f)
	{
		...
		if (p_end - p < segment_length)
		{
			mprint("dvbsub_decode: incomplete, broken or empty packet, "
				   "remaining bytes=%d, segment_length=%d\n",
		...
		{
			// debug traces
			dbg_print(CCX_DMT_DVB, "DVBSUB - PTS: %" PRId64 ", ",
			...
			case DVBSUB_PAGE_SEGMENT:
				dbg_print(CCX_DMT_DVB,
						  "(DVBSUB_PAGE_SEGMENT), SEGMENT LENGTH: %d",
						  segment_length);
			...
			case DVBSUB_REGION_SEGMENT:
				dbg_print(CCX_DMT_DVB,
						  "(DVBSUB_REGION_SEGMENT), SEGMENT LENGTH: %d",
						  segment_length);
			...
			case DVBSUB_CLUT_SEGMENT:
				dbg_print(CCX_DMT_DVB,
						  "(DVBSUB_CLUT_SEGMENT), SEGMENT LENGTH: %d",
						  segment_length);
			...
			case DVBSUB_OBJECT_SEGMENT:
				dbg_print(CCX_DMT_DVB,
						  "(DVBSUB_OBJECT_SEGMENT), SEGMENT LENGTH: %d",
						  segment_length);
			...
			case DVBSUB_DISPLAYDEFINITION_SEGMENT:
				dbg_print(
					CCX_DMT_DVB,
					"(DVBSUB_DISPLAYDEFINITION_SEGMENT), SEGMENT LENGTH: %d",
					segment_length);
				dvbsub_parse_display_definition_segment(ctx, p, segment_length);
				break;
			case DVBSUB_DISPLAY_SEGMENT: // when we get a display segment, we
										 // save the current page
				dbg_print(CCX_DMT_DVB,
						  "(DVBSUB_DISPLAY_SEGMENT), SEGMENT LENGTH: %d",
						  segment_length);
 ...
 *
 * data pointer to this function should be after description length and
 * description tag is parsed
 *
 * @param cfg preallocated dvb_config structure where parsed description will be
 * stored,Not to be NULL
 *
						  ...
						  unsigned int len)
{
	...
	/*
		Abhinav95: The way this function is called right now, only
	   cfg->lang_index[0] gets populated. E.g. for 3 stream languages, it will
	   be called 3 times, and set the language index in only the first element
	   each time. This works with the current state of the DVB code.
	*/
