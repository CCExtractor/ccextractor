...
void init_options(struct ccx_s_options *options)
{
	...
	options->settings_608.default_color =
		COL_TRANSPARENT; // Defaults to transparent/no-color.
	...
	options->use_gop_as_pts =
		0; // Use GOP instead of PTS timing (0=do as needed, 1=always, -1=never)
	options->fix_padding =
		0; // Replace 0000 with 8080 in HDTV (needed for some cards)
	options->gui_mode_reports =
		0; // If 1, output in stderr progress updates so the GUI can grab them
	options->no_progress_bar =
		0; // If 1, suppress the output of the progress to stdout
	options->enc_cfg.sentence_cap = 0; // FIX CASE? = Fix case?
	options->enc_cfg.splitbysentence =
		0; // Split text into complete sentences and prorate time?
	...
	options->investigate_packets =
		0; // Look for captions in all packets when everything else fails
	options->fullbin = 0; // Disable pruning of padding cc blocks
	options->nosync = 0;  // Disable syncing
	options->hauppauge_mode =
		0; // If 1, use PID=1003, process specially and so on
	options->wtvconvertfix = 0; // Fix broken Windows 7 conversion
	options->wtvmpeg2 = 0;
	options->auto_myth = 2; // 2=auto
	/* MP4 related stuff */
	options->mp4vidtrack =
		0; // Process the video track even if a CC dedicated track exists.
	options->extract_chapters = 0; // By default don't extract chapters.
	/* General stuff */
	options->usepicorder =
		0; // Force the use of pic_order_cnt_lsb in AVC/H.264 data streams
	options->xmltv = 0; // 1 = full output. 2 = live output. 3 = both
	options->xmltvliveinterval = 10; // interval in seconds between writing
									 // xmltv output files in live mode
	options->xmltvoutputinterval =
		0; // interval in seconds between writing xmltv full file output
	...
	options->ocr_oem =
		0; // By default, set Tesseract OEM mode OEM_TESSERACT_ONLY (0)
	...
	options->debug_mask =
		CCX_DMT_GENERIC_NOTICES; // dbg_print will use this mask to print or
								 // ignore different types
	options->debug_mask_on_debug =
		CCX_DMT_VERBOSE; // If we're using temp_debug to enable/disable debug
						 // "live", this is the mask when temp_debug=1
	/* Networking */
	options->udpsrc = NULL;
	options->udpaddr = NULL;
	options->udpport =
		0; // Non-zero => Listen for UDP packets on this port, no files.
	...
	options->demux_cfg.ts_autoprogram =
		0; // Try to find a stream with captions automatically (no -pn needed)
	options->demux_cfg.ts_cappids[0] =
		0; // PID for stream that holds caption information
	options->demux_cfg.nb_ts_cappid = 0;
	options->demux_cfg.ts_forced_program =
		-1; // Specific program to process in TS files, if
			// ts_forced_program_selected==1
	options->demux_cfg.ts_forced_program_selected = 0;
	options->demux_cfg.ts_datastreamtype =
		CCX_STREAM_TYPE_UNKNOWNSTREAM; // User WANTED stream type (i.e. use the
									   // stream that has this type)
	options->demux_cfg.ts_forced_streamtype =
		CCX_STREAM_TYPE_UNKNOWNSTREAM; // User selected (forced) stream type

	options->enc_cfg.autodash =
		0; // Add dashes (-) before each speaker automatically?
	...
	options->enc_cfg.force_dropframe =
		0; // Assume No Drop Frame for MCC Encode.
	...
	memset(options->settings_dtvcc.services_enabled, 0,
		   CCX_DTVCC_MAX_SERVICES *
			   sizeof(options->settings_dtvcc.services_enabled[0]));
	...
	stringztoms(DEF_VAL_STARTCREDITSNOTBEFORE,
				&options->enc_cfg.startcreditsnotbefore);
	stringztoms(DEF_VAL_STARTCREDITSNOTAFTER,
				&options->enc_cfg.startcreditsnotafter);
	stringztoms(DEF_VAL_STARTCREDITSFORATLEAST,
				&options->enc_cfg.startcreditsforatleast);
	stringztoms(DEF_VAL_STARTCREDITSFORATMOST,
				&options->enc_cfg.startcreditsforatmost);
	stringztoms(DEF_VAL_ENDCREDITSFORATLEAST,
				&options->enc_cfg.endcreditsforatleast);
	stringztoms(DEF_VAL_ENDCREDITSFORATMOST,
				&options->enc_cfg.endcreditsforatmost);

#ifdef PYTHON_API
	options->python_param_count = 0;
	options->python_params =
		malloc(options->python_param_count * sizeof *options->python_params);
#endif
}
