...
struct ccx_s_teletext_config {
  ...
  // uint8_t se_mode : 1; // search engine compatible mode => Uses CCExtractor's
  // write_format
  // uint64_t utc_refvalue; // UTC referential value => Moved to
  ...
  int levdistmincnt, levdistmaxpct;  // Means 2 fails or less is "the same", 10%
...
// es_functions.c
size_t process_m2v(struct lib_cc_decode *ctx, unsigned char *data,
                   size_t length, struct cc_subtitle *sub);

extern unsigned top_field_first;

// es_userdata.c
int user_data(struct lib_cc_decode *ctx, struct bitstream *ustream, int udtype,
...
void store_hdcc(struct lib_cc_decode *ctx, unsigned char *cc_data, int cc_count,
                int sequence_number, LLONG current_fts_now,
                struct cc_subtitle *sub);
