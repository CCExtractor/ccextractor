...
struct ccx_s_teletext_config
{
  ...
  uint16_t page;                /* teletext page containing cc we want to
                                   filter */
  uint16_t tid;                 /* 13-bit packet ID for teletext stream */
  double offset;                /* time offset in seconds */
  uint8_t bom:1;                /* print UTF-8 BOM characters at the
                                   beginning of output */
  uint8_t nonempty:1;           /* produce at least one (dummy) frame */

  /* 
   * uint8_t se_mode : 1;
   * // search engine compatible mode => Uses CCExtractor's ;
   * write_format ;
   * uint64_t utc_refvalue;
   * // UTC referential value => Moved to ;
   * ccx_decoders_common, so can be used for other decoders (608/xds) too
   */
  uint16_t user_page;           /* Page selected by user, which MIGHT be
                                   different to */

  /* 'page' depending on autodetection stuff */
  int levdistmincnt, levdistmaxpct;     /* Means 2 fails or less is "the
                                           same", 10% */

  /* or less is also "the same" */
  struct ccx_boundary_time extraction_start, extraction_end;    /* Segment we 
                                                                   actually
                                                                   process */
  enum ccx_output_format write_format;  /* 0=Raw, 1=srt, 2=SMI */
  int gui_mode_reports;         /* If 1, output in stderr progress updates so 
                                   the GUI */
...
struct lib_ccx_ctx
{
  ...
  /* 
   * int hex_mode=HEX_NONE;
   * // Are we processing an hex file?
   */
  ...
  /* See -d from */
  int cc_to_stdout;             /* If 1, captions go to stdout instead of
                                   file */
  ...
  unsigned teletext_warning_shown;      /* Did we detect a possible PAL (with 
                                         */

  /* teletext subs) and told the user already? */
  int epg_inited;
  ...
  unsigned inthauppauge_mode;   /* If 1, use PID=1003, process specially and
                                   so on */
  int live_stream;              /* -1 -> Not a complete file but a live
                                   stream, * without timeout 0 -> A regular
                                   file >0 -> Live * stream with a timeout of 
...
void usage (void);
int detect_input_file_overwrite (struct lib_ccx_ctx *ctx,
...
void return_to_buffer
  (struct ccx_demuxer *ctx, unsigned char *buffer, unsigned int bytes);
...
void store_hdcc
  (struct lib_cc_decode *ctx,
   ...
   int cc_count,
   int sequence_number, LLONG current_fts_now, struct cc_subtitle *sub);
...
int writeraw
  (const unsigned char *data,
   int length, void *private_data, struct cc_subtitle *sub);
...
int isValidMP4Box
  (unsigned char *buffer,
   size_t position, size_t * nextBoxLocation, int *boxScore);
...
int read_video_pes_header
  (struct ccx_demuxer *ctx,
   ...
   unsigned char *nextheader, int *headerlength, int sbuflen);
...
int parse_PMT
  (struct ccx_demuxer *ctx,
   unsigned char *buf, int len, struct program_info *pinfo);
...
void dump
  (LLONG mask,
   unsigned char *start,
   int l, unsigned long abs_start, unsigned clear_high_bit);
...
int levenshtein_dist
  (const uint64_t * s1, const uint64_t * s2, unsigned s1len, unsigned s2len);
void millis_to_date
  (uint64_t timestamp,
   char *buffer,
   enum ccx_output_date_format date_format, char millis_separator);
...
int tlt_process_pes_packet
  (struct lib_cc_decode *dec_ctx,
   uint8_t * buffer,
   uint16_t size, struct cc_subtitle *sub, int sentence_cap);
