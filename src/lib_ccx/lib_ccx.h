...
struct ccx_s_teletext_config
{
	...
	// uint8_t se_mode : 1;                                    // search engine
	// compatible mode => Uses CCExtractor's write_format uint64_t utc_refvalue;
	// // UTC referential value => Moved to ccx_decoders_common, so can be used
	// for other decoders (608/xds) too
	uint16_t user_page; // Page selected by user, which MIGHT be different to
						// 'page' depending on autodetection stuff
...
struct lib_ccx_ctx
{
	...
	int pes_header_to_stdout; // If 1, PES Header data will be outputted to
							  // console
	int dvb_debug_traces_to_stdout; // If 1, DVB subtitle debug traces will be
	...
	int current_file;	   // If current_file!=1, we are processing
	...
	unsigned
		teletext_warning_shown; // Did we detect a possible PAL (with teletext
								// subs) and told the user already?
...
// es_functions.c
size_t process_m2v(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
				   unsigned char *data, size_t length, struct cc_subtitle *sub);

extern unsigned top_field_first;

// es_userdata.c
int user_data(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
			  struct bitstream *ustream, int udtype, struct cc_subtitle *sub);
...
void store_hdcc(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
				unsigned char *cc_data, int cc_count, int sequence_number,
				LLONG current_fts_now, struct cc_subtitle *sub);
void anchor_hdcc(struct lib_cc_decode *ctx, int seq);
void process_hdcc(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
...
int writeraw(const unsigned char *data, int length, void *private_data,
			 struct cc_subtitle *sub);
void flushbuffer(struct lib_ccx_ctx *ctx, struct ccx_s_write *wb,
				 int closefile);
void writercwtdata(struct lib_cc_decode *ctx, const unsigned char *data,
				   struct cc_subtitle *sub);

// stream_functions.c
int isValidMP4Box(unsigned char *buffer, size_t position,
				  size_t *nextBoxLocation, int *boxScore);
void detect_stream_type(struct ccx_demuxer *ctx);
int detect_myth(struct ccx_demuxer *ctx);
int read_video_pes_header(struct ccx_demuxer *ctx, struct demuxer_data *data,
						  unsigned char *nextheader, int *headerlength,
						  int sbuflen);
...
void dump(LLONG mask, unsigned char *start, int l, unsigned long abs_start,
...
int levenshtein_dist(const uint64_t *s1, const uint64_t *s2, unsigned s1len,
					 unsigned s2len);
...
struct lib_cc_decode *update_decoder_list_cinfo(struct lib_ccx_ctx *ctx,
												struct cap_info *cinfo);
struct lib_cc_decode *update_decoder_list(struct lib_ccx_ctx *ctx);

struct encoder_ctx *update_encoder_list_cinfo(struct lib_ccx_ctx *ctx,
											  struct cap_info *cinfo);
