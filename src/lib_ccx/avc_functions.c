...
static void sei_rbsp(struct avc_ctx *ctx, unsigned char *seibuf,
					 unsigned char *seiend);
static unsigned char *sei_message(struct avc_ctx *ctx, unsigned char *seibuf,
								  unsigned char *seiend);
static void user_data_registered_itu_t_t35(struct avc_ctx *ctx,
										   unsigned char *userbuf,
										   unsigned char *userend);
static void seq_parameter_set_rbsp(struct avc_ctx *ctx, unsigned char *seqbuf,
								   unsigned char *seqend);
static void slice_header(struct encoder_ctx *enc_ctx, struct lib_cc_decode *ctx,
						 unsigned char *heabuf, unsigned char *heaend,
...
struct avc_ctx *init_avc(void)
{
	...
	ctx->cc_buffer_saved =
		CCX_TRUE; // Was the CC buffer saved after it was last updated?
...
void do_NAL(struct encoder_ctx *enc_ctx, struct lib_cc_decode *dec_ctx,
			unsigned char *NAL_start, LLONG NAL_length, struct cc_subtitle *sub)
{
	...
	NAL_stop = remove_03emu(NAL_start + 1,
							NAL_stop); // Add +1 to NAL_stop for TS, without it
									   // for MP4. Still don't know why

	dvprint("BEGIN NAL unit type: %d length %d ref_idc: %d - Buffered captions "
			"before: %d\n",
			nal_unit_type, NAL_stop - NAL_start - 1,
			dec_ctx->avc_ctx->nal_ref_idc, !dec_ctx->avc_ctx->cc_buffer_saved);

	if (NAL_stop == NULL) // remove_03emu failed.
	{
		mprint("\rNotice: NAL of type %u had to be skipped because "
			   "remove_03emu failed.\n",
			   nal_unit_type);
		return;
	}
	...
	else if (nal_unit_type == CCX_NAL_TYPE_SEQUENCE_PARAMETER_SET_7)
	{
		...
		// We need this to parse NAL type 1
		// (CCX_NAL_TYPE_CODED_SLICE_NON_IDR_PICTURE_1)
	...
	else if (dec_ctx->avc_ctx->got_seq_para &&
			 (nal_unit_type == CCX_NAL_TYPE_CODED_SLICE_NON_IDR_PICTURE_1 ||
			  nal_unit_type ==
				  CCX_NAL_TYPE_CODED_SLICE_IDR_PICTURE)) // Only if
														 // nal_unit_type=1
	{
		// Found coded slice of a non-IDR picture
		// We only need the slice header data, no need to implement
		// slice_layer_without_partitioning_rbsp( );
		slice_header(enc_ctx, dec_ctx, NAL_start + 1, NAL_stop, nal_unit_type,
					 sub);
	}
	else if (dec_ctx->avc_ctx->got_seq_para &&
			 nal_unit_type == CCX_NAL_TYPE_SEI)
	{
		...
		sei_rbsp(dec_ctx->avc_ctx, NAL_start + 1, NAL_stop);
			 ...
			 nal_unit_type == CCX_NAL_TYPE_PICTURE_PARAMETER_SET)
	{
	...
	dvprint("END   NAL unit type: %d length %d ref_idc: %d - Buffered captions "
			"after: %d\n",
			nal_unit_type, NAL_stop - NAL_start - 1,
			dec_ctx->avc_ctx->nal_ref_idc, !dec_ctx->avc_ctx->cc_buffer_saved);
}
...
size_t process_avc(struct encoder_ctx *enc_ctx, struct lib_cc_decode *dec_ctx,
				   unsigned char *avcbuf, size_t avcbuflen,
				   struct cc_subtitle *sub)
{
	...
	if (avcbuflen <= 5)
	{
		fatal(CCX_COMMON_EXIT_BUG_BUG, "NAL unit needs at last 5 bytes in the "
									   "buffer to process AVC video stream...");
	}
	...
	while (buffer_position <
		   avcbuf + avcbuflen -
			   2) // buffer_position points to 0x01 plus at least two bytes
	{
		...
		do
		{
			...
			buffer_position++;
			restlen = avcbuf - buffer_position + avcbuflen -
					  2; // leave room for two more bytes

			// Find the next zero
			if (restlen > 0)
			{
				buffer_position = (unsigned char *)memchr(buffer_position, 0x00,
														  (size_t)restlen);
				...
				if (buffer_position[1] == 0x00 &&
					(buffer_position[2] | 0x01) == 0x01)
				{
					// Found new start code
					NAL_stop = buffer_position;
					buffer_position =
						buffer_position + 2; // Move after the two leading 0x00
					break;
				}
			}
			else
...
int EBSPtoRBSP(unsigned char *streamBuffer, int end_bytepos, int begin_bytepos)
{
	...
	{ // starting from begin_bytepos to avoid header information
		...
		if (count == ZEROBYTES_SHORTSTARTCODE && streamBuffer[i] == 0x03)
		{
			// check the 4th byte after 0x000003, except when cabac_zero_word is
			// used, in which case the last three bytes of this NAL unit must be
			// 0x000003
			if ((i < end_bytepos - 1) && (streamBuffer[i + 1] > 0x03))
				return -1;
			// if cabac_zero_word is used, the final byte of this NAL unit(0x03)
			// is discarded, and the last two bytes of RBSP must be 0x0000
			if (i == end_bytepos - 1)
				return j;
...
// Remove 0x000003 emulation sequences. We do this on-site (same buffer) because
// the new array will never be longer than the source.
u32 avc_remove_emulation_bytes(const unsigned char *buffer_src,
							   unsigned char *buffer_dst, u32 nal_size);

unsigned char *remove_03emu(unsigned char *from, unsigned char *to)
{
	int num = to - from;
	int newsize = EBSPtoRBSP(
		from, num, 0); // TODO: Do something if newsize == -1 (broken NAL)
	if (newsize == -1)
		return NULL;
	return from + newsize;
}
...
void sei_rbsp(struct avc_ctx *ctx, unsigned char *seibuf, unsigned char *seiend)
{
	...
	else
	{
		...
		dump(CCX_DMT_GENERIC_NOTICES, (unsigned char *)seibuf, seiend - seibuf,
			 0, 0);
...
unsigned char *sei_message(struct avc_ctx *ctx, unsigned char *seibuf,
						   unsigned char *seiend)
{
	int payload_type = 0;
	while (*seibuf == 0xff)
	...
	if (seibuf > seiend)
	{
		...
		if (payload_type == 4)
		{
			dbg_print(CCX_DMT_VERBOSE,
					  "Warning: Subtitles payload seems incorrect (too long), "
					  "continuing but it doesn't look good..");
		}
		else
		{
			dbg_print(CCX_DMT_VERBOSE,
					  "Warning: Non-subtitles payload seems incorrect (too "
					  "long), continuing but it doesn't look good..");
		}
	}
	dbg_print(CCX_DMT_VERBOSE, "\n");
	// Ignore all except user_data_registered_itu_t_t35() payload
	if (!broken && payload_type == 4)
		user_data_registered_itu_t_t35(ctx, payload_start,
...
void copy_ccdata_to_buffer(struct avc_ctx *ctx, char *source, int new_cc_count)
{
	...
	{
		mprint("Warning: Probably loss of CC data, unsaved buffer being "
			   "rewritten, trailing end might get lost\n");
...
void user_data_registered_itu_t_t35(struct avc_ctx *ctx, unsigned char *userbuf,
									unsigned char *userend)
{
	...
	case 0x0031: // ANSI/SCTE 128
		...
		if (*tbuf == 0x47 && *(tbuf + 1) == 0x41 && *(tbuf + 2) == 0x39 &&
			*(tbuf + 3) == 0x34) // ATSC1_data() - GA94
		{
			...
			case 0x03:
				...
				/* if (!process_cc_data_flag)
				   {
				   mprint ("process_cc_data_flag == 0, skipping this caption
				   block.\n"); break;
				   } */
				/*
				   The following tests are not passed in Comcast's sample
				   videos. *tbuf here is always 0x41. if (! (*tbuf & 0x80)) //
				   First bit must be 1
				   {
				   printf ("Fixed bit should be 1, but it's 0 - skipping this
				   caption block.\n"); break;
				   }
				   if (*tbuf & 0x20) // Third bit must be 0
				   {
				   printf ("Fixed bit should be 0, but it's 1 - skipping this
				   caption block.\n"); break;
				   } */
				tbuf++;
				/*
				   ...
				   if (*tbuf!=0xFF)
				   {
				   printf ("Fixed value should be 0xFF, but it's %02X - skipping
				   this caption block.\n", *tbuf);
				   } */
				...
				if (cc_tmp_data[local_cc_count * 3] != 0xFF)
				{
					...
					mprint("\rWarning! Syntax problem: Final 0xFF marker "
						   "missing. Continuing...\n");
					break; // Skip Block
				}
				// Save the data and process once we know the sequence number
				if (((ctx->cc_count + local_cc_count) * 3) + 1 >
					ctx->cc_databufsize)
				{
					ctx->cc_data = (unsigned char *)realloc(
						ctx->cc_data,
						(size_t)((ctx->cc_count + local_cc_count) * 6) + 1);
					if (!ctx->cc_data)
						fatal(EXIT_NOT_ENOUGH_MEMORY,
							  "In user_data_registered_itu_t_t35: Out of "
							  "memory to allocate buffer for CC data.");
					ctx->cc_databufsize =
						(long)((ctx->cc_count + local_cc_count) * 6) + 1;
				}
		...
		else if (*tbuf == 0x44 && *(tbuf + 1) == 0x54 && *(tbuf + 2) == 0x47 &&
				 *(tbuf + 3) == 0x31) // afd_data() - DTG1
		{
			;
			// Active Format Description Data. Actually unrelated to captions.
			// Left here in case we want to do some reporting eventually. From
			// specs: "Active Format Description (AFD) should be included in
			// video user data whenever the rectangular picture area containing
			// useful information does not extend to the full height or width of
			// the coded frame. AFD data may also be included in user data when
			// the rectangular picture area containing useful information
			// extends to the fullheight and width of the coded frame."
		}
		else
		{
			dbg_print(CCX_DMT_VERBOSE, "SCTE/ATSC reserved.\n");
	...
	case 0x002F:
		...
		{
			dbg_print(CCX_DMT_VERBOSE,
					  "Not supported  user_data_type_code: %02x\n",
					  user_data_type_code);
			return;
		}
		tbuf++;
		...
		if ((((local_cc_count + ctx->cc_count) * 3) + 1) > ctx->cc_databufsize)
		{
			ctx->cc_data = (unsigned char *)realloc(
				ctx->cc_data,
				(size_t)(((local_cc_count + ctx->cc_count) * 6) + 1));
			if (!ctx->cc_data)
				fatal(EXIT_NOT_ENOUGH_MEMORY,
					  "In user_data_registered_itu_t_t35: Not enough memory "
					  "trying to allocate buffer for CC data.");
			ctx->cc_databufsize =
				(long)(((local_cc_count + ctx->cc_count) * 6) + 1);
		}
...
void seq_parameter_set_rbsp(struct avc_ctx *ctx, unsigned char *seqbuf,
							unsigned char *seqend)
{
	LLONG tmp, tmp1;
	struct bitstream q1;
		...
		profile_idc == 86 || profile_idc == 118 || profile_idc == 128)
	{
		...
		dvprint(
			"chroma_format_idc=                             % 4lld (%#llX)\n",
			chroma_format_idc, chroma_format_idc);
		if (chroma_format_idc == 3)
		{
			tmp = read_int_unsigned(&q1, 1);
			dvprint("separate_colour_plane_flag=                    % 4lld "
					"(%#llX)\n",
					tmp, tmp);
		}
		tmp = read_exp_golomb_unsigned(&q1);
		dvprint(
			"bit_depth_luma_minus8=                         % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_exp_golomb_unsigned(&q1);
		dvprint(
			"bit_depth_chroma_minus8=                       % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"qpprime_y_zero_transform_bypass_flag=          % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"seq_scaling_matrix_present_flag=               % 4lld (%#llX)\n",
			tmp, tmp);
		if (tmp == 1)
		{
			...
			{
				tmp = read_int_unsigned(&q1, 1);
				dvprint("seq_scaling_list_present_flag[%d]=                 % "
						"4lld (%#llX)\n",
						i, tmp, tmp);
				if (tmp)
				{
					// We use a "dummy"/slimmed-down replacement here.
					// Actual/full code can be found in the spec (ISO/IEC
					// 14496-10:2012(E)) chapter 7.3.2.1.1.1 - Scaling list
					// syntax
					if (i < 6)
					{
	...
	else if (ctx->pic_order_cnt_type == 1)
	{
		...
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"delta_pic_order_always_zero_flag=              % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_exp_golomb(&q1);
		dvprint(
			"offset_for_non_ref_pic=                        % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_exp_golomb(&q1);
		dvprint(
			"offset_for_top_to_bottom_field                 % 4lld (%#llX)\n",
			tmp, tmp);
		LLONG num_ref_frame_in_pic_order_cnt_cycle =
			read_exp_golomb_unsigned(&q1);
		dvprint(
			"num_ref_frame_in_pic_order_cnt_cycle           % 4lld (%#llX)\n",
			num_ref_frame_in_pic_order_cnt_cycle,
			num_ref_frame_in_pic_order_cnt_cycle);
		for (int i = 0; i < num_ref_frame_in_pic_order_cnt_cycle; i++)
		{
			tmp = read_exp_golomb(&q1);
			dvprint("offset_for_ref_frame [%d / %d] =               % 4lld "
					"(%#llX)\n",
	...
	if (!ctx->frame_mbs_only_flag)
	{
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"mb_adaptive_fr_fi_flag=                        % 4lld (%#llX)\n",
			tmp, tmp);
	}
	tmp = read_int_unsigned(&q1, 1);
	...
	if (tmp)
	{
		tmp = read_exp_golomb_unsigned(&q1);
		dvprint(
			"frame_crop_left_offset=                        % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_exp_golomb_unsigned(&q1);
		dvprint(
			"frame_crop_right_offset=                       % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_exp_golomb_unsigned(&q1);
		dvprint(
			"frame_crop_top_offset=                         % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_exp_golomb_unsigned(&q1);
		dvprint(
			"frame_crop_bottom_offset=                      % 4lld (%#llX)\n",
			tmp, tmp);
	}
	tmp = read_int_unsigned(&q1, 1);
	...
	if (tmp)
	{
		...
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"aspect_ratio_info_pres=                        % 4lld (%#llX)\n",
			tmp, tmp);
		if (tmp)
		{
			tmp = read_int_unsigned(&q1, 8);
			dvprint("aspect_ratio_idc=                              % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			if (tmp == 255)
			{
				tmp = read_int_unsigned(&q1, 16);
				dvprint("sar_width=                                     % 4lld "
						"(%#llX)\n",
						tmp, tmp);
				tmp = read_int_unsigned(&q1, 16);
				dvprint("sar_height=                                    % 4lld "
						"(%#llX)\n",
						tmp, tmp);
			}
		}
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"overscan_info_pres_flag=                       % 4lld (%#llX)\n",
			tmp, tmp);
		if (tmp)
		{
			tmp = read_int_unsigned(&q1, 1);
			dvprint("overscan_appropriate_flag=                     % 4lld "
					"(%#llX)\n",
					tmp, tmp);
		}
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"video_signal_type_present_flag=                % 4lld (%#llX)\n",
			tmp, tmp);
		if (tmp)
		{
			tmp = read_int_unsigned(&q1, 3);
			dvprint("video_format=                                  % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			tmp = read_int_unsigned(&q1, 1);
			dvprint("video_full_range_flag=                         % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			tmp = read_int_unsigned(&q1, 1);
			dvprint("colour_description_present_flag=               % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			if (tmp)
			{
				tmp = read_int_unsigned(&q1, 8);
				dvprint("colour_primaries=                              % 4lld "
						"(%#llX)\n",
						tmp, tmp);
				tmp = read_int_unsigned(&q1, 8);
				dvprint("transfer_characteristics=                      % 4lld "
						"(%#llX)\n",
						tmp, tmp);
				tmp = read_int_unsigned(&q1, 8);
				dvprint("matrix_coefficients=                           % 4lld "
						"(%#llX)\n",
						tmp, tmp);
			}
		}
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"chroma_loc_info_present_flag=                  % 4lld (%#llX)\n",
			tmp, tmp);
		if (tmp)
		{
			tmp = read_exp_golomb_unsigned(&q1);
			dvprint("chroma_sample_loc_type_top_field=                  % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			tmp = read_exp_golomb_unsigned(&q1);
			dvprint("chroma_sample_loc_type_bottom_field=               % 4lld "
					"(%#llX)\n",
					tmp, tmp);
		}
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"timing_info_present_flag=                      % 4lld (%#llX)\n",
			tmp, tmp);
		if (tmp)
		{
			tmp = read_int_unsigned(&q1, 32);
			LLONG num_units_in_tick = tmp;
			dvprint("num_units_in_tick=                             % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			tmp = read_int_unsigned(&q1, 32);
			LLONG time_scale = tmp;
			dvprint("time_scale=                                    % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			tmp = read_int_unsigned(&q1, 1);
			int fixed_frame_rate_flag = (int)tmp;
			dvprint("fixed_frame_rate_flag=                         % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			// Change: use num_units_in_tick and time_scale to calculate FPS.
			// (ISO/IEC 14496-10:2012(E), page 397 & further)
			if (fixed_frame_rate_flag)
			{
				...
				if (current_fps != (double)time_scale / (2 * num_units_in_tick))
				{
					current_fps =
						(double)time_scale /
						(2 * num_units_in_tick); // Based on formula D-2, p. 359
												 // of the ISO/IEC
												 // 14496-10:2012(E) spec.
		...
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"nal_hrd_parameters_present_flag=               % 4lld (%#llX)\n",
			tmp, tmp);
		if (tmp)
		{
			dvprint("nal_hrd. Not implemented for now. Hopefully not needed. "
					"Skipping rest of NAL\n");
			// printf("Boom nal_hrd\n");
			// exit(1);
			ctx->num_nal_hrd++;
			return;
		...
		{
			// TODO.
			mprint("vcl_hrd. Not implemented for now. Hopefully not needed. "
				   "Skipping rest of NAL\n");
			ctx->num_vcl_hrd++;
			// exit(1);
		}
		...
		{
			tmp = read_int_unsigned(&q1, 1);
			dvprint("low_delay_hrd_flag=                                % 4lld "
					"(%#llX)\n",
					tmp, tmp);
			return;
		}
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"pic_struct_present_flag=                       % 4lld (%#llX)\n",
			tmp, tmp);
		tmp = read_int_unsigned(&q1, 1);
		dvprint(
			"bitstream_restriction_flag=                    % 4lld (%#llX)\n",
			tmp, tmp);
		// ..
...
*/
void slice_header(struct encoder_ctx *enc_ctx, struct lib_cc_decode *dec_ctx,
				  unsigned char *heabuf, unsigned char *heaend,
				  int nal_unit_type, struct cc_subtitle *sub)
{
	LLONG tmp;
	...
	int current_index;
	int ird_pic_flag;
	LLONG field_pic_flag = 0; // Moved here because it's needed for
							  // ctx->avc_ctx->pic_order_cnt_type==2
	...
	dec_ctx->avc_ctx->frame_num =
		read_int_unsigned(&q1, dec_ctx->avc_ctx->log2_max_frame_num);
	...
	{
		pic_order_cnt_lsb = read_int_unsigned(
			&q1, dec_ctx->avc_ctx->log2_max_pic_order_cnt_lsb);
		dvprint("pic_order_cnt_lsb=     % 4llX\n", pic_order_cnt_lsb);
	}
	if (dec_ctx->avc_ctx->pic_order_cnt_type == 1)
	{
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In slice_header: AVC: ctx->avc_ctx->pic_order_cnt_type == 1 not "
			  "yet supported.");
	}
	...
	// sequence. B slices can be reference pics, so ignore
	// ctx->avc_ctx->nal_ref_idc.
	int isref = 0;
	switch (slice_type)
	{
	// P-SLICES
	...
	int maxrefcnt =
		(int)((1 << dec_ctx->avc_ctx->log2_max_pic_order_cnt_lsb) - 1);
	...
	if (isref && dec_ctx->frames_since_last_gop <=
					 3) // Used to be == 1, but the sample file
	...
	if (isref)
	{
		...
		if (ccx_options.usepicorder)
		{
			...
			if (dec_ctx->avc_ctx->lastmaxidx >
				dec_ctx->avc_ctx->currref +
					maxrefcnt / 2) // implies lastmaxidx > 0
	...
	if (ccx_options.usepicorder)
	{
		...
		if (dec_ctx->avc_ctx->lastmaxidx > 0)
		{
			dec_ctx->timing->current_tref =
				current_index - dec_ctx->avc_ctx->lastmaxidx - 1;
			// Set maxtref
			...
			if (dec_ctx->avc_ctx->last_gop_maxtref >
				dec_ctx->last_gop_length * 1.5)
			{
				dec_ctx->timing->current_tref =
	...
	else
	{
		...
		current_index = (int)roundportable(
			2 * (dec_ctx->timing->current_pts - dec_ctx->avc_ctx->currefpts) /
			(MPEG_CLOCK_FREQ / current_fps));
		...
		if (current_index == dec_ctx->avc_ctx->lastminidx)
		{
			...
			// fairly constant) sets the temporal reference to zero - needed to
			// set sync_pts.
			dec_ctx->timing->current_tref = 0;
		}
		if (dec_ctx->avc_ctx->lastmaxidx == -1)
		{
			// Set temporal reference to zero on minimal index and in the first
			// GOP to avoid setting a wrong fts_offset
			dec_ctx->timing->current_tref = 0;
		}
	}
	...
	dbg_print(CCX_DMT_TIME,
			  "  picordercnt:%3lld tref:%3d idx:%3d refidx:%3d lmaxidx:%3d "
			  "maxtref:%3d\n",
			  pic_order_cnt_lsb, dec_ctx->timing->current_tref, current_index,
			  dec_ctx->avc_ctx->currref, dec_ctx->avc_ctx->lastmaxidx,
			  dec_ctx->avc_ctx->maxtref);
	dbg_print(CCX_DMT_TIME, "  sync_pts:%s (%8u)",
			  print_mstime_static(dec_ctx->timing->sync_pts /
								  (MPEG_CLOCK_FREQ / 1000)),
			  (unsigned)(dec_ctx->timing->sync_pts));
	dbg_print(CCX_DMT_TIME, " - %s since GOP: %2u", slice_types[slice_type],
	...
	store_hdcc(enc_ctx, dec_ctx, dec_ctx->avc_ctx->cc_data,
			   dec_ctx->avc_ctx->cc_count, current_index,
			   dec_ctx->timing->fts_now, sub);
	dec_ctx->avc_ctx->cc_buffer_saved =
		CCX_TRUE; // CFS: store_hdcc supposedly saves the CC buffer to a
				  // sequence buffer
