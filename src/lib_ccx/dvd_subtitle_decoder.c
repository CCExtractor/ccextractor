...
int rle_decode(struct DVD_Ctx *ctx, int *color, uint8_t *nextbyte, int *pos,
			   int *m)
{
	int val;
	uint16_t rlen;
	...
	while (ctx->pos <= ctx->len && pack_end == 0)
	{
		...
		while (seq_end == 0)
		{
			...
			case 0x03: // SET_COLOR
				...
				// dbg_print(CCX_DMT_VERBOSE, "col: %x col: %x col: %x col:
				// %x\n", control->color[0], control->color[1],
				// control->color[2], control->color[3]);
				ctx->pos += 2;
				break;
			case 0x04: // SET_CONTR
				...
				// dbg_print(CCX_DMT_VERBOSE, "alp: %d alp: %d alp: %d alp:
				// %d\n", control->alpha[0], control->alpha[1],
				// control->alpha[2], control->alpha[3]);
				ctx->pos += 2;
				break;
			case 0x05: // SET_DAREA
				control->coord[0] =
					((buff[ctx->pos] << 8) | (buff[ctx->pos + 1] & 0xf0)) >>
					4; // starting x coordinate
				control->coord[1] = ((buff[ctx->pos + 1] & 0x0f) << 8) |
									buff[ctx->pos + 2]; // ending x coordinate
				control->coord[2] =
					((buff[ctx->pos + 3] << 8) | (buff[ctx->pos + 4] & 0xf0)) >>
					4; // starting y coordinate
				control->coord[3] = ((buff[ctx->pos + 4] & 0x0f) << 8) |
									buff[ctx->pos + 5]; // ending y coordinate
				// dbg_print(CCX_DMT_VERBOSE, "cord: %d cord: %d cord: %d cord:
				// %d\n", control->coord[0], control->coord[1],
				// control->coord[2], control->coord[3]);
				ctx->pos += 6;
				//(x2-x1+1)*(y2-y1+1)
				break;
			case 0x06: // SET_DSPXA - Pixel address
				control->pixoffset[0] =
					(buff[ctx->pos] << 8) | buff[ctx->pos + 1];
				control->pixoffset[1] =
					(buff[ctx->pos + 2] << 8) | buff[ctx->pos + 3];
...
void guess_palette(struct DVD_Ctx *ctx, uint32_t *rgba_palette,
...
int write_dvd_sub(struct lib_cc_decode *dec_ctx, struct DVD_Ctx *ctx,
				...
				struct cc_subtitle *sub)
{
	...
	else
	{
		...
		if (ctx->size_spu > length)
		{
			ctx->append = 1;
			dbg_print(CCX_DMT_VERBOSE,
					  "Data might be spread over several packets\n");
			return length;
		}
