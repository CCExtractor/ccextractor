...
/**
	We need stream info from PMT table when any of the following Condition
   meets: 1) Dont have any caption stream registered to be extracted 2) Want a
   streams per program, and program_number has never been registered 3) We need
   single stream and its info about codec and buffertype is incomplete
*/
int need_cap_info(struct ccx_demuxer *ctx, int program_number)
{
	...
	{
		list_for_each_entry(iter, &ctx->cinfo_tree.pg_stream, pg_stream,
							struct cap_info)
		{
...
int get_best_stream(struct ccx_demuxer *ctx)
{
	...
	list_for_each_entry(iter, &ctx->cinfo_tree.all_stream, all_stream,
						struct cap_info)
	{
		// If saw_pesstart is 0 then stream doesn't have payload. Helps against
		// empty streams
				   ...
				   int pn, void *private_data)
{
	...
	if (ctx->ts_datastreamtype != CCX_STREAM_TYPE_UNKNOWNSTREAM &&
		ctx->ts_datastreamtype != stream)
	{
		errno = EINVAL;
		return -1;
	}
	...
	list_for_each_entry(tmp, &ptr->all_stream, all_stream, struct cap_info)
	{
		if (tmp->pid == pid)
		{
			if (tmp->stream == CCX_STREAM_TYPE_UNKNOWNSTREAM ||
				tmp->codec == CCX_CODEC_NONE)
			{
...
void dinit_cap(struct ccx_demuxer *ctx)
{
	...
	{
		iter = list_entry(ctx->cinfo_tree.all_stream.next, struct cap_info,
						  all_stream);
...
struct cap_info *get_cinfo(struct ccx_demuxer *ctx, int pid)
{
						...
						struct cap_info)
	{
		if (iter->pid == pid && iter->codec != CCX_CODEC_NONE &&
			iter->stream != CCX_STREAM_TYPE_UNKNOWNSTREAM)
			return iter;
	}
	return NULL;
}
