...
void store_hdcc(struct encoder_ctx *enc_ctx, struct lib_cc_decode *dec_ctx,
				...
				LLONG current_fts_now, struct cc_subtitle *sub)
{
	...
	if (seq_index < 0 || seq_index > 2 * MAXBFRAMES)
	{
		...
		dbg_print(CCX_DMT_VERBOSE, "Too many B-frames, or missing anchor "
								   "frame. Trying to recover ..\n");
	...
	if (cc_count)
	{
		if (cc_data)
		{
			// Changed by CFS to concat, i.e. don't assume there's no data
			// already for this seq_index. Needed at least for MP4 samples. //
			// TODO: make sure we don't overflow
			dec_ctx->cc_fts[seq_index] =
				current_fts_now; // CFS: Maybe do even if there's no data?
			// if (stream_mode!=CCX_SM_MP4) // CFS: Very ugly hack, but looks
			// like overwriting is needed for at least some ES
			dec_ctx->cc_data_count[seq_index] = 0;
			memcpy(dec_ctx->cc_data_pkts[seq_index] +
					   dec_ctx->cc_data_count[seq_index] * 3,
				   cc_data, cc_count * 3 + 1);
		}
...
void process_hdcc(struct encoder_ctx *enc_ctx, struct lib_cc_decode *dec_ctx,
				  struct cc_subtitle *sub)
{
	...
	for (int seq = 0; seq < SORTBUF; seq++)
	{
		...
		// here for posterity.
		if (reset_cb < 0 && dec_ctx->cc_fts[seq] && seq < SORTBUF - 1 &&
			dec_ctx->cc_fts[seq + 1])
		{
		...
		process_cc_data(enc_ctx, dec_ctx, dec_ctx->cc_data_pkts[seq],
						dec_ctx->cc_data_count[seq], sub);
	}
