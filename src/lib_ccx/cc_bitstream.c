				   ...
				   unsigned char *end)
{
	...
	if (bstr->bitsleft < 0)
	{
		// See if we can somehow recover of this disaster by reporting the
...
uint64_t next_bits(struct bitstream *bstr, unsigned bnum)
{
	...
	if (bnum > 64)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In next_bits: Argument is greater than the maximum bit number "
			  "i.e. 64: %u!",
			  bnum);
	...
	if (bstr->end - bstr->pos < 0)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In next_bits: Bitstream can not have negative length!");
	...
	{
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In next_bits: Illegal bit position value %d!", vbit);
	}
	...
	while (1)
	{
		if (vpos >= bstr->end)
		{
			...
			fatal(CCX_COMMON_EXIT_BUG_BUG,
				  "In next_bits: Trying to read after end of data ...");
		}
...
int skip_bits(struct bitstream *bstr, unsigned bnum)
{
	...
	if (bstr->end - bstr->pos < 0)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In skip_bits: bitstream length cannot be negative!");
...
int is_byte_aligned(struct bitstream *bstr)
{
	...
	if (bstr->end - bstr->pos < 0)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In is_byte_aligned: bitstream length can not be negative!");
	...
	{
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In is_byte_aligned: Illegal bit position value %d!\n", vbit);
	}
...
void make_byte_aligned(struct bitstream *bstr)
{
	...
	if (bstr->end - bstr->pos < 0)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In make_byte_aligned: bitstream length can not be negative!");
	...
	{
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In make_byte_aligned: Illegal bit position value %d!\n", vbit);
	}
...
unsigned char *next_bytes(struct bitstream *bstr, unsigned bynum)
{
	...
	if (bstr->end - bstr->pos < 0)
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In next_bytes: bitstream length can not be negative!");
	...
	bstr->bitsleft =
		0LL + (bstr->end - bstr->pos - 1) * 8 + bstr->bpos - bynum * 8;
...
uint64_t bitstream_get_num(struct bitstream *bstr, unsigned bytes, int advance)
{
	...
	default:
		fatal(CCX_COMMON_EXIT_BUG_BUG,
			  "In bitstream_get_num: Illegal precision value [%u]!", bytes);
		break;
	}
	for (unsigned i = 0; i < bytes; i++)
	{
		unsigned char *ucpos =
			((unsigned char *)bpos) + bytes - i - 1; // Read backwards
