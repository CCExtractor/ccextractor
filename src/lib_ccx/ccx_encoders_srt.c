						 ...
						 LLONG ms_start, LLONG ms_end)
{
	...
	if (el == NULL || unescaped == NULL)
		fatal(EXIT_NOT_ENOUGH_MEMORY,
			  "In write_stringz_as_srt() - not enough memory.\n");
	int pos_r = 0;
	int pos_w = 0;
							 ...
							 struct encoder_ctx *context)
{
	...
	while (sub)
	{
		if (sub->type == CC_TEXT)
		{
			ret = write_stringz_as_srt(sub->data, context,
						   ...
						   struct encoder_ctx *context)
{
	...
	int prev_line_start = -1,
		prev_line_end = -1; // Column in which the previous line started and
							// ended, for autodash
	int prev_line_center1 = -1,
		prev_line_center2 = -1; // Center column of previous line text
	int empty_buf = 1;
	for (int i = 0; i < 15; i++)
	{
		if (data->row_used[i])
	...
	for (int i = 0; i < 15; i++)
	{
		if (data->row_used[i])
		{
			...
			if (context->autodash && context->trim_subs)
			{
				...
				if (first == -1 ||
					last == -1) // Probably a bug somewhere though
					break;
				// Is there a speaker named, for example: TOM: What are you
				// doing?
				...
				if (first > prev_line_start &&
					last < prev_line_end) // Fully contained
					do_dash = 0;
				if ((first > prev_line_start &&
					 first < prev_line_end) || // Overlap
				...
				if (center1 >= prev_line_center1 - 1 &&
					...
					center1 != -1) // Center align
					do_dash = 0;
				if (center2 >= prev_line_center2 - 2 &&
					center1 <= prev_line_center2 + 2 &&
					center1 != -1) // Center align
					do_dash = 0;
			...
			int length =
				get_decoder_line_encoded(context, context->subline, i, data);
			if (context->encoding != CCX_ENC_UNICODE)
			{
				dbg_print(CCX_DMT_DECODER_608, "\r");
				dbg_print(CCX_DMT_DECODER_608, "%s\n", context->subline);
