						  ...
						  LLONG ms_start, LLONG ms_end)
{
	...
	while (begin < unescaped + len)
	{
		...
		ret = write(context->out->fh, context->encoded_br,
					context->encoded_br_length);
							...
							struct encoder_ctx *context)
{
	...
	{
		sprintf(buf, "<SYNC start=%llu><P class=\"UNKNOWNCC\">\r\n",
				(unsigned long long)ms_start);
		...
		for (int i = sub->nb_data - 1; i >= 0; i--)
		{
			if (rect[i].ocr_text && *(rect[i].ocr_text))
			{
				if (context->prev_start != -1 || !(sub->flags & SUB_EOD_MARKER))
				{
					...
					write(context->out->fh, buf, strlen(buf));
					if (i != 0)
						write(context->out->fh, context->encoded_br,
							  context->encoded_br_length);
							  ...
							  struct encoder_ctx *context)
{
	...
	while (sub)
	{
		if (sub->type == CC_TEXT)
		{
			ret = write_stringz_as_sami(sub->data, context, sub->start_time,
										sub->end_time);
			freep(&sub->data);
			sub->nb_data = 0;
		}
