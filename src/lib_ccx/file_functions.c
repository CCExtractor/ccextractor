...
#include "lib_ccx.h"
long FILEBUFFERSIZE =
	1024 * 1024 * 16; // 16 Mbytes no less. Minimize number of real read calls()
...
LLONG get_total_file_size(
	struct lib_ccx_ctx *ctx) // -1 if one of the file(s) failed to open
{
	LLONG ts = 0;
	int h;
...
/* bytesinbuffer is the number of bytes read (in some buffer) that haven't been
added to 'past' yet. We provide this number to switch_to_next_file() so a final
...
int switch_to_next_file(struct lib_ccx_ctx *ctx, LLONG bytesinbuffer)
{
	...
	if (ctx->demux_ctx->is_open(ctx->demux_ctx))
	{
		...
		{
			mprint("\n\n\n\nATTENTION!!!!!!\n");
			mprint("In switch_to_next_file(): Processing of %s %d ended "
				   "prematurely %lld < %lld, please send bug report.\n\n",
		...
		if (ccx_options.binary_concat)
		{
			...
			ctx->demux_ctx->past =
				0; // Reset always or at the end we'll have double the size
		}
	}
	for (;;)
	{
		...
		// The following \n keeps the progress percentage from being
		// overwritten.
		mprint("\n\r-----------------------------------------------------------"
			   "------\n");
		mprint("\rOpening file: %s\n", ctx->inputfile[ctx->current_file]);
		ret = ctx->demux_ctx->open(ctx->demux_ctx,
								   ctx->inputfile[ctx->current_file]);
		if (ret < 0)
	...
	if (ctx->infd != -1)
	{
		...
		if (realpos != ctx->past - ctx->filebuffer_pos + ctx->bytesinbuffer)
		{
			fatal(
				...
				"Position desync, THIS IS A BUG. Real pos =%lld, past=%lld.\n",
				realpos, ctx->past);
		}
	}
#endif
...
void buffered_seek(struct ccx_demuxer *ctx, int offset)
{
	...
	if (offset < 0)
	{
		...
		if (ctx->filebuffer_pos < 0)
		{
			...
			{
				fatal(CCX_COMMON_EXIT_BUG_BUG, "PANIC: Attempt to seek before "
											   "buffer start, this is a bug!");
			}
...
void sleepandchecktimeout(time_t start)
{
	...
	{
		// CFS: Not 100% sure about this. Fine for files, not so sure what
		// happens if stdin is real time input from hardware.
		sleep_secs(1);
	...
	if (time(NULL) >
		start + ccx_options.live_stream) // More than live_stream seconds
										 // elapsed. No more live
		ccx_options.live_stream = 0;
	else
		sleep_secs(1);
}

void return_to_buffer(struct ccx_demuxer *ctx, unsigned char *buffer,
					  unsigned int bytes)
{
	...
	if (ctx->filebuffer_pos >
		0) // Discard old bytes, because we may need the space
	{
		// Non optimal since data is moved later again but we don't care since
		// we're never here in ccextractor.
		memmove(ctx->filebuffer, ctx->filebuffer + ctx->filebuffer_pos,
...
/**
 * @param buffer can be NULL, in case when user want to just buffer it or skip
 * some data.
 *
 ...
 */
size_t buffered_read_opt(struct ccx_demuxer *ctx, unsigned char *buffer,
						 size_t bytes)
{
	...
	{
		// Needs to return data from filebuffer_start+pos to
		// filebuffer_start+pos+bytes-1;
		int eof = (ctx->infd == -1);
		...
		while ((!eof || ccx_options.live_stream) && bytes)
		{
			...
			if (eof)
			{
				// No more data available immediately, we sleep a while to give
				// time for the data to come up
			...
			if (ready == 0) // We really need to read more
			{
				...
				{
					// We got in the buffering code because of the initial
					// buffer for detection stuff. However we don't want more
					// buffering so we do the rest directly on the final buffer.
					int i;
					do
					{
						// No code for network support here, because network is
						// always buffered - if here, then it must be files.
						if (buffer != NULL) // Read
						{
							i = read(ctx->infd, buffer, bytes);
							if (i == -1)
								fatal(EXIT_READ_ERROR,
						...
						else // Seek
						{
							LLONG op, np;
							op = LSEEK(ctx->infd, 0,
									   SEEK_CUR); // Get current pos
							if (op + bytes < 0)	  // Would mean moving beyond
												// start of file: Not supported
								return 0;
							np = LSEEK(ctx->infd, bytes,
									   SEEK_CUR); // Pos after moving
							i = (int)(np - op);
						}
						// if both above lseek returned -1 (error); i would be 0
						// here and in case when its not live stream copied
						// would decrease and bytes would...
						...
						else
						{
							...
							bytes -= i;
							  ...
							  (ccx_options.binary_concat &&
							   switch_to_next_file(ctx->parent, copied))) &&
							 bytes);
					return copied;
				}
				...
				memmove(ctx->filebuffer,
						ctx->filebuffer + (FILEBUFFERSIZE - keep), keep);
				int i;
				if (ccx_options.input_source == CCX_DS_FILE ||
					ccx_options.input_source == CCX_DS_STDIN)
					i = read(ctx->infd, ctx->filebuffer + keep,
							 FILEBUFFERSIZE - keep);
				else if (ccx_options.input_source == CCX_DS_TCP)
					i = net_tcp_read(ctx->infd, (char *)ctx->filebuffer + keep,
									 FILEBUFFERSIZE - keep);
				else
					i = net_udp_read(ctx->infd, (char *)ctx->filebuffer + keep,
									 FILEBUFFERSIZE - keep, ccx_options.udpsrc,
									 ccx_options.udpaddr);
				if (terminate_asap) /* Looks like receiving a signal here will
									   trigger a -1, so check that first */
					break;
				if (i == -1)
					fatal(EXIT_READ_ERROR, "Error reading input stream!\n");
				if (i == 0)
				{
					/* If live stream, don't try to switch - acknowledge eof
					   here as it won't cause a loop end */
					if (ccx_options.live_stream ||
						((struct lib_ccx_ctx *)ctx->parent)->inputsize <=
							origin_buffer_size ||
						!(ccx_options.binary_concat &&
						  switch_to_next_file(ctx->parent, copied)))
						eof = 1;
	...
	else // Read without buffering
	{
		...
		if (buffer != NULL)
		{
			...
			while (bytes > 0 && ctx->infd != -1 &&
				   ((i = read(ctx->infd, buffer, bytes)) != 0 ||
		...
		while (bytes != 0 && ctx->infd != -1)
		{
			...
			if (op + bytes <
				0) // Would mean moving beyond start of file: Not supported
				return 0;

			np = LSEEK(ctx->infd, bytes, SEEK_CUR); // Pos after moving
			if (op == -1 &&
				np == -1) // Possibly a pipe that doesn't like "skipping"
			{
				char c;
