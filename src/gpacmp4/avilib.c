...
int avi_parse_input_file(avi_t *AVI, int getIndex)
{
  ...
  /* Read first 12 bytes and check that this is an AVI file */
   ...
   if (AVI->is_opendml) {
      ...
      nvi = 0;
      for(audtr=0; audtr<AVI->anum; ++audtr) { nai[audtr] = 0; tot[audtr] = 0; }
...
{
   if(AVI->mode==AVI_MODE_WRITE) { AVI_errno = AVI_ERR_NOT_PERM; return (u64) -1; }
   if(!AVI->video_index)         { AVI_errno = AVI_ERR_NO_IDX;   return (u64) -1; }
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes, int *continuous)
{
   ...
   *continuous = 1;
   ...
   {
       s64 ret;
      left = (long) (AVI->track[AVI->aptr].audio_index[AVI->track[AVI->aptr].audio_posc].len - AVI->track[AVI->aptr].audio_posb);
      if(left==0)
      {
