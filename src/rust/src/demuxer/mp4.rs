...
extern "C" {
    ...
    fn ccx_mp4_process_hevc_sample(
        ctx: *mut lib_ccx_ctx,
        timescale: c_uint,
        nal_unit_size: u8,
        data: *mut u8,
...
pub unsafe fn dumpchapters_rust(_ctx: *mut lib_ccx_ctx, path: &CStr) -> c_int {
    ...
    let base = path_display.to_string();
    let out_name = format!("{}.txt", base);

    let out_cstr = std::ffi::CString::new(out_name.as_str()).unwrap();
    let mode = std::ffi::CString::new("wt").unwrap();
    let file = libc::fopen(out_cstr.as_ptr(), mode.as_ptr());
    if file.is_null() {
        return 5;
    }

    let writing_msg = format!("Writing chapters into {}\n\0", out_name);
    libc::printf(writing_msg.as_ptr() as *const c_char);
    ...
    for i in 0..nb_chapters {
        ...
        let line2 = format!("CHAPTER{:02}NAME={}\n\0", i + 1, title);
        libc::fprintf(file, line1.as_ptr() as *const c_char);
        libc::fprintf(file, line2.as_ptr() as *const c_char);
    }
