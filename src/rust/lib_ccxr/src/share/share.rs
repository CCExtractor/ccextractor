...
use libc::size_t;
use nanomsg_sys::{nn_bind, nn_send, nn_setsockopt, nn_shutdown, nn_socket, AF_SP, NN_LINGER, NN_PUB, NN_SOL_SOCKET};
use prost::Message;
use std::cmp::PartialEq;
use std::ffi::c_void;
use std::os::raw::{c_char, c_int};
use std::sync::{LazyLock, Mutex};
use std::{ffi::{CStr, CString}, thread, time::Duration};
...
pub unsafe fn ccxr_share_start(stream_name: Option<&str>) -> CcxShareStatus {
    ...
    CCX_SHARE_CTX.lock().unwrap().stream_name = Some(stream_name.unwrap_or("unknown").to_string());
...
pub unsafe fn ccxr_share_stop() -> CcxShareStatus {
    ...
    nn_shutdown(CCX_SHARE_CTX.lock().unwrap().nn_sock, CCX_SHARE_CTX.lock().unwrap().nn_binder);
...
pub unsafe fn _ccxr_share_send(msg: &CcxSubEntryMessage) -> CcxShareStatus {
    ...
    ccxr_sub_entry_message_pack(msg, &mut buf).expect(
